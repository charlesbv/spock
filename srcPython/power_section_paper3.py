...
# PLOT
if 'local_time_orbit' in sys.argv:
    ## LOCAL TIME OVER ONE ORBIT
    width_fig = 17
    height_fig = 11
    fontsize_plot = 20 # 9
    ...
    ax1.plot(x_axis, local_time[isat, index_orbit_start:index_orbit_stop],'k.', linewidth = 2)

    ax1.margins(0,0) # autoscale both axes(fist value is for the x axis, second value for the y axis)

    dy_ticks = 3 * 15
    y_ticks = np.arange(0,361,dy_ticks)
    ax1.yaxis.set_ticks(y_ticks)
    y_ticklabel_temp = [  datetime.strptime("00:00:00", "%H:%M:%S") + timedelta(hours = x) for x in np.arange(0, 25, 3) ]
    y_ticklabel = []
    for i in range(len(y_ticklabel_temp)):
        y_ticklabel.append( datetime.strftime(y_ticklabel_temp[i], "%H:%M") )
    ax1.yaxis.set_ticklabels( y_ticklabel, fontsize = fontsize_plot )

    gs.update(left=0.05, right=0.97, top = 0.90,bottom = 0.06)
    ax1.set_title('CYGNSS local time over one orbit (' + date[index_orbit_start] + ' to ' + date[index_orbit_stop][11:16] + ')', weight = 'bold', fontsize = 20,  y = 1.005) 
    ax1.legend()
    ax1.set_xlabel('Time in orbit (min)', fontsize = fontsize_plot, weight = 'bold')
    ax1.set_ylabel('Local time (HH:MM)', fontsize = fontsize_plot, weight = 'bold')

    fig_save_name = 'local_time_orbit'
...
#    index_orbit_start = index_time_average[isat, local_max[isat][1], 0]; index_orbit_stop = index_time_average[isat, local_max[isat][1]+1, 0]
    x_axis = np.arange(0, (index_orbit_stop - index_orbit_start) * dt /60., dt/60.)
    ax1.plot(x_axis, power[isat, index_orbit_start:index_orbit_stop, 0] + power[isat, index_orbit_start:index_orbit_stop, 1] + power[isat, index_orbit_start:index_orbit_stop, 2],'b', linewidth = 2, label = 'Top' )
    ax1.plot(x_axis, power[isat, index_orbit_start:index_orbit_stop, 4],'r', linewidth = 2, label = 'Front' )
    ax1.plot(x_axis, power[isat, index_orbit_start:index_orbit_stop, -1],'r', linewidth = 4, label = 'Back', linestyle = 'dotted' )
    ax1.plot(x_axis, total_power[isat, index_orbit_start:index_orbit_stop],'k', linewidth = 2, label = 'Total' )
    ax1.margins(0,0) # autoscale both axes(fist value is for the x axis, second value for the y axis)

    ax1.set_title('CYGNSS power over one orbit (' + date[index_orbit_start] + ' to ' + date[index_orbit_stop][11:16] + ')', weight = 'bold', fontsize = 20,  y = 1.005) 
    ax1.legend()
    ax1.set_xlabel('Time in orbit (min)', fontsize = fontsize_plot, weight = 'bold')
    ax1.set_ylabel('Power (W)', fontsize = fontsize_plot, weight = 'bold')

    fig_save_name = 'power_orbit'
    fig_save_name = root_save_fig_name + fig_save_name + '.eps'
    fig.savefig(fig_save_name, facecolor=fig.get_facecolor(), edgecolor='none', bbox_inches='tight')  
    os.system("rsync -av " + fig_save_name +' srbwks2014-0008.engin.umich.edu:./' + name_mission)

if 'power_year' in sys.argv:
    ...
    isat = 0
    ax1.plot( power_average[isat],'k', linewidth = 2 )
    ax1.margins(0,0) # autoscale both axes(fist value is for the x axis, second value for the y axis)
    show_local_min_max = 1
    if show_local_min_max == 1:
    ...
    ax1.set_ylim([0, max(power_average[isat])])

    ax1.set_title(input_filename.split('/')[-1].split('.')[0].replace("_"," ").title() + ' orbit-average power', weight = 'bold', fontsize = 20,  y = 1.025) 
    ax1.legend(loc = 3, fontsize = fontsize_plot)
    ax1.set_xlabel('Real Time', fontsize = fontsize_plot, weight = 'bold')
    ax1.set_ylabel('Power (W)', fontsize = fontsize_plot, weight = 'bold')

    fig_save_name = 'power_year'
    fig_save_name = root_save_fig_name + fig_save_name + '.pdf'
    fig.savefig(fig_save_name, facecolor=fig.get_facecolor(), edgecolor='none', bbox_inches='tight')  
    os.system("rsync -av " + fig_save_name +' srbwks2014-0008.engin.umich.edu:./' + name_mission)

if 'ltan_year' in sys.argv:
    ## LTAN OVER ONE YEAR
    width_fig = 15
    height_fig = width_fig * 3./4
    fontsize_plot = 20 # 9
    ...
    isat = 0
    ltan_degree = np.zeros(len(ltan[isat]))
    for iorbit in range(len(ltan[isat])):
        ltan_degree[iorbit]  = ( datetime.strptime(ltan[isat][iorbit], "%H:%M:%S") - datetime.strptime("00:00:00", "%H:%M:%S") ).seconds
    gs.update(left=0.05, right=0.97, top = 0.90,bottom = 0.06)
    ax1.plot(ltan_degree, 'k.',linewidth = 2 )
    dy_ticks = 3*3600
    y_ticks = np.arange(0,25*3600,dy_ticks)
    ax1.yaxis.set_ticks(y_ticks)
    y_ticklabel_temp = [  datetime.strptime("00:00:00", "%H:%M:%S") + timedelta(hours = x) for x in np.arange(0, 25, 3) ]
    y_ticklabel = []
    for i in range(len(y_ticklabel_temp)):
    ...
    ax1.set_title('Local time of the ascending node', weight = 'bold', fontsize = 20,  y = 1.02) 
    ax1.set_xlabel('Real Time', fontsize = fontsize_plot, weight = 'bold')
    ax1.set_ylabel('Local time of the ascending node (HH:MM)', fontsize = fontsize_plot, weight = 'bold')
...
raise Exception
#################################
fig_save_name = get_prop_dir(3) + 'output/python_propagator/paper3/cygnss_ltan_and_raan.eps' 
fig.savefig(fig_save_name, facecolor=fig.get_facecolor(), edgecolor='none', bbox_inches='tight')  
os.system("rsync -av " + fig_save_name +' srbwks2014-0008.engin.umich.edu:"Google\\ Drive/Work/PhD/Research/Defense/Notes/Astrodynamics/Figures/"')
