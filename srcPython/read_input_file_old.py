...
def read_input_file(filename):
    #filename = "ex_main_input_basic.txt"
                ...
                try: 
                    ...
                    try: 
                        ...
                        try: 
                            ...
                            print "The start date should have one of these formats: %Y-%m-%d %H:%M:%S or %Y-%m-%d %H:%M or %Y-%m-%dT%H:%M:%S or %Y-%m-%dT%H:%M or 'now n' (n being a number). The program will stop."
                            raise Exception
    ...
    if (start_date_now_n == 1): # if the user inputs the start date with 'now n' (n being the number of (decimal) hours to run the propagation for)
        if read_file_input[1+line_start_section_output].split()[0].replace("\n", "") != "now":
            if read_file_input[1+line_start_section_output].split()[0].replace("\n", "") != "out":
                one_of_the_filename_output = "spock/" + read_file_input[1+line_start_section_output].split()[0].replace("\n","") + "/" + read_file_input[1+line_start_section_output].split()[0].replace("\n","") + str(1) + "/" + read_file_input[1+line_start_section_output].split()[0].replace("\n","")  + str(1) + ".txt"
            else:
                one_of_the_filename_output = "spock/" + filename_no_path.replace(".txt","") + "/" + filename_no_path.replace(".txt","") + str(1) + "/" + filename_no_path
        else:
    ...
    if read_file_input[1+line_start_section_output].split()[0].replace("\n", "") != "now":
        if read_file_input[1+line_start_section_output].split()[0].replace("\n", "") != "out":
            for i in range(nb_sc):
                output_file_path_list.append("spock/" + read_file_input[1+line_start_section_output].split()[0] + "/" + read_file_input[1+line_start_section_output].split()[0] + str(i+1) + "/")
                output_file_name_list.append(read_file_input[1+line_start_section_output].split()[0] + str(i+1) + ".txt")
                collision_filename = "spock/" + read_file_input[1+line_start_section_output].split()[0] + "/" + read_file_input[1+line_start_section_output].split()[0] + "_collision.txt"

        else:
            for i in range(nb_sc):
                output_file_path_list.append( "spock/" + filename_no_path.replace(".txt","") + "/" + filename_no_path.replace(".txt","") + str(i+1) + "/" )
    #"spock/" + read_file_input[1+line_start_section_output].split()[0] + "/" + read_file_input[1+line_start_section_output].split()[0] + str(i+1) + "/")
                output_file_name_list.append(filename_no_path.replace(".txt","") + str(i+1) + ".txt")
                collision_filename = "spock/" + filename_no_path.replace(".txt","") + "/" + filename_no_path.replace(".txt","") + "_collision.txt"
    #"spock/" + read_file_input[1+line_start_section_output].split()[0] + "/" + read_file_input[1+line_start_section_output].split()[0] + "_collision.txt"

    else:
        newest_run = max(glob.iglob('spock/*/'), key=os.path.getctime)
        for i in range(nb_sc):
            output_file_path_list.append( newest_run + newest_run.split('/')[-2] + str(i+1) + "/")
            output_file_name_list.append(newest_run.split('/')[-2] + str(i+1) + ".txt")
            collision_filename = newest_run + newest_run.split('/')[-2] +  "_collision.txt"
    ...
    if line_start_section_kalman < len(read_file_input) - 1:
        ...
        filename_kalman_meas = file_kalman_init.readlines()[0].split()[0]
    ...
    variables.append(filename_kalman_meas)
    order_variables.append("filename_kalman_meas | " + str(len(order_variables)))
