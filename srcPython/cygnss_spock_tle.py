...
mpi_path = 'mpirun-openmpi-gcc49' # path of mpi run for the machine running this script
run_dir = "run.cygnss" # SpOCK run directory 
N_min_propagation = 200
...
os.system("python download_tle_spacetrack_run_automatically.py") 
tle_filename_all_sc_all_time = "./cygnss/2016-12-20--" + datetime.strftime( datetime.today(), "%Y-%m-%d") + ".txt" # name of the file including all TLEs of all CYGNSS from the launch until today
...
while iline < n: # go over each TLE in the file
    ...
    while ( ( sc_name_temp == sc_name[-1] ) & ( iline < n ) ): # the file includes blocks of TLE for each sc. Here go over a block for a given sc
        ...
        dt = 5 # dt that will be used to run SpOCK
        dt_output = 60
        ...
        main_input_filename = "CYG" + sc_name[-1] + "_" + date_tle_start_tle.replace(".","_") + "_temp.txt"
    ...
    "nadir",
        ...
        os.chdir("../" + run_dir)
        os.system(mpi_path + " -np 1 spock " + main_input_filename)
        os.chdir("../srcPython")
