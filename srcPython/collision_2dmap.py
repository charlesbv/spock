...
run_list = ['1126_ok_quartile_f107_5_quartile_ap_5.txt']
...
min_lat_array = [-90, min_lat -  range_lat / 10, min_lat - 30]#21.17] #-10
max_lon_array = [180, max_lon + range_lon / 10,  min_lon + 30]#153.35] #60
max_lat_array = [90, max_lat + range_lat / 10, min_lat + 30]#22.32]#40
...
m.drawcoastlines(linewidth=2.7, color='blue')
#m.bluemarble()                                                                                                               
...
for isat in range(nb_satellites):
    ...
    if isat == 0:
        m.plot(x, y,   markersize=10,color = scalarMap.to_rgba(52),markeredgecolor = 'none', label = run_list[irun].split('f107_')[1].split('_ap')[0].replace("_quartile", "") + "0%", linewidth = 6)[0]
    else:
        m.plot(x, y,   markersize=10,color = 'magenta',markeredgecolor = 'none', linewidth = 6 )[0]
