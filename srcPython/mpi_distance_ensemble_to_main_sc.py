...
dt = input_variables[2]; nb_steps = input_variables[3]; satellite_to_plot_path = input_variables[6]; satellite_to_plot = input_variables[7]; nb_ensembles_coe = input_variables[12]; nb_ensembles_attitude = input_variables[13]; nb_ensembles_cd = input_variables[11]; ensemble_to_plot_from_main_input_file_temp = input_variables[14]; 
...
for i in range(len(ensemble_to_plot_from_main_input_file_temp)):
    if (ensemble_to_plot_from_main_input_file_temp[i] == 'eci_r'):
    ...
    if (ensemble_to_plot_from_main_input_file_temp[i] == 'eci_v'):
    ...
    if (ensemble_to_plot_from_main_input_file_temp[i] == 'geodetic'):
    ...
    if (ensemble_to_plot_from_main_input_file_temp[i] == 'power'):
        ensemble_to_plot_from_main_input_file.append('power')
    if (ensemble_to_plot_from_main_input_file_temp[i] == 'attitude'):
    ...
    if (ensemble_to_plot_from_main_input_file_temp[i] == 'oe'):
    ...
    if ((ensemble_to_plot_from_main_input_file_temp[i] == 'density') & (compute_drag == 1)):
    ...
    if (ensemble_to_plot_from_main_input_file_temp[i] == 'collision'):
...
for isat in range(1):#:nb_spacecraft):
    pickle_list = []
    pickle_list_name = []
    ...
    os.system("mkdir " + path_folder_results + name_mission + '/pickle/' + name_subfolder_save + " &>/dev/null")
    #    os.system("mkdir " + path_folder_results + name_mission + '/result/video/' + name_subfolder_save )

    save_pickle_name = path_folder_results + name_mission + '/pickle/' + name_subfolder_save + satellite_to_plot[isat].replace(".txt","_") 
