...
filename_meas = "/Users/cbv/Google Drive/Work/PhD/Research/Code/kalman/spock/spock/true/true1/noise_true1.txt" #"/Users/cbv/kalman/cbv/spock/netcdf/output/cyg07.ddmi.s20170609-000000-e20170609-235959.l1.power-brcs.sand004.txt"
...
if compare_out != "":
    ...
    ax1.tick_params(axis='both', which='major', labelsize=fontsize_plot, size = 10, width = 2, pad = 7)
    ax1.xaxis.set_ticklabels("")
    ax1.scatter(x_axis, r_kalm[:,0] - r_comp[:,0], linewidth = 2,label='Kalman', color = 'b')
    ax1.plot(x_axis, r_comp[:,0] - r_comp[:,0], linewidth = 4,label='True', color = 'r')
    #legend = ax1.legend(loc = 'upper right',  fontsize = fontsize_plot)
    ax1.margins(0,0)
    ...
    ax2.tick_params(axis='both', which='major', labelsize=fontsize_plot, size = 10, width = 2, pad = 7)
    ax2.xaxis.set_ticklabels("")
    ax2.scatter(x_axis, r_kalm[:,1] - r_comp[:,1], linewidth = 2,label='Kalman', color = 'b')
    ax2.plot(x_axis, r_comp[:,1] - r_comp[:,1], linewidth = 4,label='True', color = 'r')
    ax2.margins(0,0)
    ...
    [i.set_linewidth(2) for i in ax3.spines.itervalues()] # change the width of the frame of the figure
    ax3.tick_params(axis='both', which='major', labelsize=fontsize_plot, size = 10, width = 2, pad = 7) 
    ax3.scatter(x_axis, r_kalm[:,2] - r_comp[:,2], linewidth = 2,label='Kalman', color = 'b')
    ax3.plot(x_axis, r_comp[:,2] - r_comp[:,2], linewidth = 4,label='True', color = 'r')
    ax3.margins(0,0)    
    ...
    print "Magnitude of delta r at time " + str(itime) + ": ", np.linalg.norm(mag_delta_r_temp), "km"
