...
for i in range(len(ensemble_to_plot_temp)):
    ...
    if ((ensemble_to_plot_temp[i] == 'cd') & (compute_drag == 1)):
        ensemble_to_plot.append('cd')
    if ((ensemble_to_plot_temp[i] == 'srp') & (compute_drag == 1)):
        ensemble_to_plot.append('srp')
...
for ipickle in range(nb_pickle):
    ...
    if ( 'srp_ensemble' in pickle_list_name_temp ):
        srp_ensemble = pickle_list[ pickle_list_name_temp.index('srp_ensemble') ]
...
step_std = 0 # step in hours to calculate the standard deviation
...
#            os.system("rsync -av " + fig_save_name +" srbwks2014-0008.engin.umich.edu:./" + name_mission + '/' + name_subfolder_save)
        ...
        step_std_in_index = (int)(step_std * 3600. / dt)
...
if "cd" in sys.argv:
    if ( 'cd_ensemble' in pickle_list_name ):
        ...
        ax2 = fig.add_subplot(gs[0, 0])
        ax2.set_title('BC distribution', weight = 'bold', fontsize = fontsize_plot,  y = 1.008)# ' + 'after ' + '{0:.0f}'.format(when_plot_in_hour / 24.) + ' days of propagation', weight = 'bold', fontsize = fontsize_plot,  y = 1.008) 
        ax2.set_ylabel('# of ensembles', fontsize = fontsize_plot, weight = 'bold') # (out of ' + str(nb_ensembles)+ ')'
        ax2.tick_params(axis='both', which='major', labelsize=fontsize_plot, size = 10, width = 2, pad = 7) # set a bunch of parameters for the ticks and their labels
        [i.set_linewidth(2) for i in ax2.spines.itervalues()] # change the width of the frame of the figure
        bin_width = iqr_daily/10.
        ...
        ax2.set_xlim([med - 10*iqr_daily, med+10*iqr_daily])
        ax2.margins(0,0) # autoscale both axes(fist value is for the x axis, second value for the y axis)
        ax2.text(ax2.get_xlim()[0] + ( ax2.get_xlim()[1] - ax2.get_xlim()[0] )/ 60., ax2.get_ylim()[1] - (ax2.get_ylim()[1] - ax2.get_ylim()[0])/5., 'std = ' + '{0:.3e}'.format(std), fontsize = fontsize_plot)
        ax2.set_xlabel('BC (m$^2$/kg)', fontsize = fontsize_plot, weight = 'bold')

        ax2.legend(fontsize = fontsize_plot, loc = 3, scatterpoints=1)
        if save_results == 1:
            fig_save_name = 'cd_distribution_isc' + str(isat)
            fig_save_name = root_save_fig_name + fig_save_name + '.pdf'
            fig.savefig(fig_save_name, facecolor=fig.get_facecolor(), edgecolor='none', bbox_inches='tight')  
            #os.system("rsync -av " + fig_save_name +" srbwks2014-0008.engin.umich.edu:./" + name_mission + '/' + name_subfolder_save)
            os.system("scp -p "  + fig_save_name + " desk:")
...
if "srp" in sys.argv:
    if ( 'srp_ensemble' in pickle_list_name ):
        # Plot the distribution of the Srp
        index_when_plot = 0
        ratio_fig_size = 4./3
        fig = plt.figure(num=None, figsize=(height_fig * ratio_fig_size, height_fig), dpi=80, facecolor='w', edgecolor='k')
        fig.suptitle('', y = 0.975,fontsize = (int)(fontsize_plot*1.1), weight = 'bold',)
        ...
        med = np.median(srp_ensemble[index_when_plot, :])
        mad = np.median( np.abs( srp_ensemble[index_when_plot, :] - np.median(srp_ensemble[index_when_plot, :]) ) )  
        quartile10 = np.percentile(srp_ensemble[index_when_plot, :], 10) 
        quartile25 = np.percentile(srp_ensemble[index_when_plot, :], 25) 
        quartile75 = np.percentile(srp_ensemble[index_when_plot, :], 75) 
        quartile90 = np.percentile(srp_ensemble[index_when_plot, :], 90) 
        std = np.std(srp_ensemble[index_when_plot, :])
        iqr_daily = quartile75 - quartile25
        quartiles_1090_daily = quartile90 - quartile10

        ax2 = fig.add_subplot(gs[0, 0])
        ax2.set_title('SRP coeff. distribution', weight = 'bold', fontsize = fontsize_plot,  y = 1.008)# ' + 'after ' + '{0:.0f}'.format(when_plot_in_hour / 24.) + ' days of propagation', weight = 'bold', fontsize = fontsize_plot,  y = 1.008) 
        ax2.set_ylabel('# of ensembles', fontsize = fontsize_plot, weight = 'bold') # (out of ' + str(nb_ensembles)+ ')'
        ax2.tick_params(axis='both', which='major', labelsize=fontsize_plot, size = 10, width = 2, pad = 7) # set a bunch of parameters for the ticks and their labels
        [i.set_linewidth(2) for i in ax2.spines.itervalues()] # change the width of the frame of the figure
        bin_width = iqr_daily/10.
        hist_data = srp_ensemble[index_when_plot, :]
        n, bins, patches = ax2.hist(hist_data , bins = np.arange(min(hist_data), max(hist_data) + bin_width, bin_width),  histtype='stepfilled', alpha = 0.7, color = 'r',label = '')  
        plt.vlines(quartile25, 0, max(n),linewidth = 2); plt.vlines(quartile75, 0,max(n), linewidth = 2, label = '25-75% quartiles') 
        plt.vlines(quartile10, 0, max(n), linewidth = 4, linestyle = 'dotted'); plt.vlines(quartile90, 0, max(n), linewidth = 4, linestyle = 'dotted', label = '10-90% deciles') 
        ax2.plot(med, 0, 'k',marker = '.', markersize = 20)
        ...
        ax2.margins(0,0) # autoscale both axes(fist value is for the x axis, second value for the y axis)
        ax2.text(ax2.get_xlim()[0] + ( ax2.get_xlim()[1] - ax2.get_xlim()[0] )/ 60., ax2.get_ylim()[1] - (ax2.get_ylim()[1] - ax2.get_ylim()[0])/5., 'std = ' + '{0:.3e}'.format(std), fontsize = fontsize_plot)
        ax2.set_xlabel('SRP coeff. (m$^2$/kg)', fontsize = fontsize_plot, weight = 'bold')

        ax2.legend(fontsize = fontsize_plot, loc = 3, scatterpoints=1)
        if save_results == 1:
            fig_save_name = 'srp_distribution_isc' + str(isat)
            fig_save_name = root_save_fig_name + fig_save_name + '.pdf'
            fig.savefig(fig_save_name, facecolor=fig.get_facecolor(), edgecolor='none', bbox_inches='tight')  
            os.system("scp -p "  + fig_save_name + " desk:")
            #os.system("rsync -av " + fig_save_name +" srbwks2014-0008.engin.umich.edu:./" + name_mission + '/' + name_subfolder_save)
