...
#                     legend.get_title().set_fontsize(str(fontsize_plot))
            ...
            if 'density' in var_to_read:         # !!!!! first date of given output is not necesarrily the same as for the other variables  
                ...
                colorVal = scalarMap.to_rgba(isc_irun)
                y_axis = density[isc]
                ax_density.plot(x_axis, y_axis, linewidth = 2, color = colorVal, label = label_arr[isc] )
                ...
                if isc == nb_sc - 1:
                    ...
                    ax_density.margins(0,0); ax_density.set_xlim([min(xticks), max(xticks)])
            #        ax_density.set_xlim([ax_density.get_xlim()[0], most_recent_tle_among_all_sc])

                    legend = ax_density.legend(loc='center left', bbox_to_anchor=(1, 0.5), numpoints = 1,  title="SC #", fontsize = fontsize_plot)
                    legend.get_title().set_fontsize(str(fontsize_plot))
