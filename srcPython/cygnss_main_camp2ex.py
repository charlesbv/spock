...
os.system('/usr/bin/uuencode ' + filename_out + ' ' + filename_out + ' | /usr/bin/mail -s "me CYGNSS position by SpOCK - Camp2Ex - ' + date_start + ' to ' + date_stop + '" cbv@umich.edu')

#os.system('/usr/bin/uuencode /Users/cbv/cygnss/camp2ex/' + filename_out.replace(".txt", ".tgz") + ' /Users/cbv/cygnss/camp2ex/' + filename_out.replace(".txt", ".tgz") + ' | /usr/bin/mail -s "CYGNSS SP positions by SpOCK - PISTON - ' + datetime.strptime(date_start, "%Y-%m-%dT%H:%M:%S").strftime('%d %B')[:6] + '" timothy.j.landdddg@nasa.gov')
