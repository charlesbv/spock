...
for iday in range(nb_day):
    ...
    if len([x for x in os.listdir("/Users/cbv/cygnss/netcdf/" + str(doy_array[iday]).zfill(3)) if x.endswith(".nc") and x.startswith("cyg0" + str(cygfm))]) > 1: # if more than one file for this sc then don't take this day into account in the analsyis
...
fig_save_name = 'difference_sat_position_spock_vs_netcdf.pdf'
fig.savefig(fig_save_name, facecolor=fig  .get_facecolor(), edgecolor='none', bbox_inches='tight')
...
fig_save_name = 'difference_sat_radius_spock_vs_netcdf.pdf'
fig.savefig(fig_save_name, facecolor=fig  .get_facecolor(), edgecolor='none', bbox_inches='tight')
