...
date_start_val_start = '2018-09-19T00:00:00'#'2018-03-20T00:00:00'#"2017-06-01T00:00:00"
if islin == 1:
...
pitch_max = 100. # filter out when pitch is greater than this value (in magnitude)
roll_max = 100. # filter out when roll is greater than this value (in magnitude)
yaw_max = 100. # filter out when yaw is greater than this value (in magnitude)
...
for idate in range(0,nb_date):# !!!!!!! should be: nb_date):
    ...
    date_stop_val = datetime.strftime(date_start_val_array[idate]+ timedelta(seconds = 9.0*24*3600+23*3600+59*60+59), "%Y-%m-%dT%H:%M:%S") #58*60), "%Y-%m-%dT%H:%M:%S")#0.0*24*3600+23*3600+59*60+59), "%Y-%m-%dT%H:%M:%S") #50*60), "%Y-%m-%dT%H:%M:%S")#0.0*24*3600+23*3600+59*60+59), "%Y-%m-%dT%H:%M:%S") #'2017-06-06T01:02:00'#'2017-06-03T00:45:00'#'2017-06-06T01:02:00'#'2017-06-02T00:56:00'#'2017-06-06T01:02:00'#'2017-06-02T00:43:00'  # !!!! shoul be + timedelta(seconds = 6*24*3600+23*3600+59*60+59), "%Y-%m-%dT%H:%M:%S")#"2017-08-20T06:00:00" # date to stop the validation
...
y_label = 'Percentage (%)'
itime_day = 0 # which day to look at the distirbution over (distibution oof all spec over this day). from 1 (not 0) to nb_day (not nb_day-1). ex: if itime_day = 2 then it'll show the distribution from day 1 to day 2
