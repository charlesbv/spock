...
from orbit_average_with_value_at_equator import *
...
start_epoch = '2017-07-25' # Please use format 'YYYY-MM-DD'
end_epoch = '2017-08-03' # Please use format 'YYYY-MM-DD'
sma_tle_at_tle_epoch_all = []
date_tle_from_tle_all = []
# which norad_id to consider
norad_arr = ['41888']#['41884', '41885', '41886', '41887', '41888', '41889', '41890', '41891']
nb_sc = len(norad_arr)
for i in range(nb_sc):
    norad_id = norad_arr[i]
    if load_tle_result != 1:
        ...
        cygnss_real_mass = 29.
        order_gravity = 20
        forces = "drag sun_gravity moon_gravity solar_pressure"
        ...
        speed_tle_at_tle_epoch = np.zeros([nb_tle])
        ...
        lat_tle_at_tle_epoch = np.zeros([nb_tle])
        ...
        for itle in range(nb_tle):
            ...
            dt_tle = 10 # we don't care about dt here because we only look at the initialization
            ...
            date_end_temp = date_start_temp + timedelta(seconds = 200 * 60.)# 2 * dt_tle)  # !!!! aumm orbit period is < 100 min
            ...
            print 'XXXXXXXXXX',main_input_filename
            ...
            speed_tle_at_tle_epoch[itle] = np.sqrt( v_eci_tle_at_tle_epoch[itle, 0]*v_eci_tle_at_tle_epoch[itle, 0] + v_eci_tle_at_tle_epoch[itle, 1]*v_eci_tle_at_tle_epoch[itle, 1] + v_eci_tle_at_tle_epoch[itle, 2]*v_eci_tle_at_tle_epoch[itle, 2] )
            ...
            lat_tle_at_tle_epoch[itle] = read_tle_out[n_header].split()[9]
            ...
            # Orbit average
            sma_orbit_averaged, time_averaged, index_time_averaged, bla = orbit_average_with_value_at_equator(sma_tle, lat_tle, date_tle )
        ...
        pickle.dump( date_tle_from_tle, open( 'date_tle_from_tle_' + start_epoch.replace("-","_") + '_to_' + end_epoch.replace("-","_") + '_' +  norad_id + ".pickle", "w" ) )
        pickle.dump( sma_tle_at_tle_epoch, open( 'sma_tle_at_tle_epoch_' + start_epoch.replace("-","_") + '_to_' + end_epoch.replace("-","_") + '_' + norad_id + ".pickle", "w" ) )

    else:
        date_tle_from_tle = pickle.load(open( 'date_tle_from_tle_' + start_epoch.replace("-","_") + '_to_' + end_epoch.replace("-","_") + '_' +  norad_id + ".pickle","r")) # firrst time I saved pickle was without the date ->  pickle.load(open( 'date_tle_from_tle_' + norad_id + ".pickle","r"))
        sma_tle_at_tle_epoch = pickle.load(open( 'sma_tle_at_tle_epoch_' + start_epoch.replace("-","_") + '_to_' + end_epoch.replace("-","_") + '_' + norad_id + ".pickle","r"))
        date_tle_from_tle_all.append(date_tle_from_tle)
        sma_tle_at_tle_epoch_all.append(sma_tle_at_tle_epoch)
...
if spock_prop == 1:
    # # SPOCK PROPAGATION BUT ONLY FOR ONE CYGNSS
    norad_id = '41888'
    ...
    cd_array =  np.array([2.4])    #np.arange(2.6,4.4,0.8) np.array([2.4])
    ...
    sma_average_spock = []
    sma_perigee_apogee = []
    sma_temporal_average_spock = []
    x_axis_sma_temporal_average_spock = []
    sma_at_equator_spock = []
    radius_at_equator_spock = []
    speed_at_equator_spock = []
    ...
    for irun in range(nb_run_spock):
        ...
        spock_main_input(
            ...
            # for ATTITUDE section
            'att_high_drag_lower.txt',#"(82;0; 0) (0;0;0)",
            # for GROUNDS_STATIONS section
            "0",#"my_ground_stations.txt"
            # for SPICE section
            spice_path, 
    ...
    #    raise Exception
        ...
        isc = 0 # here only one sc in main input file
        var_to_read = ["altitude", "sma", "radius", "eccentricity","latitude", "position", "velocity", "speed"]
        var_out, var_out_order = read_output_file( output_file_path_list[isc] + output_file_name_list[isc], var_to_read )
        if irun  == 0:
            ...
            nb_sc = var_in[find_in_read_input_order_variables(var_in_order, 'nb_sc')]; 
            r_spock = np.zeros([nb_run_spock, nb_steps_new,3]); v_spock = np.zeros([nb_run_spock, nb_steps_new,3])
            alt_spock = np.zeros([nb_run_spock, nb_steps_new]); sma_spock = np.zeros([nb_run_spock, nb_steps_new]); speed_spock = np.zeros([nb_run_spock, nb_steps_new]); radius_spock = np.zeros([nb_run_spock, nb_steps_new]); ecc_spock = np.zeros([nb_run_spock, nb_steps_new]); lat_spock = np.zeros([nb_run_spock, nb_steps_new])
        r_spock[irun,:,:] = var_out[find_in_read_input_order_variables(var_out_order, 'position')] 
        v_spock[irun,:,:] = var_out[find_in_read_input_order_variables(var_out_order, 'velocity')] 
...
#        nb_steps_orbit_temporal = (int)  (95. * 60./ dt_output_spock)
#         ii = 0
#         while (ii+1)* nb_steps_orbit_temporal < nb_steps_new:
#             sma_temporal_average_spock.append( np.mean( sma_spock[irun, ii*nb_steps_orbit_temporal:(ii+1) * nb_steps_orbit_temporal] ) )
#             x_axis_sma_temporal_average_spock.append( (datetime.strptime(date_spock[ii*nb_steps_orbit_temporal],"%Y/%m/%d %H:%M:%S.%f" ) - oldest_tle).total_seconds() )
#             ii = ii + 1
        ...
        speed_spock[irun, :] = var_out[find_in_read_input_order_variables(var_out_order, 'speed')]
        ecc_spock[irun, :] = var_out[find_in_read_input_order_variables(var_out_order, 'eccentricity')]
        # Orbit average
        sma_orbit_averaged, time_averaged, index_time_averaged, sma_at_equator_temp = orbit_average_with_value_at_equator(sma_spock[irun, :nb_steps_new], lat_spock[isc, :nb_steps_new], date_spock )
        blaa, time_averaged, index_time_averaged, radius_at_equator_temp = orbit_average_with_value_at_equator(radius_spock[irun, :nb_steps_new], lat_spock[isc, :nb_steps_new], date_spock )
        blaaa, time_averaged, index_time_averaged, speed_at_equator_temp = orbit_average_with_value_at_equator(speed_spock[irun, :nb_steps_new], lat_spock[isc, :nb_steps_new], date_spock )
        ...
        sma_at_equator_spock.append(sma_at_equator_temp)
        radius_at_equator_spock.append(radius_at_equator_temp)
        speed_at_equator_spock.append(speed_at_equator_temp)



        nb_orbit_spock[irun] = len(time_averaged)
        for iorbit in range(nb_orbit_spock[irun]):
            radius_perigee = np.min(radius_spock[irun, index_time_averaged[iorbit][0]:index_time_averaged[iorbit][2]])
            radius_apogee = np.max(radius_spock[irun, index_time_averaged[iorbit][0]:index_time_averaged[iorbit][2]])
            sma_perigee_apogee.append(  radius_apogee )#+ radius_perigee ) / 2. )
            ...
            nb_seconds_between_start_orbit_and_date_start = ( date_average_spock_start_orbit_run[-1] - oldest_tle ).total_seconds() # with respect to first tle date
    ...
    sma_at_equator_spock = np.array(sma_at_equator_spock)
    radius_at_equator_spock = np.array(radius_at_equator_spock)
    speed_at_equator_spock = np.array(speed_at_equator_spock)
...
sma_at_equator_spock2 = np.zeros([nb_run_spock, nb_orbit_spock])
for iorbit in range(nb_orbit_spock):
    sma_at_equator_spock2[0, iorbit] = 1.0 / ( (2.0/radius_at_equator_spock[irun, iorbit] ) - ( (speed_at_equator_spock[irun, iorbit]*speed_at_equator_spock[irun, iorbit])/earth_mu ) )
...
nb_seconds_between_step_and_date_start_ref = np.zeros([nb_steps_new])
for idt in range(nb_steps_new):
    nb_seconds_between_step_and_date_start_ref[idt] = ( datetime.strptime(date_spock[idt], "%Y/%m/%d %H:%M:%S.%f") - date_start_ref ).total_seconds()
...
for irun in range(nb_run_spock):
    ...
    x_spock_per_cd_run_interpo = []
    y_spock_per_cd_run_interpo = []
    z_spock_per_cd_run_interpo = []
    vx_spock_per_cd_run_interpo = []
    vy_spock_per_cd_run_interpo = []
    vz_spock_per_cd_run_interpo = []
    r_spock_per_cd_run_interpo = []
    v_spock_per_cd_run_interpo = []
    ...
    for itle in range(nb_tle):
        if len( np.where(nb_seconds_between_step_and_date_start_ref <= x_axis_tle[itle])[0] ) > 0: # get the SpOCK step right before the TLE
            ...
            x_spock_per_cd_run_at_ti = r_spock[irun, ti_index, 0]
            y_spock_per_cd_run_at_ti = r_spock[irun, ti_index, 1]
            z_spock_per_cd_run_at_ti = r_spock[irun, ti_index, 2]
            vx_spock_per_cd_run_at_ti = v_spock[irun, ti_index, 0]
            vy_spock_per_cd_run_at_ti = v_spock[irun, ti_index, 1]
            vz_spock_per_cd_run_at_ti = v_spock[irun, ti_index, 2]
            ...
            if ti_index + 1 < nb_steps_new:
                tf = nb_seconds_between_step_and_date_start_ref[ti_index+1]
                alt_spock_per_cd_run_at_tf = alt_spock[irun, ti_index+1]
                x_spock_per_cd_run_at_tf = r_spock[irun, ti_index+1, 0]
                y_spock_per_cd_run_at_tf = r_spock[irun, ti_index+1, 1]
                z_spock_per_cd_run_at_tf = r_spock[irun, ti_index+1, 2]
                vx_spock_per_cd_run_at_tf = v_spock[irun, ti_index+1, 0]
                vy_spock_per_cd_run_at_tf = v_spock[irun, ti_index+1, 1]
                vz_spock_per_cd_run_at_tf = v_spock[irun, ti_index+1, 2]
                ...
                x_slope = (x_spock_per_cd_run_at_tf - x_spock_per_cd_run_at_ti)/ ( tf - ti )
                y_slope = (y_spock_per_cd_run_at_tf - y_spock_per_cd_run_at_ti)/ ( tf - ti )
                z_slope = (z_spock_per_cd_run_at_tf - z_spock_per_cd_run_at_ti)/ ( tf - ti )
                vx_slope = (vx_spock_per_cd_run_at_tf - vx_spock_per_cd_run_at_ti)/ ( tf - ti )
                vy_slope = (vy_spock_per_cd_run_at_tf - vy_spock_per_cd_run_at_ti)/ ( tf - ti )
                vz_slope = (vz_spock_per_cd_run_at_tf - vz_spock_per_cd_run_at_ti)/ ( tf - ti )
                ...
                x_spock_per_cd_run_interpo.append( x_slope * ( x_axis_tle[itle] - ti ) + x_spock_per_cd_run_at_ti )
                y_spock_per_cd_run_interpo.append( y_slope * ( x_axis_tle[itle] - ti ) + y_spock_per_cd_run_at_ti )
                z_spock_per_cd_run_interpo.append( z_slope * ( x_axis_tle[itle] - ti ) + z_spock_per_cd_run_at_ti )
                vx_spock_per_cd_run_interpo.append( vx_slope * ( x_axis_tle[itle] - ti ) + vx_spock_per_cd_run_at_ti )
                vy_spock_per_cd_run_interpo.append( vy_slope * ( x_axis_tle[itle] - ti ) + vy_spock_per_cd_run_at_ti )
                vz_spock_per_cd_run_interpo.append( vz_slope * ( x_axis_tle[itle] - ti ) + vz_spock_per_cd_run_at_ti )
                r_spock_per_cd_run_interpo.append([x_spock_per_cd_run_interpo[-1], y_spock_per_cd_run_interpo[-1], z_spock_per_cd_run_interpo[-1] ])
                v_spock_per_cd_run_interpo.append([vx_spock_per_cd_run_interpo[-1], vy_spock_per_cd_run_interpo[-1], vz_spock_per_cd_run_interpo[-1] ])
    ...
    x_spock_interpo.append(x_spock_per_cd_run_interpo)
    y_spock_interpo.append(y_spock_per_cd_run_interpo)
    z_spock_interpo.append(z_spock_per_cd_run_interpo)
    vx_spock_interpo.append(vx_spock_per_cd_run_interpo)
    vy_spock_interpo.append(vy_spock_per_cd_run_interpo)
    vz_spock_interpo.append(vz_spock_per_cd_run_interpo)

    r_spock_interpo.append( r_spock_per_cd_run_interpo )
    v_spock_interpo.append( v_spock_per_cd_run_interpo )
...
x_spock_interpo = np.array(x_spock_interpo)
y_spock_interpo = np.array(y_spock_interpo)
z_spock_interpo = np.array(z_spock_interpo)
vx_spock_interpo = np.array(vx_spock_interpo)
vy_spock_interpo = np.array(vy_spock_interpo)
vz_spock_interpo = np.array(vz_spock_interpo)

r_spock_interpo = np.array(r_spock_interpo)
v_spock_interpo = np.array(v_spock_interpo)
...
# v_mag( &rrss, r);
# v_mag( &vrss, v);
rmag_spock_interpo = np.zeros([nb_run_spock, nb_tle-1])
vmag_spock_interpo = np.zeros([nb_run_spock, nb_tle-1])
sma_spock_interpo2 = np.zeros([nb_run_spock, nb_tle-1])
for irun in range(nb_run_spock):
    for itle in range(nb_tle-1):
        rmag_spock_interpo[irun, itle] = np.linalg.norm( r_spock_interpo[irun, itle ,:] )
        vmag_spock_interpo[irun, itle] = np.linalg.norm( v_spock_interpo[irun, itle ,:] )
        sma_spock_interpo2[irun, itle] = 1.0 / ( (2.0/rmag_spock_interpo[irun, itle] ) - ( (vmag_spock_interpo[irun, itle]*vmag_spock_interpo[irun, itle])/earth_mu ) )
        

#Read file of maneuverse  (high drag and Sun pointed)
filename_man = 'maneuvers.txt'
...
sunpointed = []
sc_name = []
nb_highdrag = np.zeros([8]).astype(np.int)
nb_sunpointed = np.zeros([8]).astype(np.int)
...
for isc in range(8):
    ...
    while read_file_man[iline].rstrip() != 'sunpointed':  # reached sunpointd is this is false. otherwise fill highdrag dates
        ...
        man_start = datetime.strptime(man_start, '%d%m%y') #280717
        man_end = datetime.strptime(man_end, '%d%m%y') #280717
...
fig_title = 'SMA from TLE and from SpOCK as a function of time'
y_label = 'SMA (km)'
x_label = 'Real time'
...
if spock_prop == 1:
    for irun in range(nb_run_spock):
        ...
        #ax_sma_average.plot(x_axis_average_spock[irun, :], sma_average_spock[irun, :], linewidth = 2, color = 'b')
        #ax_sma_average.plot(x_axis_average_spock[irun, :], sma_perigee_apogee, linewidth = 2, color = 'b')
        #ax_sma_average.plot(x_axis_average_spock[irun, :], sma_at_equator_spock[irun, :], linewidth = 2, color = 'b', label = 'SpOCK')
        ax_sma_average.plot(x_axis_average_spock[irun, :], sma_at_equator_spock2[irun, :], linewidth = 2, color = 'b', label = 'SpOCK')
        #ax_sma_average.plot(x_axis_average_spock[irun, :], radius_at_equator_spock[irun, :], linewidth = 2, color = 'b', label = 'SpOCK')
        #ax_sma_average.plot(x_axis_average_spock[irun, :], speed_at_equator_spock[irun, :], linewidth = 2, color = 'b', label = 'SpOCK')
#         ax_sma_average.plot(nb_seconds_between_step_and_date_start_ref, sma_spock[irun,:] )
        #ax_sma_average.plot(nb_seconds_between_step_and_date_start_ref, radius_spock[irun,:] )
        #ax_sma_average.plot(x_axis_average_spock[irun, 14::15], sma_at_equator_spock[irun, 14::15], linewidth = 2, color = 'r', label = 'SpOCK')
        #ax_sma_average.plot(x_axis_sma_temporal_average_spock, sma_temporal_average_spock, linewidth = 2, color = 'r', label = 'SpOCK')
        ax_sma_average.plot(date_spock_interpo, sma_spock_interpo[irun, :], linewidth = 2, color = 'k', label = 'SpOCK')
        #ax_sma_average.plot(date_spock_interpo, sma_spock_interpo2[irun, :], linewidth = 2, color = 'b', label = 'SpOCK')

max_sma_tle = 0
min_sma_tle = 100000000
for isc in range(nb_sc):
    norad_id = norad_arr[isc] # has to be a string
    fm_id = norad_id_to_cygnss_name(norad_id)
    index_fm = np.where(np.array(sc_name) == fm_id)[0][0]
    #ax_sma_average.plot(x_axis_average_tle[:,0], sma_average_tle[:,0], linewidth = 2, color = 'r', label =  'TLE')
    ax_sma_average.plot(nb_seconds_oldest_tle_to_this_tle[isc], sma_tle_at_tle_epoch_all[isc], linewidth = 2, color = 'r', label =  'TLE')#fm_id + ' (' + str(nb_highdrag[index_fm]) + ')')
    ax_sma_average.scatter(nb_seconds_oldest_tle_to_this_tle[isc], sma_tle_at_tle_epoch_all[isc], linewidth = 2, color = color_arr[index_fm])#fm_id + ' (' + str(nb_highdrag[index_fm]) + ')')
#     ax_sma_average.plot(nb_seconds_oldest_tle_to_this_tle[isc], radius_tle_at_tle_epoch, linewidth = 2, color = 'r', label =  'TLE')#fm_id + ' (' + str(nb_highdrag[index_fm]) + ')')
#     ax_sma_average.scatter(nb_seconds_oldest_tle_to_this_tle[isc], radius_tle_at_tle_epoch, linewidth = 2, color = color_arr[index_fm])#fm_id + ' (' + str(nb_highdrag[index_fm]) + ')')
#     ax_sma_average.plot(nb_seconds_oldest_tle_to_this_tle[isc], speed_tle_at_tle_epoch, linewidth = 2, color = 'r', label =  'TLE')#fm_id + ' (' + str(nb_highdrag[index_fm]) + ')')
#     ax_sma_average.scatter(nb_seconds_oldest_tle_to_this_tle[isc], speed_tle_at_tle_epoch, linewidth = 2, color = color_arr[index_fm])#fm_id + ' (' + str(nb_highdrag[index_fm]) + ')')


#     nb_tle_now = len(date_tle_from_tle_all[isc])
#     for iman in range(nb_highdrag[index_fm]):
#         if len(np.where(np.array(nb_seconds_oldest_tle_to_this_tle[isc]) > nb_seconds_first_tle_to_start_highdrag[index_fm][iman])[0]) > 0: # = 0 if no TLE is more recent than this maneuvre
...
#             ax_sma_average.scatter(nb_seconds_oldest_tle_to_this_tle[isc][index_tle_start_man], sma_tle_at_tle_epoch_all[isc][index_tle_start_man],marker = '.', color = color_arr[index_fm],s = 300)
...
#                 ax_sma_average.scatter(nb_seconds_oldest_tle_to_this_tle[isc][index_tle_end_man], sma_tle_at_tle_epoch_all[isc][index_tle_end_man],marker = 's', color = color_arr[index_fm], s = 70)

            
#     for iman in range(nb_sunpointed[index_fm]):
#         if len(np.where(np.array(nb_seconds_oldest_tle_to_this_tle[isc]) > nb_seconds_first_tle_to_start_sunpointed[index_fm][iman])[0]) > 0: # = 0 if no TLE is more recent than this maneuvre
...
#             ax_sma_average.scatter(nb_seconds_oldest_tle_to_this_tle[isc][index_tle_start_man], sma_tle_at_tle_epoch_all[isc][index_tle_start_man],marker = '+', color = color_arr[index_fm],s = 200)
...
#                 ax_sma_average.scatter(nb_seconds_oldest_tle_to_this_tle[isc][index_tle_end_man], sma_tle_at_tle_epoch_all[isc][index_tle_end_man],marker = '+', color = color_arr[index_fm], s = 200)
...
legend = ax_sma_average.legend(loc='center left', bbox_to_anchor=(1, 0.5), numpoints = 1,  title="CYG (#HD)", fontsize = fontsize_plot)
legend.get_title().set_fontsize(str(fontsize_plot))


fig_save_name = 'history_tle_sma_and_high_drag_maneuvers_' + start_epoch.replace("-", "_") + "_to_" + end_epoch.replace("-", "_")
...
#         #ax_sma_average.plot(x_axis_average_spock[irun, :], sma_average_spock[irun, :], linewidth = 2, color = 'b')
# #         ax_sma_average.plot(date_spock_interpo, sma_spock_interpo[irun, :], linewidth = 2, color = 'k', label = 'SpOCK')
# #         ax_sma_average.plot(date_spock_interpo, x_spock_interpo[irun, :], linewidth = 2, color = 'b', label = 'SpOCK')
#         #ax_sma_average.plot(date_spock_interpo, rmag_spock_interpo[irun, :], linewidth = 2, color = 'b', label = 'SpOCK')
#         #ax_sma_average.plot(date_spock_interpo, vmag_spock_interpo[irun, :], linewidth = 2, color = 'b', label = 'SpOCK')
#         ax_sma_average.plot(nb_seconds_between_step_and_date_start_ref, v_spock[irun,:,2] )
#         ax_sma_average.scatter(date_spock_interpo, vz_spock_interpo[irun, :], marker = '.', s = 100)  
...
#     index_fm = np.where(np.array(sc_name) == fm_id)[0][0]
# #     ax_sma_average.plot(nb_seconds_oldest_tle_to_this_tle[isc], sma_tle_at_tle_epoch_all[isc], linewidth = 2, color = 'r', label =  'TLE')#fm_id + ' (' + str(nb_highdrag[index_fm]) + ')')
# #     ax_sma_average.plot(nb_seconds_oldest_tle_to_this_tle[isc], r_eci_tle_at_tle_epoch[:, 0], linewidth = 2, color = 'r', label =  'TLE')
# #     ax_sma_average.plot(nb_seconds_oldest_tle_to_this_tle[isc], r_eci_tle_at_tle_epoch[:, 1], linewidth = 2, color = 'r', label =  'TLE')
# #     ax_sma_average.plot(nb_seconds_oldest_tle_to_this_tle[isc], r_eci_tle_at_tle_epoch[:, 2], linewidth = 2, color = 'r', label =  'TLE')
# #     ax_sma_average.plot(nb_seconds_oldest_tle_to_this_tle[isc], v_eci_tle_at_tle_epoch[:, 0], linewidth = 2, color = 'r', label =  'TLE')
# #     ax_sma_average.plot(nb_seconds_oldest_tle_to_this_tle[isc], v_eci_tle_at_tle_epoch[:, 1], linewidth = 2, color = 'r', label =  'TLE')
# #     ax_sma_average.plot(nb_seconds_oldest_tle_to_this_tle[isc], v_eci_tle_at_tle_epoch[:, 2], linewidth = 2, color = 'r', label =  'TLE')
# #    ax_sma_average.plot(nb_seconds_oldest_tle_to_this_tle[isc], radius_tle_at_tle_epoch, linewidth = 2, color = 'r', label =  'TLE')
# #    ax_sma_average.plot(nb_seconds_oldest_tle_to_this_tle[isc], speed_tle_at_tle_epoch, linewidth = 2, color = 'r', label =  'TLE')
