...
pickle_folder = "/Users/cbv/cygnss/coverage_temporal_spatial_relation/sep18/pickle" # with or without the last '/' (doesnt matter). also the directory doesnt need to exist 
...
width_cell_array = [25] # in km
coverage_array = [95] # in percentage in ASCENDING ORDER
...
for cygfm in range(1, 9):
    ...
    date_spock = data_spec[0]; lon_spec_sc = data_spec[1]; lat_spec_sc = data_spec[2]; gain_spec_sc = data_spec[3]; gps_name_sc = data_spec[4]; 
    lon_spec_list.append(lon_spec_sc); lat_spec_list.append(lat_spec_sc)
print 'Done reading the SP positions.'
lon_spec_temp = np.array(lon_spec_list)
lat_spec_temp = np.array(lat_spec_list)
lon_spec = np.moveaxis(lon_spec_temp,[0,1,2],[1,2,0]) # reoganization of shape
...
#if load != 1:
nb_time = lon_spec.shape[2] # !!!!!!!!! remove "/ 4"
nb_sc = lon_spec.shape[1]
nb_spec = lon_spec.shape[0]
