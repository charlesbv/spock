...
# arguments:
# 1: SpOCK run dir
# 2: main input file name of SpOCK (without path)
...
min_lon_array = [-180, min_lon - range_lon / 10]# 152.08] #0
min_lat_array = [-90, min_lat -  range_lat / 10]#21.17] #-10
max_lon_array = [180, max_lon + range_lon / 10]#153.35] #60
max_lat_array = [90, max_lat + range_lat / 10]#22.32]#40
step_lon = [60,range_lon / 10]#10
step_lat = [30, range_lat / 10]#10
...
#        x, y =  m(lon[isat, index_tca_in_propagation: index_tca_in_propagation+2] , lat[isat,index_tca_in_propagation: index_tca_in_propagation+2])
        ...
        if isat == 0:
            m.plot(x, y,  marker='s', markersize=10,color = colorVal,markeredgecolor = 'none', label = run_list[irun].split('f107_')[1].split('_ap')[0].replace("_quartile", "") + "0%"  )[0]
        else:
            m.plot(x, y,  marker='o', markersize=10,color = colorVal,markeredgecolor = 'none'  )[0]
...
if save_results == 1:
    fig_save_name = 'position_sc_before_collision'
