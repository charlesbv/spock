...
list_elements_can_be_plot_or_save = ['radius', 'speed', 'altitude', 'latitude', 'eccentricity', 'sma', 'sma_average', 'radius_perigee', 'radius_apogee', 'argument_perigee', 'raan', 'beta', 'given_output', 'power', 'density', 'density_average', 'temperature', 'cd', 'tot_area_drag', 'position','velocity','acceleration_lvlh_drag_mag', 'phase_angle'] # !!!!! first date of given output is not necesarrily the same as for the other variables  
...
######## PLOT
        ...
        # Radius
            if 'radius' in var_to_read:        
                ...
                x_axis_temp = phase_angle[isc,:nb_steps_new] # !!!!!!!! remove line
                ax_radius.scatter(x_axis_temp, y_axis, linewidth = 2, color = colorVal, label = label_arr[isc]) # !!!!! should be x_axis
...
#             #        ax_radius.set_xlim([ax_radius.get_xlim()[0], most_recent_tle_among_all_sc])
            ...
            if 'sma' in var_to_read:        
                ...
                colorVal = scalarMap.to_rgba(isc_irun)
                y_axis = sma[isc,:nb_steps_new] 
                if isc == 0:
                    min_y = np.min(y_axis)
                    max_y = np.max(y_axis)
                ax_sma.plot(x_axis, y_axis, linewidth = 2, color = colorVal, label = label_arr[isc])
                ax_sma.plot(x_axis, sma_ave[isc,:nb_steps_new], linewidth = 2, color = colorVal, linestyle = 'dashed')
...
#                     legend.get_title().set_fontsize(str(fontsize_plot))
            ...
            if 'density' in var_to_read:         # !!!!! first date of given output is not necesarrily the same as for the other variables  
                if ( isc == 0 ) & (irun == 0) :
                    ...
                    x_label = 'Phase angle ' + u'(\N{DEGREE SIGN})'#Real time'
                ...
                for iorb in range(nb_orb):
                    ...
                    x_axis_density = phase_angle[isc, iorb_start:iorb_end]#argument_perigee[isc,:nb_steps_new] #latitude[isc,:nb_steps_new]
#                     if iorb == 0:
...
#                     else:
                    if iorb >= 0: # ignore first orbit because for density control the desnity is equal to msis for first orbit -> not interested in plotting for the current analysis
                        if iorb == 1:
                            ...
                            ax_density.scatter(x_axis_density, y_axis, s = 5, color = colorVal)
                    if iorb >= 1:
