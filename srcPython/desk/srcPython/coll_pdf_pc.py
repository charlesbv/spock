...
#hist_pdf = ax.hist(pc_sort[:, 0])[0] / nb_run * 100
ax.plot(bin_cdf_array, cumulative_density_prob/cumulative_density_prob[-1]*100, linewidth = 3) # /cumulative_density_prob[-1] to norm because the max should be 1. it's actually close to 1 but not exactly 1 because of reasons that i dont remember (it's due to the probability of f107 and ap)
ax.margins(0,0)
ax.set_ylim([0, 105])
# pc[irun,:]:  0 pc, 1 probability of (f107,ap) to occur, 2 probablity of f107 to occur, 3 f107, 4 probablity of ap to occur, 5 ap

fig_save_name = 'pdf_and_cdf_pc'  
