...
#sys.path.append("/home1/cbv/Code/spock/srcPython")
...
for i in range(len(ensemble_to_plot_temp)):
    ...
    if (ensemble_to_plot_temp[i] == 'cd'):
        ensemble_to_plot_from_main_input_file.append('cd');
    if (ensemble_to_plot_temp[i] == 'srp'):
        ensemble_to_plot_from_main_input_file.append('srp');


ensemble_to_plot = []
skip_arg = 2

#n_main = 3 # !!!!!!!!!!!! remove this line. it was put on july 13 2018 because i was looking at collision (dt before, at, after TCA)
...
#                print range(0,n,dt_read_output_in_time_step)
                for i in range(0,n,dt_read_output_in_time_step):
                    if rank == 0:
                        print("%d" % ( i/float(n) * 100 ) + " %")            
                        sys.stdout.write("\033[F") # Cursor up one line
                    date_ensemble.append( datetime.strptime(read_file_x[i + nb_lines_header].split()[0] + ' ' + read_file_x[i + nb_lines_header].split()[1], "%Y/%m/%d %H:%M:%S") )
                    for j in range(nb_ensembles):
                ...
                date_ensemble = np.array(date_ensemble)
        if rank == rank_y_eci:
        ...
        if rank == rank_cd:
            if ( 'cd' in ensemble_to_plot ): 
                ...
                print "Read the ensembles on cd by iProc", rank
                file_x = open(dir_final_output_ensemble+"ensemble_cd_"+satellite_to_plot[isat], "r")
                read_file_x = file_x.readlines()
                nb_lines_header = 11
                n_cd =1 #onl one time step for cd len(read_file_x) - nb_lines_header # !!!!!!!!!!!this has been added between when computing collisions the time of output stop at end of time spanning TCA

                cd_ensemble = np.zeros([n_cd, nb_ensembles])

                #date_ensembl
#                print range(0,n,dt_read_output_in_time_step)
                i  = 0 # only one time step for cd
                if rank == 0:
                    print("%d" % ( i/float(n_cd) * 100 ) + " %")            
                    sys.stdout.write("\033[F") # Cursor up one line

                for j in range(nb_ensembles):
                    cd_ensemble[i,j] = float( read_file_x[i + nb_lines_header].split()[j ] )
                file_x.close()
                pickle_list.append(cd_ensemble); pickle_list_name.append('cd_ensemble')
                save_pickle_name = save_pickle_name + 'cd_'
        if rank == rank_srp:
            if ( 'srp' in ensemble_to_plot ): 
                
                # Read the ensembles on srp
                print "Read the ensembles on srp by iProc", rank
                file_x = open(dir_final_output_ensemble+"ensemble_srp_"+satellite_to_plot[isat], "r")
                read_file_x = file_x.readlines()
                nb_lines_header = 11
                n_srp =1 #onl one time step for srp len(read_file_x) - nb_lines_header # !!!!!!!!!!!this has been added between when computing collisions the time of output stop at end of time spanning TCA

                srp_ensemble = np.zeros([n_srp, nb_ensembles])

                #date_ensembl
#                print range(0,n_srp,dt_read_output_in_time_step)
                i  = 0 # only one time step for srp
                if rank == 0:
                    print("%d" % ( i/float(n_srp) * 100 ) + " %")            
                    sys.stdout.write("\033[F") # Cursor up one line

                for j in range(nb_ensembles):
                    srp_ensemble[i,j] = float( read_file_x[i + nb_lines_header].split()[j ] )
                file_x.close()
                pickle_list.append(srp_ensemble); pickle_list_name.append('srp_ensemble')
                save_pickle_name = save_pickle_name + 'srp_'
...
#                        min_nb_period = min([])
        ...
        if ( ( 'x_eci' in ensemble_to_plot ) & ( 'y_eci' in ensemble_to_plot ) & ( 'z_eci' in ensemble_to_plot ) ): 
            if rank == rank_x_eci:
                ...
                nb_lines_header = 10
                n_main = len(read_file_main_sc) - nb_lines_header
                ...
                for i in range(0,n_main,dt_read_output_in_time_step):#for i in range(0,np.min([n, n_main]),dt_read_output_in_time_step):
                ...
                for i in range(0,np.min([n, n_main]),dt_read_output_in_time_step):
                    print("%d" % ( i/float(n) * 100 ) + "%")
                    sys.stdout.write("\033[F") # Cursor up one line
                    if len(np.where(date_main_datetime_round_sec == date_ensemble[i])[0]) != 1:
                        print "***! Time steps in ensemble SC output files are not in main SC output files so the LVLH distributions can't be computed.  If computing the probability of collision, make sure that the time step of the output (second line of section #OUTPUT) is the same as the time step of the integration (3rd line of section #TIME). The program will stop. !***"; raise Exception

                    i_main = np.where(date_main_datetime_round_sec == date_ensemble[i])[0][0]

                    for j in range(nb_ensembles):
                        distance_ensemble_main_sc[i,j] = np.sqrt( (x_eci_ensemble[i,j] - x_eci_main_sc[i_main])**2 + (y_eci_ensemble[i,j] - y_eci_main_sc[i_main])**2 +  (z_eci_ensemble[i,j] - z_eci_main_sc[i_main])**2 )#*1000 
                        # * 1000 to convert the distance from km to m
                        
                        position_main_sc_eci = np.array([x_eci_main_sc[i_main], y_eci_main_sc[i_main], z_eci_main_sc[i_main]])
                        ...
                        velocity_main_sc_eci = np.array([vx_eci_main_sc[i_main], vy_eci_main_sc[i_main], vz_eci_main_sc[i_main]])
