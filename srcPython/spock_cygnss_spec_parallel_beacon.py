...
if predict_spec == 1:
    ...
    gps_tle = '0'
mass = 29
geometry_filename = "/Users/cbv/cygnss_geometry_2016_acco09.txt"

# for ORBIT section
tle_filename = 'cygnss_' + start_date_no_time + '.txt'
# for FORCES section
gravity_order = 20 #!!!!!!!!change back to 4
forces = 'drag solar_pressure sun_gravity moon_gravity'
...
print path_mpirun + " -np 4 spock_grav_read_bin_earth_map " + main_input_filename 
os.system(path_mpirun + " -np 4 spock_grav_read_bin_earth_map " + main_input_filename ) # can't redirect output at all (on windows it said "can't access file, busy with another process" or something like that and it made it crash. So either accept that the ouput is printed to the temrinal, or don't print anything with spock, spec and storm
