...
if ( len(sys.argv) > 2 ): # if the argument 'latest_tle' has been called than do not consider tle_epoch to download the TLE but download the latest TLE from space-track.org. The name of the TLE will still be "gps_" + tle_epoch + ".txt"
    if ( sys.argv[2] == 'latest_tle' ):
        ...
        os.system('wget  --post-data="identity=cbv@umich.edu&password=cygnssisawesome" --cookies=on --keep-session-cookies --save-cookies=cookies.txt "https://www.space-track.org/ajaxauth/login" -olog')# !!! can't redirect in windows version > /dev/null 2>&1")
        name_tle = "gps_" + tle_epoch  + ".txt"
        os.system('wget --limit-rate=100K --keep-session-cookies --load-cookies=cookies.txt ' + link_spacetrack + ' -O ' + name_tle)#!!! can't redirect in windows version + " > /dev/null 2>&1")
