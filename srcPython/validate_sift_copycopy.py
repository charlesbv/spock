...
date_stop_val = "2017-08-22T06:00:00" # date to stop the validation
...
while iday_count < nb_day:
    ...
    itime = -1
    date_flight_raw = []
    while itime < nb_time_flight_temp:
        itime = itime + 1
...
for itime in range(nb_time_spock_netcdf):
    ...
    r_cyg_spock = np.array([x_cyg_spock_same_time_as_netcdf[itime][0], y_cyg_spock_same_time_as_netcdf[itime][0], z_cyg_spock_same_time_as_netcdf[itime][0]])
    r_cyg_netcdf = np.array([x_cyg_netcdf[itime], y_cyg_netcdf[itime], z_cyg_netcdf[itime]])
    r_cyg_diff = r_cyg_spock - r_cyg_netcdf
...
fig_title = 'Difference in position SpOCK vs L1' # L1 is netcdf
y_label = 'Difference (km)'
x_label = 'Prediction time (hours)'
...
x_axis = nb_seconds_since_initial_epoch_spock / 3600.
...
fig_save_name = 'difference_spec_position_spock_vs_netcdf.pdf'
