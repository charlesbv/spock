...
# Copy of gps_tle.py on 02/14/2019. only differeence is the format of the first line of each tle, so the tle file can be handled by tds-bop, the program used in the beacon campaign
...
norad_id_and_prn_operational_gps = []
for itle in range(nb_tle):
    ...
    norad_id_operational_gps.append(norad_id)
    norad_id_and_prn_operational_gps.append([norad_id, prn])
tle_celestrak.close()

# rm file form celestrak
os.system("rm gps-ops.txt")

# download TLEs of these GPS from space-track for the epoch tle_epoch 
## put NORAD ID in link
if ( len(sys.argv) > 2 ): # if the argument 'latest_tle' has been called than do not consider tle_epoch to download the TLE but download the latest TLE from space-track.org. The name of the TLE will still be "gps_" + tle_epoch + ".txt"
...
# Nor add the PRN of each GPS on the first line (before the name of the GPS)
...
norad_id_and_prn_operational_gps_arr = np.array(norad_id_and_prn_operational_gps)
for itle in range( nb_tle):

    norad_id = read_new_tle_spacetrack_without_prn[itle*3+2].split()[1]
    where_prn = np.where(norad_id_and_prn_operational_gps_arr == norad_id)[0][0]
    prn = norad_id_and_prn_operational_gps_arr[where_prn, 1]
    print >> new_tle_spacetrack_with_prn, ' '.join(read_new_tle_spacetrack_without_prn[itle*3].split()[:2]) + ' (PRN ' + prn + ')'
