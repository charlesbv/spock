...
run_list = ["1_inc_75", "1_inc_80", "1_inc_85", "1_inc_90"] #  list of cygnss_coverage_temporal_spatial_relation.py runs to compare. !!!!! need to run cygnss_coverage_temporal_spatial_relation_equal_cell_area2.py for each of these runs first
...
max_lat_grid_array =    [60,65,70,75,80,85]      # !!!!!! needs to be the same as in cygnss_coverage_temporal_spatial_relation.py
...
root_fig = "cygnss_extended"
# for cyggnss extended: # for cygnss_extended
#for inclinatino at 90 deg:: 90
pickle_folder = "/Users/cbv/coverage_temporal_spatial_relation/pickle_1217" # with or without the last '/' (doesnt matter) the 2 pcikle to try are pickle_1215 and pickle_1217 (dont remember the difference between both, i think it depedns if you lok at low or high lat). There is also pickle_0127 but this one is for cells of 50 km and 200 km wide to compare two planes vs one plane at low latitudes. I actually had to remove pickle_0127 because no more space on computer. It's still on Pleiades though.
pickle_visu_subfolder = "2d_visu" # with or without the last '/' (doesnt matter)
plot_folder = "/Users/cbv/coverage_temporal_spatial_relation/plot0129/"
plot_visu_subfolder = "2d_visu_35"
# end of PARAMETERS TO SET UP BEFORE RUNNING THIS SCRIPT
run_list_arr = np.array(run_list)
label_arr = run_list_arr
label_arr[np.where(run_list_arr == "1_inc_35")[0]] = "CYGNSS"
label_arr[np.where(run_list_arr == "2_ortho_inc_35_35")[0]] = 'CYG + 35'
label_arr[np.where(run_list_arr == "2_ortho_inc_35_65")[0]] = "CYG + 65"
label_arr[np.where(run_list_arr == "2_ortho_inc_35_75")[0]] = "CYG + 75"
label_arr[np.where(run_list_arr == "2_ortho_inc_35_85")[0]] = "CYG + 85"
label_arr[np.where(run_list_arr == "2_ortho_inc_35_35_4sceach")[0]] = "2 planes of 4 sat."
label_arr[np.where(run_list_arr == "1_inc_90")[0]] = "90" # '1 plane of 8 sat.'
label_arr[np.where(run_list_arr == "2_ortho_inc_90_90")[0]] = ""
label_arr[np.where(run_list_arr == "4_ortho_inc_90_90_90_90")[0]] = ""
label_arr[np.where(run_list_arr == "1_inc_65")[0]] = "65"
label_arr[np.where(run_list_arr == "1_inc_70")[0]] = "70"
label_arr[np.where(run_list_arr == "1_inc_75")[0]] = "75"
label_arr[np.where(run_list_arr == "1_inc_80")[0]] = "80"
label_arr[np.where(run_list_arr == "1_inc_85")[0]] = "85"
label_arr[np.where(run_list_arr == "2_ortho_inc_90_90")[0]] = "2 planes of 4 sat."
label_arr[np.where(run_list_arr == "4_ortho_inc_90_90_90_90")[0]] = "4 planes of 2 sat."
...
width_cell_array =  [5, 10, 15, 20, 25] # in km#width_cell_array = [0.01, 0.05, 0.1, 0.15, 0.20, 0.25] #!!!!!! should be [0.01, 0.05, 0.1, 0.15, 0.20, 0.25] # in degrees. for the histogram of revisit time: [200]
...
for irun in range(nb_run):
    print irun, nb_run-1
    nb_cell_covered_average_over_lon_all_run.append(pickle.load(open(pickle_folder + run_list[irun] + "_nb_cell_covered_average_over_lon.pickle"))) 
    percentage_cell_at_least_one_revisit_average_over_lon_all_run.append(pickle.load(open(pickle_folder + run_list[irun] + "_percentage_cell_at_least_one_revisit_average_over_lon.pickle")))
    ...
    for ilat_center in range(nb_lat_center):
        for itype in range(nb_type_cell):
            for icoverage in range(nb_coverage):
                if len(np.where(nb_cell_covered_average_over_lon_all_run[irun][ilat_center, itype, :] >= coverage_array[icoverage])[0]) == 0: # this means the coverage goal coverage_array[icoverage] was not reached for this type of cell at this latitude of the grid for this run
...
# Histogram of revisted time for a given lat and cell type
ilat_center = 0
itype = 0
...
binsize =binsize / 60.
for irun in range(nb_run):
    time_since_last_visit_of_this_cell_list = pickle.load(open(pickle_folder + run_list[irun] + "_time_since_last_visit_of_this_cell_list_ilat_" + str(max_lat_grid_array[ilat_center]) + "_cell_" + str(width_cell_array[itype]) + ".pickle"))
    mean_revisit_time_average_over_lon_all_run.append(pickle.load(open(pickle_folder + run_list[irun] + "_mean_revisit_time_average_over_lon.pickle"))) 
...
# Width of bin vs Time to reach coverage goal for different latitude center of the window, for a given coverage goal -> only one constelaltion configuration (so not a difference in time to reach coverage between two constellation config)
...
x_label = 'Number of hours to reach coverage goal (hours)'
...
for ilat_center in range(0,nb_lat_center):
    ...
    toplot = time_to_reach_coverage_average_over_lon_again_all_run[irun][ilat_center, where_gt_0[0], icoverage]
    ax.scatter(toplot/3600. , width_cell_array[where_gt_0[0]], linewidth = 2, color = color_array[ilat_center], marker = 'o', s = 100)
    ax.loglog(toplot/3600. , width_cell_array[where_gt_0[0]], linewidth = 3, color = color_array[ilat_center], label  = str(max_lat_grid_array[ilat_center]) + u'\N{DEGREE SIGN}')
    if np.max(toplot/3600.)> max_x:
        max_x = np.max(toplot/3600. )

ax.minorticks_off()
xticks = [0,5,10,20,40,60,80,120,200, 360]
xticks_label = []
...
legend = ax.legend(loc='upper right', bbox_to_anchor=(1, 1), numpoints = 1,  title="Lat. of grid", fontsize = fontsize_plot)
legend.get_title().set_fontsize(str(fontsize_plot))


fig_save_name = plot_folder + 'time_to_reach_cov/' + run_list[irun] + '_time_to_reach_coverage_' + str(coverage_array[icoverage]).replace(".","_") + '.pdf'
fig.savefig(fig_save_name, facecolor=fig.get_facecolor(), edgecolor='none', bbox_inches='tight')  
...
ilat_center = np.where(max_lat_grid_array == 80)[0][0]       
itype = -1
icoverage = 2
...
for irun in range(nb_run):# !!!!!!!! range(1,nb_run):
    ax.plot(xaxis, nb_cell_covered_average_over_lon_all_run[irun][ilat_center, itype, :], linewidth = 3, color = color_array[irun], label = label_arr[irun]+ u'\N{DEGREE SIGN}')
...
legend = ax.legend(loc='lower right', bbox_to_anchor=(1, 0), numpoints = 1,  title="Orbit inc.", fontsize = fontsize_plot)#Orbit inc.
legend.get_title().set_fontsize(str(fontsize_plot))

fig_save_name = plot_folder + 'coverage_vs_time/' + root_fig + '_compare_coverage_vs_time_cellwidth_' + str(width_cell_array[itype]).replace(".","_") + '_latitude_' + str(max_lat_grid_array[ilat_center]) + '.pdf'
fig.savefig(fig_save_name, facecolor=fig.get_facecolor(), edgecolor='none', bbox_inches='tight')  
...
for itype in range(nb_type_cell):
    ...
    xaxis = max_lat_grid_array
    for irun in range(nb_run):# !!!!!!!! range(1,nb_run):
        ax.plot(xaxis, mean_revisit_time_average_over_lon_all_run[irun][:, itype]/3600., linewidth = 3, color = color_array[irun], label = str(run_list[irun])) 


#     legend = ax.legend(loc='center left', bbox_to_anchor=(1, 0.5), numpoints = 1,  title="2nd plane inc.", fontsize = fontsize_plot)
#     legend.get_title().set_fontsize(str(fontsize_plot))

    fig_save_name = plot_folder + 'revisit_time/' + root_fig + '_compare_revisit_time_vs_lat_grid_cell_width_' + str(width_cell_array[itype]).replace(".","_") + '.pdf'
    fig.savefig(fig_save_name, facecolor=fig.get_facecolor(), edgecolor='none', bbox_inches='tight')  
...
# load pickle
itype = 0
cell_array_type_lon_lat_cumul_small_cell = pickle.load(open(pickle_folder + pickle_visu_subfolder + run_list[irun]  + "_cell_array_type_lon_lat_cumul_ilat_" + str((int)(max_lat_grid_array[ilat_center]*deg2rad/deg2rad)) + '_cell_' + str(width_cell_array[itype])   + ".pickle")) 
itype = -1
cell_array_type_lon_lat_cumul_big_cell = pickle.load(open(pickle_folder + pickle_visu_subfolder + run_list[irun]  + "_cell_array_type_lon_lat_cumul_ilat_" + str((int)(max_lat_grid_array[ilat_center]*deg2rad/deg2rad)) + '_cell_' + str(width_cell_array[itype])   + ".pickle")) 

lon_spec = pickle.load(open(pickle_folder + pickle_visu_subfolder +  run_list[irun] + "_save_lon_spec.pickle")) /deg2rad
lat_spec = pickle.load(open(pickle_folder + pickle_visu_subfolder + run_list[irun] + "_save_lat_spec.pickle")) /deg2rad
...
# load pickle
irun = 0
cell_array_type_lon_lat_cumul_config1 = pickle.load(open(pickle_folder + pickle_visu_subfolder + run_list[irun]  + "_cell_array_type_lon_lat_cumul_ilat_" + str((int)(max_lat_grid_array[ilat_center]*deg2rad/deg2rad)) + '_cell_' + str(width_cell_array[itype])   + ".pickle")) 
lon_spec_config1 = pickle.load(open(pickle_folder + pickle_visu_subfolder +  run_list[irun] + "_save_lon_spec.pickle")) /deg2rad
lat_spec_config1 = pickle.load(open(pickle_folder + pickle_visu_subfolder + run_list[irun] + "_save_lat_spec.pickle")) /deg2rad

irun = 1
cell_array_type_lon_lat_cumul_config2 = pickle.load(open(pickle_folder + pickle_visu_subfolder + run_list[irun]  + "_cell_array_type_lon_lat_cumul_ilat_" + str((int)(max_lat_grid_array[ilat_center]*deg2rad/deg2rad)) + '_cell_' + str(width_cell_array[itype])   + ".pickle")) 
lon_spec_config2 = pickle.load(open(pickle_folder + pickle_visu_subfolder +  run_list[irun] + "_save_lon_spec.pickle")) /deg2rad
lat_spec_config2 = pickle.load(open(pickle_folder + pickle_visu_subfolder + run_list[irun] + "_save_lat_spec.pickle")) /deg2rad
