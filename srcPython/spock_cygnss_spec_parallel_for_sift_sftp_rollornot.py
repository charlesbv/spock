...
attitude_temp = sys.argv[4]
attitude = "nadir"
if attitude_temp == "plus":
    attitude = "(0; 22; 0) (0; 0; 0)"
elif attitude_temp == "minus":
    attitude = "(0; -22; 0) (0; 0; 0)"
elif attitude_temp == "":
    attitude = "nadir"
elif attitude_temp == "no":
    attitude = "nadir"
else:
    '***! The attitude must be input as "plus", "minus", "no", or "" (empty character). The program will stop. !***'; raise Exception
...
print  >> log_file, path_mpirun + " -np " + nb_proc_avail + " spock_sift " + main_input_filename
if ( ( 'win' in platform ) & (('darwin' in platform ) == False )):
    os.system(path_mpirun + " -np " + nb_proc_avail + " spock_sift.exe " + main_input_filename) # cant redirect otuput. really you can't.
else:
    os.system(path_mpirun + " -np " + nb_proc_avail + " spock_sift " + main_input_filename)
...
if predict_spec == 1:
    ...
    print >> log_file, path_mpirun + " -np " + nb_proc_avail + " spec_sift " + main_input_filename + " -lon=0 -rot=0 -min"
    if ( ( 'win' in platform ) & (('darwin' in platform ) == False )):
        os.system(path_mpirun + " -np " + nb_proc_avail + " spec_sift.exe " + main_input_filename + " -lon=0 -rot=0 -min")
    else:
        os.system(path_mpirun + " -np " + nb_proc_avail + " spec_sift " + main_input_filename + " -lon=0 -rot=0 -min")
    ...
    # print >> log_file, path_mpirun + " -np " + nb_proc_avail + " storm_sift " + main_input_filename + " 0"
    # if ( ( 'win' in platform ) & (('darwin' in platform ) == False )):
    #     os.system(path_mpirun + " -np " + nb_proc_avail + " storm_sift.exe " + main_input_filename + " 0")
    # else:
    #     os.system(path_mpirun + " -np " + nb_proc_avail + " storm_sift " + main_input_filename + " 0")
