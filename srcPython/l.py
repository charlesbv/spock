...
if len(port_ant_prn_list) != 0: # This means that one of the PRN was assigneed to the port antenna starting at the second hald of the overpass + 160 s
    if len(port_ant_prn_list) == 1: # exactly onr PRN was assigned to port
        ...
        second_score_idate.append(prn_that_is_port)
        ...
        first_score_idate.append(prn_min_gap_with_this_port_prn)   
    elif len(port_ant_prn_list) >= 2: # two or more PRNs were assigned to port
        second_score_idate.append(port_ant_prn_list[0])
        first_score_idate.append(port_ant_prn_list[1])
    ...
    while icomb >= -ncomb: # go through score_index_sort from the combination that gives the higeshest score (score_index_sort[-1,:]) to the combination that gives the lowest score (score_index_sort[-ncomb, :])
        ...
        gap_prn_here[icomb] = gap_first_half_prn[prn_out_here, prn_in_here]
    ...
    list_gap = np.where(gap_prn_here == np.min(gap_prn_here))[0]
    ...
    if len(list_gap) > 1: # more than one combination minimzie the gap
        ...
        list_min_gap = []
        score_prn_list_min_gap = []
        list_min_gap.append(score_index_sort[-ncomb + list_gap[imin]])
        score_prn_list_min_gap.append(score_first_half_prn_symm[score_index_sort[-ncomb + list_gap[imin]][0], score_index_sort[-ncomb + list_gap[imin]][1]])
        while ((imin < nmin) & (found_imin == 0)):
            imin = imin + 1
            if score_index_sort[-ncomb + list_gap[imin]] == min_gap: # more than one combination minimze the gap
                list_min_gap.append(score_index_sort[-ncomb + list_gap[imin]])
                score_prn_list_min_gap.append(score_first_half_prn_symm[score_index_sort[-ncomb + list_gap[imin]][0], score_index_sort[-ncomb + list_gap[imin]][1]])
            else:
                found_imin = 1
