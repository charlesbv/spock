...
dt = input_variables[find_in_read_input_order_variables(order_input_variables, 'dt_output')]
...
for isat in range(nb_satellites):
    ...
    if isat == 0:
        ...
        nb_seconds_from_start_to_end = (int)((nb_steps-1)*dt)
...
#for isat in range(nb_satellites):
isat = 0
ax.plot(x_axis, nb_station_in_sight_per_sat[isat, 0, :], linewidth = 2, color = 'b', label = 'North')
ax.plot(x_axis, nb_station_in_sight_per_sat[isat, 1, :], linewidth = 2, color = 'r', label = 'South')
ax.plot(x_axis, nb_station_in_sight_per_sat[isat, 0, :] + nb_station_in_sight_per_sat[isat, 1, :], linewidth = 2, color = 'k', label = 'Both')

legend = ax.legend(loc='upper right', numpoints = 1,  title="", fontsize = fontsize_plot)
legend.get_title().set_fontsize(str(fontsize_plot))

fig_save_name = 'nb_stations_in_sight.pdf'
