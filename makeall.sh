...
if [ $py_only -ne 1 ];then
	...
	# # Specular points binary files
	echo $path_compiler -o "$path_spock_abso"/spec_debug ./src/find_specular_points_aspherical_dev.c -lm -w -I"$path_spice_abso" -I"$path_spice_abso"/include "$path_spice_abso"/lib/csupport.a "$path_spice_abso"/lib/cspice.a # change the path of the executable here
