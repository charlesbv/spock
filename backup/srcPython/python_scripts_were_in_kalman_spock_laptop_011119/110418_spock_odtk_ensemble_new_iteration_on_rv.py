# copy of clean_071318_spock_odtk_ensemble_new_iteration_on_rv.py on 110418
...
obs_rv_filename = 'HD_data/high_drag_data_20180521/spock_FM5_20180521_eng_pvt_query-15601_10h.txt'
obs_att_filename = 'HD_data/high_drag_data_20180521/spock_FM5_20180521_eng_adcs_query-15600_10h.txt' 
...
rho_mod_arr = np.array([0.25, 0.5, 0.75, 1., 1.25, 1.5, 1.75, 2., 3., 4.])#np.arange(rho_mod_min, rho_mod_max+drho_mod, drho_mod)
...
while var_min_stop_simu[-1]*1000 > value_stop_simu:#12:#10.5: # iterate to minimize the distance to observation
    it = it + 1
    main_input_filename = date_start_str.replace(":","_") + '_' + date_end_str.replace(":","_")+ '_iteration' + str(it) + '_gravity20.txt'
...
#         v2 = '4.16872280000000e+01'#'4.16890430000000e+01' #'4.16930640000000e+01'
    ...
    ## Run SpOCK
    if ispleiades != 1:
        os.system(path_mpirun + ' -np ' + str(nb_proc) + ' spock_dev ' + main_input_filename)
    ...
    fig, ax = plt.subplots(); ax.plot(nb_seconds_since_start_spock_ok / 3600., np.array(distance_ref)*1000); plt.show(); fig_save_name = 'fig/' + main_input_filename.replace('.txt', '.png'); fig.savefig(fig_save_name, facecolor=fig.get_facecolor(), edgecolor='none', bbox_inches='tight'); 
...
#             v2 = format(v_spock_ok[0, (int)(which_ens_min_dist[-1]), 2], '.14e')
    ...
    rho_center = which_rho_min_dist[iinter]
    print '-->\n-->'
...
for iinter in range(nb_interval):
    for irho in range(nb_rho):
        ...
        if np.array(distance_rho[iinter][irho][-1])*1000 < ymax:
            ax.text(x_axis[-1], np.array(distance_rho[iinter][irho][-1])*1000, str(rho_mod_arr[irho]), horizontalalignment = 'left', fontsize = fontsize_plot, weight = 'bold', color = 'b', alpha = alpha_arr[irho], verticalalignment = 'center')
        print irho
...
fig_save_name = 'fig/071318_distance_optimum_rho_to_obs_' + main_input_filename_root.replace(".txt", ".pdf")
fig.savefig(fig_save_name, facecolor=fig.get_facecolor(), edgecolor='none', bbox_inches='tight')  
