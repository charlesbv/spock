...
import sys
import numpy as np
inter_input = (int)(sys.argv[1])
rho_input = np.float(sys.argv[2])
...
nb_interval = 24#!!!! (int) ( ( date_obs_end - date_obs_start ).total_seconds()/ ( interval_sec ) ) #!!!!!! should be (int) ( ( date_obs_end - date_obs_start ).total_seconds()/ ( interval_sec ) )
...
for iinter in range(nb_interval):#!!!!! shoul be nb_interval):
    ...
    for ik in range(nk):
        ...
        if iinter >= inter_input:
            rho_control[iinter] = rho_input #!!!!!!!!!! remove
        # !!!!!!! end of  remove block
    ...
    ax.set_xlim([0, 72]); ax.set_ylim([-200, 1200])
    ax.margins(0,0)


    fig_save_name = 'fig/inter_input' + str(inter_input) + "_rho_input" + str(rho_input).replace(".", "") + "_nbinter"+ str(nb_interval) + "_iinter"+ str(iinter) +  ".pdf"
