...
while var_min_stop_simu[-1]*1000 > value_stop_simu:#12:#10.5: # iterate to minimize the distance to observation
    ...
    fig, ax = plt.subplots(); ax.plot(nb_seconds_since_start_spock_ok / 3600., np.array(distance_ref)*1000); plt.show(); fig_save_name = 'fig/' + main_input_filename.replace('.txt', '.png'); fig.savefig(fig_save_name, facecolor=fig.get_facecolor(), edgecolor='none', bbox_inches='tight'); raise Exception
