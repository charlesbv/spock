...
date_start_val_start = '2017-06-03T00:34:00'#"2017-06-01T00:00:00"
dir_run_spock = '.'#'/Users/cbv/cygnss' # '.' # no slash
...
for idate in range(1):#nb_date):# !!!!!!! should be: nb_date):
...
#     print idate, nb_date-1, date_start_val, date_stop_val
    ...
    date_start_val = '2017-06-03T00:34:00'#'2017-06-02T00:15:00' # !!! should be datetime.strftime(date_start_val_array[idate], "%Y-%m-%dT%H:%M:%S")#"2017-08-20T00:00:00" # date to start the validation
    date_stop_val = '2017-06-06T01:02:00' #'2017-06-06T01:02:00'#'2017-06-03T00:45:00'#'2017-06-06T01:02:00'#'2017-06-02T00:56:00'#'2017-06-06T01:02:00'#'2017-06-02T00:43:00'  # !!!! shoul be + timedelta(seconds = 6*24*3600+23*3600+59*60+59), "%Y-%m-%dT%H:%M:%S")#"2017-08-20T06:00:00" # date to stop the validation
...
#                 # !!!!!!!!! end of remove block below                                                     
        ...
        #        r_cyg_eci_netcdf_dt_output[itime,:], v_cyg_eci_netcdf_dt_output[itime,:] = ecef2eci( r_cyg_netcdf_dt_output[itime,:], v_cyg_netcdf_dt_output[itime,:], datetime.strftime(date_spock_not_interpolated_same_time_as_netcdf[itime], "%Y-%m-%dT%H:%M:%S"), 0)
            ...
            diff_fom_same_prn_v=s_fom_time = []#np.zeros([nb_time_spock_netcdf, 16]) - 1e30 # diffce betwe RCG netcdf and RCG SpOCK for spec with same PRN. Keep track of time and of which netcdf RCG the difference is for
        ...
        #         gain_spock_that_min_error[itime] = gain_spock_same_time_as_netcdf[itime][which_spec_spock_min] # spock gain of the spec that minimizes the distance to netcdf
                ...
                for ispec in range(4):
                    ...
                    if len(np.where(gps_spock_same_time_as_netcdf[itime] == prn_netcdf_here)[0]) > 0:
                        ...
                        diff_fom_same_prn_vs_fom_time.append( [itime, fom_netcdf[itime][ispec], gain_spock_same_time_as_netcdf[itime][where_prn_spock] - fom_netcdf[itime][ispec] ] )
        ...
        #     pickle.dump(time_same_prn, open("/Users/cbv/cygnss/pickle/" + spock_input_filename.split('/')[-1].replace(".txt", "")  + "_time_same_prn.pickle", "w"))
        ...
        if os.path.exists("/Users/cbv/cygnss/pickle/pickle_sift/" + spock_input_filename.split('/')[-1].replace(".txt", "")  + "_correct_vs_fom.pickle"): # if netcdf data didn't go thorugh the differerent filters (attitude, mask,...) then these pickles were not created)
        
            correct_vs_fom = pickle.load(open("/Users/cbv/cygnss/pickle/pickle_sift/" + spock_input_filename.split('/')[-1].replace(".txt", "")  + "_correct_vs_fom.pickle"))
            correct_vs_fom_percent = pickle.load( open("/Users/cbv/cygnss/pickle/pickle_sift/" + spock_input_filename.split('/')[-1].replace(".txt", "")  + "_correct_vs_fom_percent.pickle"))
            time_diff_prn = pickle.load( open("/Users/cbv/cygnss/pickle/pickle_sift/" + spock_input_filename.split('/')[-1].replace(".txt", "")  + "_time_diff_prn.pickle"))
            time_same_prn = pickle.load( open("/Users/cbv/cygnss/pickle/pickle_sift/" + spock_input_filename.split('/')[-1].replace(".txt", "")  + "_time_same_prn.pickle"))
            which_spec_diff = pickle.load(open("/Users/cbv/cygnss/pickle/pickle_sift/" + spock_input_filename.split('/')[-1].replace(".txt", "")  + "_which_spec_diff.pickle"))
            gps_spock = pickle.load(open("/Users/cbv/cygnss/pickle/pickle_sift/" + spock_input_filename.split('/')[-1].replace(".txt", "")  + "_gps_spock.pickle"))
            fom_spock = pickle.load(open("/Users/cbv/cygnss/pickle/pickle_sift/" + spock_input_filename.split('/')[-1].replace(".txt", "")  + "_fom_spock.pickle"))
            gps_netcdf = pickle.load(open("/Users/cbv/cygnss/pickle/pickle_sift/" + spock_input_filename.split('/')[-1].replace(".txt", "")  + "_gps_netcdf.pickle"))
            fom_netcdf = pickle.load(open("/Users/cbv/cygnss/pickle/pickle_sift/" + spock_input_filename.split('/')[-1].replace(".txt", "")  + "_fom_netcdf.pickle"))

            fom_netcdf_diff_prn_all = pickle.load(open("/Users/cbv/cygnss/pickle/pickle_sift/" + spock_input_filename.split('/')[-1].replace(".txt", "")  + "_fom_netcdf_diff_prn_all.pickle"))
            nb_spec_diff_gain_not_0 = pickle.load( open("/Users/cbv/cygnss/pickle/pickle_sift/" + spock_input_filename.split('/')[-1].replace(".txt", "")  + "_nb_spec_diff_gain_not_0.pickle"))
            r_spec_diff_lvlh_same_prn = pickle.load( open("/Users/cbv/cygnss/pickle/pickle_sift/" + spock_input_filename.split('/')[-1].replace(".txt", "")  + "_r_spec_diff_lvlh_same_prn.pickle"))
            nb_seconds_since_initial_epoch_spock = pickle.load( open("/Users/cbv/cygnss/pickle/pickle_sift/" + spock_input_filename.split('/')[-1].replace(".txt", "")  + "_nb_seconds_since_initial_epoch_spock.pickle"))
...
for idate in range(nb_date_ok):
    ...
    #x_axis_day = np.arange(0.5, nb_day_simu+0.5)+nb_seconds_since_initial_epoch_spock[0] / 3600. / 24. # start at 0.5 and go up to nb_day_simu + 0.5 
...
diff_fom_same_prn_vs_fom_time_arr = np.array(diff_fom_same_prn_vs_fom_time)
...
for icorrect in range(nb_spec_correct):
    ifom = (int)(diff_fom_same_prn_vs_fom_time_arr[icorrect, 1])
    fom_diff = diff_fom_same_prn_vs_fom_time_arr[icorrect, 2]
...
for ispec in range(4):
    ...
    for idate in range(nb_date_ok):
        # ax.scatter(nb_seconds_since_initial_epoch_spock/ 3600. / 24., r_spec_diff_lvlh_same_prn_all_date[idate][:,ispec,0], marker = '+', s = 5, color = 'k', label = 'Along-track', alpha = 0.4)
        # !!!!!!! reomve line below and uncomment line above
        ax.scatter(np.arange(0, len(r_spec_diff_lvlh_same_prn_all_date[idate][:,ispec,0]), 1) / (len(r_spec_diff_lvlh_same_prn_all_date[idate][:,ispec,0]) - 1) * x_axis_day[-1] ,r_spec_diff_lvlh_same_prn_all_date[idate][:,ispec,0], marker = '+', s = 5, color = 'k', label = 'Along-track', alpha = 0.4)
    ...
    for idate in range(nb_date_ok):
        #ax.scatter(nb_seconds_since_initial_epoch_spock/ 3600. / 24., r_spec_diff_lvlh_same_prn_all_date[idate][:,ispec,1], marker = '+', s = 5, color = 'k', label = 'Along-track', alpha = 0.4)
        # !!!!!!! reomve line below and uncomment line above
...
y_label = 'Percentage (%)'
itime_day = 7 # which day to look at the distirbution over (distibution oof all spec over this day). from 1 (not 0) to nb_day (not nb_day-1). ex: if itime_day = 2 then it'll show the distribution from day 1 to day 2
coordtype = ['Along-track', 'Cross-track']
#for itime_day in range(nb_day):
for idate in range(9,10):#nb_date_ok):
    ...
    #         ax.plot([q90_r_spec_diff_lvlh_all_date[idate][itime_day - 1, ispec,icoord], q90_r_spec_diff_lvlh_all_date[idate][itime_day - 1, ispec,icoord]], [0, 100], linestyle = 'dashed', linewidth = 2, color = 'r')
        ...
        fig_save_name = 'distribution_specular_lvlh_error_ispec_' + coordtype[icoord].replace("-","").lower() + '_day_' + str(itime_day) +'_idate_' + str(idate) +  '.pdf'
        fig.savefig(fig_save_name, facecolor=fig  .get_facecolor(), edgecolor='none', bbox_inches='tight')
