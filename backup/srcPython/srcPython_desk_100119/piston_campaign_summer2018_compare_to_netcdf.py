#This script is a copy of newer_validate_sift.py on 09/19/18. I modified it to validate the files I sent to timLAng during the PISTON campaign in Aug-Oct 2018.
# Important: 
...
download_netcdf = 0 # if 1, doenlaod from sftp 2 the netecdf files coresponinf to the prediction file 
date_start_comparison = '2018-08-15'
date_stop_comparison = '2018-09-03'
# end PARAMETERS TO SET UP BEFORE RUNNING THIS SCRIPT                                                                                                             



date_start_comparison = datetime.strptime(date_start_comparison, "%Y-%m-%d")
date_stop_comparison = datetime.strptime(date_stop_comparison, "%Y-%m-%d")

# Read SpOCK predition file
list_file = []
for file in os.listdir("toshare"):
    if file.endswith("9.txt"):
        list_file.append("toshare/" + file)
...
no_data_netcdf_all_file = []
time_success_all_file = []
nb_file = len(list_file)
list_file_ok = []
for ifile in range(nb_file):
    spock_input_filename = list_file[ifile] #'toshare/20180826T000000_to_20180826T235959.txt'
    spock_input_file = open(spock_input_filename)
    read_spock_input_file = spock_input_file.readlines()
    # only if date is between date_start_comparison and date_stop_comparison
    date_start_here = datetime.strptime(read_spock_input_file[1].split()[0], "%Y-%m-%dT%H:%M:%S" )
    if ( ( date_start_here >= date_start_comparison ) & (date_start_here <= date_stop_comparison) ):
        print spock_input_filename
        list_file_ok.append(spock_input_filename)
        npred = len(read_spock_input_file)
        lon_spock_temp = []; lat_spock_temp = []; date_spock = []
        for ipred in np.arange(1, npred, 10):
            date_spock.append( datetime.strptime(read_spock_input_file[ipred].split()[0], "%Y-%m-%dT%H:%M:%S" ) )
            lon_spock_temp.append( np.float( read_spock_input_file[ipred].split()[1] ) )
            lat_spock_temp.append( np.float( read_spock_input_file[ipred].split()[2] ) )

        lon_spock = np.array(lon_spock_temp)
        lat_spock = np.array(lat_spock_temp)
        date_spock_not_interpolated = date_spock


        # # Read netcdf file
        # ## Downlaod them (needs all 8 files)
        ...
        date_start_date = date_spock[0]
        date_stop_date = date_spock[-1]
        ...
        lat_spec_netcdf_all_sc = [] 
        quality_flags_all_sc = [] 
        lon_spec_netcdf_all_sc = [] 

        fom_netcdf_all_sc = []
        index_netcdf_all_sc = []
        date_flight_all_sc = []
        date_flight_rounded_all_sc = []
        nb_seconds_since_initial_epoch_spock_all_sc = []


        ## Read the netcdf files, for each FM

        for cygfm in range(1, 9): # 1 to 8                                                                                                          
            lat_spec_netcdf = [] 
            quality_flags = [] 
            lon_spec_netcdf = [] 
            ...
            print cygfm
            filename_spec_flight = path_netcdf + yy + '/'  + doy + "/" + [x for x in os.listdir(path_netcdf + yy + '/' + np.str(doy).zfill(3)) if x.endswith(".nc") and x.startswith("cyg0" + np.str(cygfm))][0]
            ...
            fh.close()
            nb_time_flight_temp = len(lat_spec_netcdf_temp)
            date_flight_t = []
            ...
            while itime < nb_time_flight_temp - 60:
                #print itime, nb_time_flight_temp
                itime = itime + 1
        ...
        #         for iaround in np.arange(-60, 60, 1):
        #             if ( (time_remove == 0) & ((date_flight_date_rounded + timedelta(seconds = iaround)) in date_spock)): # if this time is not in date_spock
        #                 ipdb.set_trace()
        #                 index_in_spock_date_netcdf_same.append(date_spock.index(date_flight_date_rounded + timedelta(seconds = iaround) ))
        #             else:
        #                 time_remove = 1
        ...
        #             time_success[index_in_spock_date_netcdf_same[-1]] = 0
        #             spock_pos = (lat_spock[index_in_spock_date_netcdf_same[-1]], lon_spock[index_in_spock_date_netcdf_same[-1]])
        #             for ispec in range(4):
        #                 if index_in_spock_date_netcdf_same[-1] == 0:  
        #                     print spock_pos,  lon_spec_netcdf[-1][ispec], lat_spec_netcdf[-1][ispec] , date_spock[index_in_spock_date_netcdf_same[-1]],date_flight_date_rounded

        #                 # !!!! for PISTON the longitude was between lon 115, 140 and lat 0, 25 so here we exclude SPs outitdes of this region
        #                 if ( ( lon_spec_netcdf[-1][ispec] >= 110 ) & ( lon_spec_netcdf[-1][ispec] <= 145 ) & (lat_spec_netcdf[-1][ispec] >= -5 ) & (lat_spec_netcdf[-1][ispec] <= 30) ): 

        #                     netcdf_pos = (lat_spec_netcdf[-1][ispec], lon_spec_netcdf[-1][ispec]) # for geopy.distance.geodesic: lat lon (lon from -180 to 180). !!!! here no conversion needed cause any the SP at lon > 145 are excluded

        #                     dist_spock_spec = geopy.distance.geodesic(netcdf_pos, spock_pos).km
        # #                     if index_in_spock_date_netcdf_same[-1] == 0:
        # #                         print spock_pos, netcdf_pos, dist_spock_spec

        #                     if dist_spock_spec < dist_threshold:
        #                         time_success[index_in_spock_date_netcdf_same[-1]] = 1
            ...
            lat_spec_netcdf_all_sc.append( lat_spec_netcdf ) 
            quality_flags_all_sc.append( quality_flags ) 
            lon_spec_netcdf_all_sc.append( lon_spec_netcdf ) 

            fom_netcdf_all_sc.append( fom_netcdf )
            index_netcdf_all_sc.append( index_netcdf )
            date_flight_all_sc.append( date_flight )
            date_flight_rounded_all_sc.append( date_flight_rounded )
            nb_seconds_since_initial_epoch_spock_all_sc.append( nb_seconds_since_initial_epoch_spock )


            #raise Exception


        # Copmare SpOCK and Netcdf
        dist_threshold = 20. # distnac ein km under which the predicitno is considered as correct
        interval_find_same = 10. # interval in second to look for the same sp in netcdf file ( actually interval in time stpes of netcdf fbut heses are sually seconds anyway)
        interval_find_same = (int)(interval_find_same)
        no_data_netcdf = []
        nspock = len(date_spock) ###### should be len(date_spock) 
        time_success = np.zeros([nspock]) -1 # at a given index, is 1 if the sp is less than dist_threshol, 0 if greater, -1 if no data in netcdf
        for itime in range(nspock):
            no_data_netcdf_sub = []
            first_time = 0
            print itime, nspock - 1
            spock_pos = (lat_spock[itime], lon_spock[itime])
            for isc in range(8):
            ## find index in netcdf
                if time_success[itime] != 1:
                    try:
                        index = date_flight_rounded_all_sc[isc].index(date_spock[itime])
                        if first_time == 0:
                            time_success[itime] = 0
                        first_time = 1
                        if ((index > interval_find_same) & (index < len(date_flight_rounded_all_sc[isc]) - interval_find_same)):
                            #ipdb.set_trace()
                            for iaround in range(-interval_find_same, interval_find_same):
                                #print date_flight_rounded_all_sc[isc][index+iaround]
                                for ispec in range(len(lon_spec_netcdf_all_sc[isc][index+iaround])):
                                    if ( ( lon_spec_netcdf_all_sc[isc][index+iaround][ispec] >= 110 ) & ( lon_spec_netcdf_all_sc[isc][index+iaround][ispec] <= 145 ) & (lat_spec_netcdf_all_sc[isc][index+iaround][ispec] >= -5 ) & (lat_spec_netcdf_all_sc[isc][index+iaround][ispec] <= 30) ): 
                                        netcdf_pos = (lat_spec_netcdf_all_sc[isc][index+iaround][ispec], lon_spec_netcdf_all_sc[isc][index+iaround][ispec]) # for geopy.distance.geodesic: lat lon (lon from -180 to 180). !!!! here no conversion needed cause any the SP at lon > 145 are excluded
                    ...
                    #                         print spock_pos, netcdf_pos, dist_spock_spec
                                        ...
                                        if dist_spock_spec < dist_threshold:
                                            time_success[itime] = 1
                                            #print '1'
                    except ValueError:
                        no_data_netcdf_sub.append(isc)
            no_data_netcdf.append([itime, no_data_netcdf_sub])

        nb_correct.append( len(np.where(time_success == 1)[0]) )
        nb_incorrect.append( len(np.where(time_success == 0)[0]) )
        nb_total.append( len(np.where(time_success != -1)[0]) )
        percentage_correct.append( nb_correct[-1] * 100./nb_total[-1] )
        percentage_incorrect.append( nb_incorrect[-1] * 100./nb_total[-1] )
        no_data_netcdf_all_file.append(no_data_netcdf)
        time_success_all_file.append(time_success)
                                       
        print 'Correct', percentage_correct[-1]
        print 'Incorrect', percentage_incorrect[-1]
raise Exception
# PARAMETERS TO SET UP BEFORE RUNNING THIS SCRIPT
