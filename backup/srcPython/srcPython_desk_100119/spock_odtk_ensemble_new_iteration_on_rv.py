...
interval = 0.75 # interval of time to compare the two trajectories (data and SpOCK). In hours
...
while min_mean_dist_itime_iens[-1]*1000 > 10: # iterate to minimize the distance to observation
    it = it + 1
    ...
    # the standard deviation on r0/v0 is divided by iter_std_fac but only ig the new distance to observations is smaller than at the previous iteration
        ...
        if min_mean_dist_itime_iens[-1] < min_mean_dist_itime_iens[-2]:
            if it == 1:
                iter_std_fac = 2.
            else:
                iter_std_fac = min_mean_dist_itime_iens[-2] / min_mean_dist_itime_iens[-1]
            sigma_x = np.sqrt(sigma_x)/iter_std_fac
    ...
    which_ens_min_dist.append( np.where( mean_dist_itime_iens ==  np.min(mean_dist_itime_iens) )[0][0] )
    min_mean_dist_itime_iens.append( np.min(mean_dist_itime_iens) )

    main_input_filename_root = main_input_filename
    print it, min_mean_dist_itime_iens[-1]*1000., np.sqrt(sigma_x)
...
r0 = format(r_spock_ok[0, (int)(which_ens_min_dist), 0], '.14e')
r1 = format(r_spock_ok[0, (int)(which_ens_min_dist), 1], '.14e')
r2 = format(r_spock_ok[0, (int)(which_ens_min_dist), 2], '.14e')
v0 = format(v_spock_ok[0, (int)(which_ens_min_dist), 0], '.14e')
v1 = format(v_spock_ok[0, (int)(which_ens_min_dist), 1], '.14e')
v2 = format(v_spock_ok[0, (int)(which_ens_min_dist), 2], '.14e')
