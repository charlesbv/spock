...
nProcs = pickle_list[ pickle_list_name.index('nProcs') ]
nb_ensembles= (int)(nb_ensembles / nProcs) * nProcs
nb_ensembles_per_proc = (int)(nb_ensembles /  nProcs)
if nProcs > np.sqrt( nb_ensembles ):
    print "Note: to plot the density, F10.7, F10.7A, and Ap of ensembles as a function of time, it is recommended to run SpOCK with nProcs <= sqrt( nb_ensembles )."
...
fontsize_plot = 20 
when_plot_in_hour = 48#5 + 50/60. # can be overwritten for each plot by uncommenting the same line in each plot section
...
if "x_eci" in sys.argv:
    if ( 'x_eci_ensemble' in pickle_list_name ):
        ...
        index_when_plot = (int) (when_plot_in_hour * 60L / dt)
...
if "f107" in sys.argv:
    ...
    if ( 'f107_ensemble' in pickle_list_name ):
        ...
        ax1.tick_params(axis='both', which='major', labelsize=fontsize_plot, size = 10, width = 2, pad = 7) # set a bunch of parameters for the ticks and their labels
        [i.set_linewidth(2) for i in ax1.spines.itervalues()] # change the width of the frame of the figure
        for iens in np.arange(nProcs)*((int)((nb_ensembles_per_proc)/np.float(nProcs)) + nb_ensembles_per_proc ):
            if iens == 0:
