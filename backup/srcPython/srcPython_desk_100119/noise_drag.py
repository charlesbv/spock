...
sigma_noise_r = [0.2, 0.2, 0.2] # in m, std for each component of position
sigma_noise_v = [0.001, 0.001, 0.001] # in m/s, std for each component of velocity
sigma_noise_r0 = sigma_noise_r
sigma_noise_v0 = sigma_noise_v

show_plot = 0 # set it to 1 to plot the true and noise orbits
...
for itime in range(n):
    ...
    date.append( datetime.strptime( date_temp, "%Y/%m/%d %H:%M:%S.%f" ) )
...
for itime in range(0, n): # !!!!!! should be range(n)
    if itime == 0:
        ...
        noise_v[itime, 0] = np.random.normal(true_v[itime, 0], sigma_noise_v0[0]) ##### !!!!!!!!!!!! see noise.py
    ...
    # # !!!!!!!!! no measuremnt error below
    # noise_r[itime, 0] = true_r[itime, 0]
    # noise_r[itime, 1] = true_r[itime, 1]
    # noise_r[itime, 2] = true_r[itime, 2]
    # noise_v[itime, 0] = true_v[itime, 0]
    # noise_v[itime, 1] = true_v[itime, 1]
    # noise_v[itime, 2] = true_v[itime, 2]
