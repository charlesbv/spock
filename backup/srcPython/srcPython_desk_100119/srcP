...
pleiades = 1 # set this to 1 if running this script on pleiades
import os
import sys 
if pleiades == 1:
    sys.path.append("/home1/cbussy/installation_spock/spock_development_new_structure_kalman_dev/srcPython")
else:
...
#pickle_folder = "/Users/cbv/coverage_temporal_spatial_relation/pickle_1215_again_lap" # with or without the last '/' (doesnt matter). also the directory doesnt need to exist 
#pickle_folder = "/raid4/cbv/coverage_temporal_spatial_relation" # with or without the last '/' (doesnt matter). also the directory doesnt need to exist 
pickle_folder = "/nobackup/cbussy/coverage_temporal_spatial_relation/pickle_0127_again" # with or without the last '/' (doesnt matter). also the directory doesnt need to exist 
...
width_cell_array = [50,200] #[5, 10, 15, 20, 25] # in km
coverage_array = [70, 80, 90, 95, 99] # in percentage in ASCENDING ORDER
...
for itype in range( nb_type_cell): # !!!!!!!!!!
    ...
    no_revisit_dt = 95*width_cell*np.sqrt(2)/(2*np.pi*re)*60 # this is the max time in seconds that the subsatellite point stays in a cell!!!!!!!! assumes the sc orbital period is 95 min
        ...
        for ilon_center in range(nb_lon_center): #!!!!!!!!!!!
            ...
            for itime in range( nb_time_spec_in_grid ):
                ...
                if cell_ilon_ilat_itype[icell_lat_here-icell_min_lat_grid, icell_lon_here-icell_min_lon_grid] == 0: # if this cell has not been filled yet. We remove icell_min_lat_grid and icell_min_lon_grid because cell_ilon_ilat_itype is defiend from 0 to nb_cell_lat and 0 to nb_cell_lon
                    ...
                    if time_since_last_visit_of_this_cell > no_revisit_dt: # only record the revisit time if it's larger than the time it takes for a sc to fly over the cellx
    ...
    #                                itime_visu = (int)(which_time[itime+1]/dt_visu)+1
            ...
            pickle.dump(time_since_last_visit_of_this_cell_list, open( pickle_folder +  pickle_root +  "_time_since_last_visit_of_this_cell_list_ilat_" + str((int)(max_lat_grid_array[ilat_center]*180./np.pi)) + '_cell_' + str(width_cell_array[itype])   + ".pickle", "w"))
            if nb_time_spec_in_grid > 0 :
                ...
                if ilon_center == 0:  # 2d visu only for one position of grid in longitudinal direction  
                    if ((itype == 0) | (itype == nb_type_cell-1)):
                        ...
                        pickle.dump(cell_array_type_lon_lat_cumul, open( pickle_folder + pickle_visu_subfolder +  pickle_root +  "_cell_array_type_lon_lat_cumul_ilat_" + str((int)(max_lat_grid_array[ilat_center]*180./np.pi)) + '_cell_' + str(width_cell_array[itype])   + ".pickle", "w"))
...
pickle.dump(nb_cell_covered_average_over_lon, open( pickle_folder + pickle_root + "_nb_cell_covered_average_over_lon.pickle", "w"))
pickle.dump(percentage_cell_at_least_one_revisit_average_over_lon, open( pickle_folder + pickle_root + "_percentage_cell_at_least_one_revisit_average_over_lon.pickle", "w"))
pickle.dump(mean_revisit_time_average_over_lon, open( pickle_folder + pickle_root + "_mean_revisit_time_average_over_lon.pickle", "w"))
#    pickle.dump(time_to_reach_coverage_average_over_lon, open(  pickle_folder + pickle_root + "_time_to_reach_coverage_average_over_lon.pickle", "w"))
pickle.dump(save_lon_spec, open( pickle_folder + pickle_visu_subfolder + pickle_root + "_save_lon_spec.pickle", "w"))
pickle.dump(save_lat_spec, open( pickle_folder + pickle_visu_subfolder + pickle_root + "_save_lat_spec.pickle", "w"))
...
if pleiades != 1:
    ...
    fig_save_name = '/Users/cbv/coverage_temporal_spatial_relation/plot/new' + input_filename.replace(".txt", "") + '_coverage_vs_time_latitude_' + str(min_lat_grid).replace(".","_") + '_average_over_lon.pdf'
    fig.savefig(fig_save_name, facecolor=fig.get_facecolor(), edgecolor='none', bbox_inches='tight')  
    ...
    fig_save_name = '/Users/cbv/coverage_temporal_spatial_relation/plot/new/' + input_filename.replace(".txt", "") + '_diff_latitude_coverage_' + str(coverage_array[icoverage]).replace(".","_") + '_average_over_lon.pdf'
    fig.savefig(fig_save_name, facecolor=fig.get_facecolor(), edgecolor='none', bbox_inches='tight')  
    ...
    fig_save_name = '/Users/cbv/coverage_temporal_spatial_relation/plot/new/' + input_filename.replace(".txt", "") + '_diff_coverage_latitude_' + str(min_lat_grid).replace(".","_") + '_average_over_lon.pdf'
    fig.savefig(fig_save_name, facecolor=fig.get_facecolor(), edgecolor='none', bbox_inches='tight')  
    ...
    legend = ax.legend(loc='center left', bbox_to_anchor=(1, 0.5), numpoints = 1,  title="Width of bins", fontsize = fontsize_plot)
    legend.get_title().set_fontsize(str(fontsize_plot))
    fig_save_name = '/Users/cbv/coverage_temporal_spatial_relation/plot/coverage_vs_time_latitude_' + str(min_lat_grid).replace(".","_") + '.pdf'
    fig.savefig(fig_save_name, facecolor=fig.get_facecolor(), edgecolor='none', bbox_inches='tight')  
    ...
    fig_save_name = '/Users/cbv/coverage_temporal_spatial_relation/plot/diff_coverage_latitude_' + str(min_lat_grid).replace(".","_") + '.pdf'
    fig.savefig(fig_save_name, facecolor=fig.get_facecolor(), edgecolor='none', bbox_inches='tight')  
