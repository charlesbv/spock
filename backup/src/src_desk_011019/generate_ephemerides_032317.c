...
int generate_ephemerides(   CONSTELLATION_T  *CONSTELLATION,
			    ...
			    int iDebugLevel)
{
  int output_only_at_tca = 0; // set output_only_at_tca to 1 to output only at tca  (if computing collisions)
  ...
  int save_include_drag, save_include_solar_pressure, save_solar_cell_efficiency;
  int choose_tle_to_initialise_orbit = 0;
  int ccc;
  int compute_collisions = 0;
  //  int start_ensemble;
  // COLLISIONS
  if ( ( OPTIONS->nb_satellites_not_including_gps > 1 ) && ( OPTIONS->nb_ensembles > 0 ) && ( strcmp(OPTIONS->type_orbit_initialisation, "collision" ) == 0 )){
...
/*   - which iProc runs which main sc / which sc is run by which iProc */
  ...
  // For each iProc, set up the first main sc (iStart_save[iProcf]) and the last main sc (iEnd_save[iProcf]) that it's going to run. iStart_save and iEnd_save are two arrays that have the same values for all procs (so if you're iProc 0 or iProc 1, you have value recorded for iStart_save[0] and iEnd_save[0] and the same value recorded for iStart_save[1] and iEnd_save[1]) -> they are not "iProc-dependent"
    ...
    nscEachPe = (OPTIONS->n_satellites - OPTIONS->nb_gps)/nProcs;
    nscLeft = (OPTIONS->n_satellites - OPTIONS->nb_gps) - (nscEachPe * nProcs);
...
/*     } */
  ...
  int ielt;
  int *array_sc;
  array_sc = malloc((OPTIONS->nb_ensemble_min_per_proc + 1) * sizeof(int));
  array_sc[0] = 0;
...
/*     } */
  ...
  for (ii = 0; ii < OPTIONS->n_satellites; ii++) { // go over all sc
	...
	sprintf(temp_nb_proc, "%d", nProcs);
	for (fff = 0; fff < OPTIONS->nb_ensembles_output_files ; fff ++){           
	  if ( (strcmp(OPTIONS->filename_output_ensemble[fff], "tca" )!= 0 ) && (strcmp(OPTIONS->filename_output_ensemble[fff], "dca" )!= 0 )){
...
// write the initial position (at the TLE epoch start)
	...
	while ( ( ( CONSTELLATION->spacecraft[ii][1L + iProc * OPTIONS->nb_ensemble_min_per_proc].et - twrite ) < 0 ) && ( ( twrite - CONSTELLATION->spacecraft[ii][1L + iProc * OPTIONS->nb_ensemble_min_per_proc].et ) > CONSTELLATION->spacecraft[ii][1L + iProc * OPTIONS->nb_ensemble_min_per_proc].INTEGRATOR.dt ) ){
	  ...
	  if (ii < OPTIONS->n_satellites - OPTIONS-> nb_gps){
		...
		propagate_spacecraft( &CONSTELLATION->spacecraft[ii][eee], PARAMS, starttime, &density, GROUND_STATION, OPTIONS, CONSTELLATION, iProc, iDebugLevel );
	...
	propagate_spacecraft( &CONSTELLATION->spacecraft[ii][0], PARAMS, starttime, &density, GROUND_STATION, OPTIONS, CONSTELLATION, iProc, iDebugLevel );
      }
      //      }
      // // ENSEMBLES SPACECRAFT (BUT NOT FOR GPS SATELLITES) 
      if (ii < OPTIONS->n_satellites - OPTIONS-> nb_gps){ // if main sc is not a gps
 ...
 if (iProc == 0){
   if (compute_collisions == 1){ // if computing the probability of collision
     ...
     while ( ( CONSTELLATION->et < endtime -0.01 ) && ( min_altitude_constellation > 100.0) ){ // propagate all unrpertubed orbits to determine times of close approach
       ...
       printf("Propagating the unpertubed orbits to compute times of close approach... %.0f%%\r", ( CONSTELLATION->et - starttime ) / ( endtime - starttime ) *100.0);
	 ...
	 if ( (CONSTELLATION->spacecraft[ii][0].et - twrite) >= OPTIONS->dt - 0.01) { // write output
	   ...
	   if ( ( fmod( CONSTELLATION->spacecraft[ii][0].et - starttime, OPTIONS->dt_output ) < OPTIONS->dt / 2. ) || ( fabs( fmod( CONSTELLATION->spacecraft[ii][0].et - starttime, OPTIONS->dt_output ) - OPTIONS->dt_output ) < OPTIONS->dt / 2. ) || ( CONSTELLATION->spacecraft[ii][0].et > endtime - 0.01) )  {
	     ...
	     write_output( CONSTELLATION->spacecraft[ii] , 0, choose_tle_to_initialise_orbit,ii, OPTIONS->n_satellites,OPTIONS->nb_gps,  OPTIONS->nb_ensembles_min, OPTIONS->nb_ensemble_min_per_proc,  iProc,  OPTIONS->nb_ensembles_output_files, OPTIONS->filename_output_ensemble, previous_lat,OPTIONS,PARAMS->EARTH.earth_fixed_frame,1,0, ( CONSTELLATION->et + OPTIONS->dt) , nProcs, iDebugLevel, compute_collisions, start_ensemble);
	     et2utc_c(CONSTELLATION->spacecraft[ii][0].et, "ISOC" ,0 ,255 , times_att);
	     fprintf(density_file,"%s %e\n", times_att, density);
...
/*   while ( ( CONSTELLATION->et < endtime -0.01 ) && ( min_altitude_constellation > 100.0) ){ // propagate the constellation by dt  */
/*     /\* printf("\n"); *\/ */
/*     /\* pti(iProc, "iProc"); *\/ */
/*     //             etprint(CONSTELLATION->et, "t"); */
...
/* 	      if (ii == 0){//  send the position/velocity/acceleration of a primary sc computed by a iProc to all other iProc  !!!!!!!!!! FOR NOW WORKS ONLY IF TWO REFERENCE SATELLIES ONLY  ("TWO" because see next if condition) */
/* 		if (nProcs > 1){ */

/* 		  for (ccc = 0; ccc < nProcs; ccc++){ // send the position/velocity/acceleration computed by a iProc to all other iProc */
/* 		    if ( ccc != iProc ){ */
...
/* 									  OPTIONS, iProc,  nb_coll_per_step_per_iproc_in_tca[1][itca][iProc], et_time_step_of_save_tca, nb_time_steps_in_tca_time_span, itca, eee_prim, eee, tca_file, dca_file, OPTIONS->write_collision_files );// !!!!!!!!!! FOR NOW WORKS ONLY IF TWO REFERENCE SATELLIES ONLY */
...
/*        if ( ((CONSTELLATION->spacecraft[ii][1 + iProc * OPTIONS->nb_ensemble_min_per_proc].et - twrite) >= OPTIONS->dt - 0.01) ){// && ( itca != -1 ) ) { // write output */
...
/*        if ( ((CONSTELLATION->spacecraft[ii][start_ensemble + iProc * OPTIONS->nb_ensemble_min_per_proc].et - twrite) >= OPTIONS->dt - 0.01) ){// && ( itca != -1 ) ) { // write output */
/* 	 //	    test_print("A")		 */
/* 	 //    etprint(CONSTELLATION->et, "1");		 */
/* /\* 	    etprint(starttime, "start"); *\/ */
/* /\* 	    etprint(CONSTELLATION->spacecraft[ii][start_ensemble + iProc * OPTIONS->nb_ensemble_min_per_proc].et, "et"); *\/ */
/* //	    printf("%e %e %e %f %d || %f\n", CONSTELLATION->spacecraft[ii][start_ensemble + iProc * OPTIONS->nb_ensemble_min_per_proc].et-starttime, OPTIONS->dt_output, starttime, fmod( CONSTELLATION->spacecraft[ii][start_ensemble + iProc * OPTIONS->nb_ensemble_min_per_proc].et - starttime, OPTIONS->dt_output ) , ( fmod( CONSTELLATION->spacecraft[ii][start_ensemble + iProc * OPTIONS->nb_ensemble_min_per_proc].et - starttime, OPTIONS->dt_output ) < 0.01), fabs( fmod( CONSTELLATION->spacecraft[ii][start_ensemble + iProc * OPTIONS->nb_ensemble_min_per_proc].et - starttime, OPTIONS->dt_output ) - OPTIONS->dt_output ) ); */
/* 	    if ( ( fmod( CONSTELLATION->spacecraft[ii][start_ensemble + iProc * OPTIONS->nb_ensemble_min_per_proc].et - starttime, OPTIONS->dt_output ) < OPTIONS->dt / 2.) || ( fabs( fmod( CONSTELLATION->spacecraft[ii][start_ensemble + iProc * OPTIONS->nb_ensemble_min_per_proc].et - starttime, OPTIONS->dt_output ) - OPTIONS->dt_output ) < OPTIONS->dt / 2.) || ( CONSTELLATION->spacecraft[ii][start_ensemble + iProc * OPTIONS->nb_ensemble_min_per_proc].et > min_end_time - 0.01) )  { */
/* 	  //    test_print("B"); */
		    ...
		    int *start_ensemble)
{
  ...
  et2utc_c(SC[0].et, "ISOC" ,3 ,255 , times_with_milliseconds);    
  ...
  if (init_flag == 1) {
    ...
    if (write_reference_sc){
      ...
      fprintf(SC[0].fpout, "// \n");
      fprintf(SC[0].fpout, "// Please note this file is auto generated \n");
	...
	fprintf(SC[0].fpout ,"// Epoch of last TLE for %s: %s \n",  SC[0].name_sat, text);
      ...
      fprintf(SC[0].fpout, "// \n");
      fprintf(SC[0].fpout, "// Version control is under Joel Getchius' Mac and Charles Bussy-Virat University of Michigan's Mac \n");
      fprintf(SC[0].fpout, "// \n");
      fprintf(SC[0].fpout, "// ---------------------------------------------------------------------------------- \n");
	...
	fprintf(SC[0].fptle, "// ---------------------------------------------------------------------------------- \n");
	fprintf(SC[0].fptle, "// \n");
	fprintf(SC[0].fptle, "// Please note this file is auto generated \n");
	...
	fprintf(SC[0].fptle, "// \n");
	fprintf(SC[0].fptle, "// Trajectory is specified with: TIME  \t\tr_i2cg_INRTL(3)(KM) \t\t     v_i2cg_INRTL(3)(KM/S) \t      LONG(DEG)    LAT(DEG)     ALT(KM)       SMA(KM)     INC(DEG)      ECC       TRUE ANO(DEG)  RAAN(DEG) ARG PERIG(DEG) RIGHT ASC(DEG) LOCAL TIME(DEG)  a_i2cg_INRTL(3)(KM/S^2)\n");
	fprintf(SC[0].fptle, "// \n");
	fprintf(SC[0].fptle, "// Version control is under Joel Getchius' Mac and Charles Bussy-Virat University of Michigan's Mac \n");
	fprintf(SC[0].fptle, "// \n");
	fprintf(SC[0].fptle, "// ---------------------------------------------------------------------------------- \n");
	fprintf(SC[0].fptle, "#START \n");
      ...
      fprintf(SC[0].fpecef, "// ---------------------------------------------------------------------------------- \n");
      fprintf(SC[0].fpecef, "// \n");
      fprintf(SC[0].fpecef, "// Please note this file is auto generated \n");
      fprintf(SC[0].fpecef, "// Trajectory Spacecraft %s in a constellation of %d spacecraft and %d GPS satellites \n", SC[0].name_sat, n_sc-n_gps, n_gps);
      if (sc_index >= (n_sc - n_gps))
	fprintf(SC[0].fpecef ,"// Epoch of last TLE for %s: %s \n",  SC[0].name_sat, text);
      ...
      fprintf(SC[0].fpecef, "// \n");
      fprintf(SC[0].fpecef, "// Version control is under Joel Getchius' Mac and Charles Bussy-Virat University of Michigan's Mac \n");
      fprintf(SC[0].fpecef, "// ---------------------------------------------------------------------------------- \n");
      ...
      fprintf(SC[0].fp, "// ---------------------------------------------------------------------------------- \n");
      fprintf(SC[0].fp, "// \n");
      fprintf(SC[0].fp, "// Please note this file is auto generated \n");
	...
	fprintf(SC[0].fp ,"// Epoch of last TLE for %s: %s \n",  SC[0].name_sat, text);
      fprintf(SC[0].fp, "// \n");
      fprintf(SC[0].fp, "// Version control is under Joel Getchius' Mac and Charles Bussy-Virat University of Michigan's Mac \n");
      fprintf(SC[0].fp, "// \n");
      fprintf(SC[0].fp, "// Trajectory is specified with: \n");
      fprintf(SC[0].fp, "//    ET  \t\tr_i2cg_INRTL(3)(KM) \t\t     v_i2cg_INRTL(3)(KM/S) \t      LONG(DEG)    LAT(DEG)     ALT(KM)       SMA(KM)     INC(DEG)      ECC       TRUE ANO(DEG)  RAAN(DEG) ARG PERIG(DEG) RIGHT ASC(DEG) LOCAL TIME(DEG)  a_i2cg_INRTL(3)(KM/S^2)");
	...
	if (SC[0].INTEGRATOR.solar_cell_efficiency != -1){
	  fprintf(SC[0].fpower, "// ---------------------------------------------------------------------------------- \n");
	  fprintf(SC[0].fpower, "// \n");
	  fprintf(SC[0].fpower, "// Please note this file is auto generated \n");
	  ...
	  fprintf(SC[0].fpower, "// \n");
	  fprintf(SC[0].fpower, "// Version control is under Joel Getchius' Mac and Charles Bussy-Virat University of Michigan's Mac \n");
	  fprintf(SC[0].fpower, "// \n");
	  fprintf(SC[0].fpower, "// ---------------------------------------------------------------------------------- \n");
...
/* 	    } */
	    ...
	    fprintf(SC[0].fpiproc[fff], "// \n");
	    fprintf(SC[0].fpiproc[fff], "// Please note this file is auto generated \n");
	    fprintf(SC[0].fpiproc[fff], "// Ensembles for Spacecraft %s in a constellation of %d spacecraft and %d GPS satellites \n", SC[0].name_sat, n_sc-n_gps, n_gps);    
	    fprintf(SC[0].fpiproc[fff], "// \n");
	    fprintf(SC[0].fpiproc[fff], "// Trajectory is specified with:  time %s (%d ensembles - %d processors) \n", name_file_ensembles[fff], nb_ensembles_min, nProcs);
	    fprintf(SC[0].fpiproc[fff], "// \n");
	    fprintf(SC[0].fpiproc[fff], "// Version control is under Joel Getchius' Mac and Charles Bussy-Virat University of Michigan's Mac \n");
	    fprintf(SC[0].fpiproc[fff], "// \n");
	    fprintf(SC[0].fpiproc[fff], "// ---------------------------------------------------------------------------------- \n");
	...
	//      }
      ...
      /* 	    fprintf(SC[0].fpiproc_attitude[iProc][fff], "// \n"); */
      /* 	    fprintf(SC[0].fpiproc_attitude[iProc][fff], "// Please note this file is auto generated \n"); */
      ...
      /* 	    fprintf(SC[0].fpiproc_attitude[iProc][fff], "// \n"); */
      /* 	    fprintf(SC[0].fpiproc_attitude[iProc][fff], "// Version control is under Joel Getchius' Mac and Charles Bussy-Virat University of Michigan's Mac \n"); */
      /* 	    fprintf(SC[0].fpiproc_attitude[iProc][fff], "// \n"); */
      /* 	    fprintf(SC[0].fpiproc_attitude[iProc][fff], "// ---------------------------------------------------------------------------------- \n"); */
  ...
  if (write_reference_sc == 1){
    ...
    fprintf(SC[0].fptle, "%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f",
    	    SC[0].r_i2cg_INRTL[0],
    	    SC[0].r_i2cg_INRTL[1],
    	    SC[0].r_i2cg_INRTL[2],
    	    SC[0].v_i2cg_INRTL[0],
  ...
  if (write_reference_sc == 1){
    ...
    fprintf(SC[0].fp, " " );
    fprintf(SC[0].fp, "%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f",
	    SC[0].r_i2cg_INRTL[0],
	    SC[0].r_i2cg_INRTL[1],
	    SC[0].r_i2cg_INRTL[2],
	    SC[0].v_i2cg_INRTL[0],
    ...
    fprintf(SC[0].fpecef, "%s", text );  //  fprintf(SC[0].fpecef, "%s", text_ecef );
    fprintf(SC[0].fpecef, " " );
    fprintf(SC[0].fpecef, "%f %f %f %f %f %f\n",
...
int interpolate_position(   char filenamella[300],
			    ...
			    int             sat_number,
			    OPTIONS_T *OPTIONS,
			    char main_directory_location[300])
...
int print_progress(double min_end_time, double et , double starttime, int iProc, int nb_gps){
  ...
  if (iProc == 0){ // print progress
    ...
    if (nb_gps > 0){
      printf("Propagating the constellation and the GPS satellites... %.0f%%\r", ( et - starttime ) / ( min_end_time - starttime ) *100.0);
    }
    else {
