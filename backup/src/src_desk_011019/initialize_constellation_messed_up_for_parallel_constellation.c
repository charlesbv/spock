...
/*     for (cccc = 1; cccc < nProcs; cccc++){ */
/*       MPI_Send(&to_send_first, 1, MPI_INT, cccc, 0, MPI_COMM_WORLD); */
/*     } */
 
/*   } // end of if iProc == 0 */
/*   else{ */
/*     if (nProcs > 1){ */
/*       MPI_Recv(&to_send_first, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); */
/*       printf("Iproc %d has value %d for send first\n", iProc, to_send_first  ); */
/*     } */
/*   } */
...
/*   } */
  ...
  // eee = 0 corresponds to the main satellites. So ii = 0 and eee = 0 corresponds to the main sc 0, ii = 1 and eee = 0 to the main sc 1, ii = 1 and eee = 0 to the main sc 2.
  // The main sc are distributed evenly among all iProc. For instance, if there are 3 main sc and 3 iProc, then iProc 0 runs main sc 0 and 1, and iProc 1 runs main sc 2.
  // For each main sc, the ensembles (which are eee >= 1) are distributed evenly among all iProc. For instance, if there are 3 main sc, 50 ensembles (for each main sc), and 3 iProc, then:
  // - iProc 0 runs main sc 0 and 1, and ensemble sc eee = 1 to 25 for EACH main sc 0, 1, and 2
  // - iProc 1 runs main sc 2, and ensemble sc eee = 25 to 50 for EACH main sc 0, 1, and 2
  // Each ensemble (eee >= 1) has the same parameters as its main sc. So all eee >= 1 corresponding to main sc 0 have the same parameters as main sc 0 (SC[0][eee]), all eee >= 1 corresponding to main sc 1 have the same parameters as main sc 1 (SC[1][eee]), and so on (of course, they don't have exactly the same parameterse because as ensembles, w vary some of these parameters but that's done later). This means that if iProc X initialized the parameters for main sc Y (ii = Y and eee = 0), iProc X needs to send all the parameters to all eee >= 1 corresponding to main sc Y (SC[Y][eee>=1]). Since these SC[Y][eee>=1] are distributed evenly among all iProc, this means that iProc X needs to send all the parameters of main SC Y to all iProc.
  // Conclusion:
  // each iProc runs eee >= 1 for every main sc
  // main sc are distributed evenly among the iProc (so each iProc does NOT run every main sc)
  // The variable start_ensemble is set to 0 if the given iProc runs the given main sc. It is set to 1 if the given iProc does not run the given main sc (so it's another iProc that runs this given main sc). In the previous example:
  // - for iProc 0: start_ensemble = 0 for ii = 0 and ii = 1. start_ensemble = 1 for ii = 2
  // - for iProc 1: start_ensemble = 0 for ii = 2. start_ensemble = 1 for ii = 0 and ii = 1
  int iStart, iEnd, nCygEachPe, nCygLeft,i;
  ...
  //    printf("iProc %d | %d - %d\n", iProc, iStart, iEnd - 1);
    ...
    for (iProcf = 0; iProcf < nProcs; iProcf++){
      for (i=0; i<iProcf; i++) {
	iStart_save[iProcf] += nCygEachPe;
	if (i < nCygLeft && iProcf > 0) iStart_save[iProcf]++;
      }
      iEnd_save[iProcf] = iStart_save[iProcf]+nCygEachPe;
      if (iProcf  < nCygLeft) iEnd_save[iProcf]++;
...
/*   } */
  ...
  //  printf("iProc %d: from eee = %d to eee = %d\n", iProc, start_ensemble + iProc * OPTIONS->nb_ensemble_min_per_proc, 1 + iProc * OPTIONS->nb_ensemble_min_per_proc + OPTIONS->nb_ensemble_min_per_proc-1);
  //MPI_Finalize();exit(0);
  for (ii = 0; ii < OPTIONS->n_satellites - OPTIONS->nb_gps; ii++){ // go through all main SC other than GPS
    ...
    for ( ielt = 0; ielt < OPTIONS->nb_ensemble_min_per_proc + 1; ielt ++ ){
      eee = array_sc[ielt];
      printf("iProc %d | ii %d | eee %d\n", iProc, ii, eee);
      if ( (start_ensemble[ii] == 0) && (eee == 0)){ // eee = 0 represents the main spacecraft (eee > 0 is a sc from an ensemble)
	for (ccc = 0; ccc < nProcs; ccc++){
	  if (ccc != iProc){
      ...
      else if ( ( start_ensemble[ii] != 0 ) && ( eee == 0 ) ){// if the iProc is not runnning the main sc (eee = 0 is so that youu receive only once)
	MPI_Recv(&CONSTELLATION->spacecraft[ii][0].OE.sma, 1, MPI_DOUBLE, which_iproc_is_running_main_sc[ii], 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); // iProc reveives from the iProc that is running the main sc
...
/* 	    printf("Sent to %d (out of %d) -  isc %d\n", ccc, nProcs, ii); */
/* 	  } */
/* 	} */
