...
// END CBV 05/09/16
	  ...
	  while (MaxNormPower > -1.0e32) {
	    ...
	    for (iGps=0; iGps<nGps_prop; iGps++) {
	      if (NormPowerAllGps[iGps] > MaxNormPower) {
	      if (iGps == 30){
		//		printf("weird iGps %d  %e %e | %d\n", iGps, NormPowerAllGps[iGps], MaxNormPower, (NormPowerAllGps[iGps] > MaxNormPower) );
	      }

		MaxNormPower = NormPowerAllGps[iGps];
		iMaxNormPower = iGps;
	      if (iNumber == 2){
		//		printf("iGps %d %e %e\n", iGps, NormPowerAllGps[iGps] ,MaxNormPower);
	      }
