...
/* 	  } */
	  ...
	  sat[nSats].latGeod[nPts] = sat[nSats].latGeod[nPts] / dtor;
...
double compute_diff(double theta, double r, double r0, double r1, double d, double delta, double dgeoc_geod, int printvar) { //dgeoc_geod absolute diffference between geocentric and geodetic latitudes
  ...
  diff  = twopi - sigma - betamin - betamin - dgeoc_geod - dgeoc_geod; // !!!!!!! before 09-02-18 used to be diff  = twopi - sigma - 2*beta. after 09-02-18: diff  = twopi - sigma - betamin - betamin - dgeoc_geod - dgeoc_geod
...
int specular_point(float CygLatGeod, float CygLat, float CygLon, float CygR,
  ...
  double timing;
  double latGeod_sp ;
  maxDiff = 0.000001;//!!!!!!!!! before 09-01-18 used to be 0.000001
  ...
  nOuter = 0;
    ...
    dgeoc_geod = fabs(latGeod_sp - lat_sp) * dtor;
      ...
      diffLeft   = compute_diff(theta, rguess, CygR, GpsR, d, deltaLeft, dgeoc_geod, printvar);
      diffCenter = compute_diff(theta, rguess, CygR, GpsR, d, deltaCenter, dgeoc_geod, printvar);
      diffRight  = compute_diff(theta, rguess, CygR, GpsR, d, deltaRight, dgeoc_geod, printvar);
...
/*       } */
    ...
    lat_sp = dlat * delta/theta + CygLat;  ///// before 09-02-18 used to be lat_sp = dlat * delta/theta + CygLat; 
    latGeod_sp = dlatGeod * delta/theta + CygLatGeod;
...
//  lat_sp = asin(sZ/r_sp);
  ...
  if (printvar == 1){
    printf( "lat again %f\n", info->latitude );
    printf("ecef_to_lvlh(r_cyg_spock_ecef_file_same_time_as_netcdf[itime, :], v_cyg_spock_ecef_file_same_time_as_netcdf[itime, :], r_spec_netcdf - np.array([%f, %f, %f]))\n", x_sp/1000, y_sp/1000, z_sp/1000);
...
int find_specular_points( struct SatelliteDetails GPS[nGps],
  ...
  for (iCygnss=iStart; iCygnss<iEnd; iCygnss++) {
	...
	int iPt_temp = 56;// cbv to test things
	if (iCygnss == 3){
		  if (iPt == iPt_temp ){
	  int sss;
	  for (sss = 0; sss < 7; sss++){
...
/* 		tmp = (float) cLat; fwrite(&tmp,sizeof(tmp),1,fpOutput); */
		...
		// CBV 05/09/16
	      ...
	      tmp = (float) cLat; fwrite(&tmp,sizeof(tmp),1,fpOutput); // used to be in iUseMin
