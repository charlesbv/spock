...
// write the initial position (at the TLE epoch start)
      if (strcmp( OPTIONS->type_orbit_initialisation, "tle" ) == 0){
	// !!!!!! was commented on 12-05-18      write_output( CONSTELLATION->spacecraft[ii] , 2, choose_tle_to_initialise_orbit, ii, OPTIONS->n_satellites,OPTIONS->nb_gps, OPTIONS->nb_ensembles_min,  OPTIONS->nb_ensemble_min_per_proc,  iProc,  OPTIONS->nb_ensembles_output_files, OPTIONS->filename_output_ensemble,previous_lat,OPTIONS,PARAMS->EARTH.earth_fixed_frame,1,1,( CONSTELLATION->et ), nProcs, iDebugLevel, compute_collisions, start_ensemble, array_sc, CONSTELLATION, PARAMS);
      }
...
// !!!!!! was commented on 12-05-18 	  write_output( CONSTELLATION->spacecraft[ii] , 2, choose_tle_to_initialise_orbit, ii, OPTIONS->n_satellites,OPTIONS->nb_gps, OPTIONS->nb_ensembles_min,  OPTIONS->nb_ensemble_min_per_proc,  iProc,  OPTIONS->nb_ensembles_output_files, OPTIONS->filename_output_ensemble,previous_lat,OPTIONS,PARAMS->EARTH.earth_fixed_frame,1,1, ( CONSTELLATION->et + OPTIONS->dt ), nProcs, iDebugLevel, compute_collisions, start_ensemble, array_sc, CONSTELLATION, PARAMS);
	...
	if ( OPTIONS->nb_ensembles_min > 0 ) { // if running ensembles
      ...
      if (strcmp( OPTIONS->type_orbit_initialisation, "tle" ) == 0){
   // !!!!!! was commented on 12-05-18    write_output( CONSTELLATION->spacecraft[ii] , 0, choose_tle_to_initialise_orbit, ii, OPTIONS->n_satellites,OPTIONS->nb_gps, OPTIONS->nb_ensembles_min,  OPTIONS->nb_ensemble_min_per_proc,  iProc,  OPTIONS->nb_ensembles_output_files, OPTIONS->filename_output_ensemble,previous_lat,OPTIONS,PARAMS->EARTH.earth_fixed_frame,1,1, ( CONSTELLATION->et + OPTIONS->dt ), nProcs, iDebugLevel, compute_collisions, start_ensemble, array_sc, CONSTELLATION, PARAMS);
      }
		    ...
		    )
{
  ...
  if (init_flag == 1) {
	...
	fprintf(SC[0].fp ,"// Epoch of last TLE for %s: %s \n",  SC[0].name_sat, text);
      ...
      fprintf(SC[0].fp, "//    ET  \t\tr_i2cg_INRTL(3)(KM) \t\t     v_i2cg_INRTL(3)(KM/S) \t      LONG(DEG)    LAT(DEG)     ALT(KM)       SMA(KM)     INC(DEG)      ECC       TRUE ANO(DEG)  RAAN(DEG) ARG PERIG(DEG) RIGHT ASC(DEG) LOCAL TIME(DEG)  a_i2cg_INRTL(3)(KM/S^2) a_i2cg_LVLH(3)(KM/S^2) a_i2cg_LVLH_gravity(3)(KM/S^2) a_i2cg_LVLH_drag(3)(KM/S^2) a_i2cg_INRTL_drag(3)(KM/S^2) a_i2cg_INRTL_gravity(3)(KM/S^2) beta_angle(deg) PHASE_ANGLE(DEG) ARG_PERIG_AVER(DEG) SMA_AVER(KM)");
