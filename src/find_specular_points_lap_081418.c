...
//  printf("%d %d %d\n", order_rotation[0], order_rotation[1], order_rotation[2]);
/*   m_print(pitch_mat, "pitch_mat"); */
/*   m_print(roll_mat, "roll_mat"); */
/*   m_print(yaw_mat, "yaw_mat"); */
...
/* 			 cygnss_v); */
 ...
 double C2S_orbit_norm[3];
 v_norm(C2S_orbit_norm, C2S_orbit);
...
/*  printf("%f %f %f\n", v_angle[0], v_angle[1], v_angle[2]); */
 ...
 // printf("\n%f %f %f\n\n", acos(cygnss_r_dot_C2G)/dtor, acos(cygnss_r_dot_C2G_orbit)/dtor, elev_gps_from_cyg_body);
 //  printf("\n%f %f %f\n\n", info->elev_gps_from_cyg, elev_gps_from_cyg_orbit, elev_gps_from_cyg_body);
  ...
  for (iCygnss=iStart; iCygnss<iEnd; iCygnss++) {
    ...
    if (CYGNSS[iCygnss].IsGood == 1) {
      ...
      for (iPt=0; iPt < nPts; iPt++) { //      for (iPt=1; iPt < nPts; iPt++) { // CBV 05/09/16
		int iPt_temp = 2; // cbv to test things
	if (iCygnss == 3){
		  if (iPt == iPt_temp ){
	  int sss;
	  for (sss = 0; sss < 7; sss++){
