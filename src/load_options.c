...
// A bit before, we read main input file to figure out the path to spice (where leap second file, and other files are). Now we figure out the other SPICE kernels to load
  ...
  if ( strcmp(text_spice, "default") == 0 ){ // if 'default' chosen in section #SPICE. So that means that either the line below the path was empty or that the user put 'default' in this line
    ...
    strcpy(earth_binary_pck_path_temp, "earth_000101_180418_180125.bpc");// !!!!! used to be "earth_000101_160830_160608.bpc");
    strcat(earth_binary_pck_path, "earth_000101_180418_180125.bpc");// !!!!! used to be "earth_000101_160830_160608.bpc");
  }
...
/*   strcat(OPTIONS->tle_constellation_gps_filename, "/"); */
  ...
  int nb_spaces_geo ;
  char *line_copy_geo ;
  line_copy_geo = malloc(sizeof(char) * strlen(line));
...
/*     strcat(filename_f107_ap_mod, "/"); */
	...
	int year_current = tm_current.tm_year + 1900;  
	//	year_current = 2017;// !!!!!!!!!!!!!!!!!! remove collision paper 2
...
int found_new_surface;
  ...
  int nb_phi, nb_theta;
  nb_theta = (int)((180 - OPTIONS->area_attitude_opengl_theta0) / OPTIONS->area_attitude_opengl_dtheta ) + 1;
  nb_phi = (int)((360 - OPTIONS->area_attitude_opengl_phi0) / OPTIONS->area_attitude_opengl_dphi ) + 1;
  ...
  if ( OPTIONS->area_attitude_opengl[itheta] == NULL){
    ...
    for (iphi = 0; iphi < nb_phi; iphi++){
      getline(&line, &len, fp_opengl);
      sscanf(line, "%lf %lf %lf", &theta_dummy, &phi_dummy, &OPTIONS->area_attitude_opengl[itheta][iphi]); // OPTIONS->area_attitude_opengl in cm^2. Will be converted to km^2 in initialize_constellation.c
    }
  }

/*   for (itheta = 0; itheta < nb_theta; itheta++){ */
/*     for (iphi = 0; iphi < nb_phi; iphi++){ */
/*       printf("%f\n", OPTIONS->area_attitude_opengl[itheta][iphi]); */
/*     } */
/*   } */
