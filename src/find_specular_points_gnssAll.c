...
// This file is a copy of find_specular_points.c on June 6 2018. It was written for the analysis with Scottt and CHris on sampling with all GNSS constellations (GPS, Galielo, Glonass, etc).
// I call the executable spec_gnss
#include <stdio.h>
#include <string.h>
#include <math.h>
#include "mpi.h"
...
enum {nTimesMax=720000};//604800}; // used to be: 1209600    (14 days)
enum {nGps=130};
enum {nCygnss=8};
...
int specular_point(float CygLat, float CygLon, float CygR,
  ...
  cct = acos(r/CygR);
  // gps - smax_ancenter of earth - tangent point
  gct = acos(r/GpsR);
...
/*     fy = (theta_sat - AntennaInfo.theta[jj])/AntennaInfo.dTheta; */
    ...
    //        gainR = AntennaInfo.gain[iAnt][(elevIndex * ANTENNA_GAIN_MAP_WIDTH) + azimIndex / 2 ] ;
		...
		printf("%d %d %d %d\n", elevIndex, azimIndex, iAnt ,gainR); 
		//    MPI_Finalize();exit(0);
	      }
...
/* 	} */
	...
	for (iPtInner=0; iPtInner < nPtsInner; iPtInner++) {
	  ...
	  for (iGps=0; iGps<nGps_prop; iGps++) {
	    SpecularPointsAllGps[iGps].gain = -100; // added for all-GNSS (Galieloe, Glonass, GPS, ...) study (not in actual SIFT or for CYGNSS with GPS only)
...
/* 	  } */
	  ...
	  for (iGps=0; iGps<nGps_prop; iGps++) {
	    ...
	    if (((SpecularPointsAllGps[iGps].elev_not_int <= 80.) && (SpecularPointsAllGps[iGps].gain >= 2) ) || ((SpecularPointsAllGps[iGps].gain >= 0) && (SpecularPointsAllGps[iGps].elev_gps_from_cyg >= 10))) {	    //	    if (iOrder[iGps] < nSpecularPointsMax) {
	      ngps_in_sight = ngps_in_sight + 1;
