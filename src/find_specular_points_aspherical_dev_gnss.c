...
/**
 * \brief The azimuth step size for the antenna gain map (degrees)
 */
#define ANTENNA_GAIN_MAP_AZIMUTH_STEP (15)
/**
 * \brief The elevation step size for the antenna gain map (degrees)
 */
#define ANTENNA_GAIN_MAP_ELEVATION_STEP (5)
/**
 * \brief The map size in the azimuth direction
 */
//#define ANTENNA_GAIN_MAP_WIDTH (360 / ANTENNA_GAIN_MAP_AZIMUTH_STEP / 2)
#define ANTENNA_GAIN_MAP_WIDTH (360 / ANTENNA_GAIN_MAP_AZIMUTH_STEP )
/**
 * \brief The map size in the elevation direction
 */
#define ANTENNA_GAIN_MAP_HEIGHT (90 / ANTENNA_GAIN_MAP_ELEVATION_STEP)

/**
 * \brief Elevation mask for reflection processing
 */
#define REFL_ELEVATION_CUT_OFF (5.0)
...
int nSpecularPointsMax = 25; // !!!!!!!! should be 4

double re    = 6378000.0;
double rpole = 6356752.3;//6378000.0; // !!!!!!!! modified by cbv on 09-01018 for aspherical. used to be 6378000.0
double req   =  6378137.0;//6378000.0; // !!!!!!!! modified by cbv on 09-01018 for aspherical. used to be 6378000.0
double dtor  =       0.017453292519943295;
...
struct specular_point_info {
  ...
  double diff;
  double latitude;
  double latitude_geod;
  double longitude;
  double radius;
  int    nIters;

  int8_t elev; // cbv 04-27-18
  int16_t azim; // cbv 04-27-18
  ...
  //  double gain;
  int8_t gain;
  int8_t gain_antenna0;
    int8_t gain_antenna1;
    int8_t which_ant; // 2 is starboard, 3 is port (to follow the conventions made onboard)
  int8_t which_ant_previous; // 2 is starboard, 3 is port (to follow the conventions made onboard)
    double power;
  //int power;
  //  double NormPower;
  int8_t NormPower;
...
enum {nTimesMax=3024000}; // used to be: 604800    (7 days)
enum {nGps=170};
enum {nCygnss=8};
...
double  heading[nTimesMax];
  double lat[nTimesMax]; // geocentric latitude
  double latGeod[nTimesMax]; // geodetic latitude
...
struct AntennaInfoStructure {
  ...
  int8_t gain[2][ANTENNA_GAIN_MAP_HEIGHT * ANTENNA_GAIN_MAP_WIDTH]; // cbv new antenna onboard. 2 for 2 antennas (port and starboard)
  ...
  float yaws[10];
  float pitches[10];
  int numAzFine, numElFine; // number of elevation and azimuth in in the E2ES fine antenna gain antennaRx_approx.bin
};

struct AntennaDataPixelStructure{
  double gain_dB;
  double el_deg;
  double az_deg;
};

/* struct rcgDataStructure{ */
/*   struct *AntennaDataPixelStructure */
  
/* } */

struct AntennaInfoStructure AntennaInfo;
struct AntennaDataPixelStructure *rcg_data;
...
// different from the one in prop_math.c (in prop_math it's inrtl not ecef, and the y is port not startboard and the z in zenith not nadir). also do tsome correction witht he Earth rotation velcoity because it's what tehy do oin the on-board algoirithm 

int compute_T_ecef_2_lvlh_not_like_prop_math( double T_ecef_2_ntw[3][3],
			   double r[3],
			   double v[3])
{
  ...
  double we[3] ; // Earth rotation velcoity
  we[0] = 0.;
  we[1] = 0.;
  we[2] = omega_rot_atmo_scal;

  double vCross[3];
  /* Adjust receiver velocity for Earth rotation */
  v_cross(vCross, we, r);
  double vI[3];
  v_add(vI, v, vCross);
  /* Calculate the nadir vector (Z axis) */
  double norm ;
  v_mag(&norm, r);
  double W[3];
  v_scale(W, r, (-1.0/norm));
  /* Orbit anti-normal (Y axis) */
  v_cross(vCross, r, vI);
  v_mag(&norm, vCross);
  double V[3];
  v_scale(V, vCross, (-1.0/norm));
  /* Along-Track Vector (X axis) */
  double U[3];
  v_cross(U, V, W);
  /* Form the rotation matrix T as [U'; V'; W'] */
  T_ecef_2_ntw[0][0] = U[0]; T_ecef_2_ntw[0][1] = U[1]; T_ecef_2_ntw[0][2] = U[2];
  T_ecef_2_ntw[1][0] = V[0]; T_ecef_2_ntw[1][1] = V[1]; T_ecef_2_ntw[1][2] = V[2];
  T_ecef_2_ntw[2][0] = W[0]; T_ecef_2_ntw[2][1] = W[1]; T_ecef_2_ntw[2][2] = W[2];
...
//  compute_T_enu_to_ecef taken from src/prop_math.c
...
// compute_heading taken from src/generate_ephemerides.c
...
int read_antenna_rcg() { // fine antenna used in the CYGNSS E2ES to calculate an accurate RCG (not used to select the PRN)

  int iError;
  iError = 0;
    FILE *file;
    char filename[25];
    strcpy(filename, "antennaRx_approx.bin");
    // open file
    ...
    int numEl = floor(a);
    int numAz = floor(b);
    printf("   Num Az: %d  Num El: %d \n", numAz, numEl);

    struct AntennaDataPixelStructure antennaDataPixel;
    rcg_data = malloc(numEl * numAz * sizeof(antennaDataPixel));
      if(rcg_data == NULL) {
        printf("Error: Not enough memory to load E2ES antenna data\n");
        exit(1);
    }
    ...
    for(int i=0;i<N;i++){ 
        rcg_data[i].gain_dB = tempBuffer[0*N+i]; 
        rcg_data[i].el_deg  = tempBuffer[1*N+i];
        rcg_data[i].az_deg  = tempBuffer[2*N+i];
	//	printf("%f %f %f\n", rcg_data[i].el_deg, rcg_data[i].az_deg, rcg_data[i].gain_dB);
    }
    AntennaInfo.numAzFine = numAz;
    AntennaInfo.numElFine = numEl;
    free(tempBuffer);
    fclose(file);   
...
// cbv new antenna onboard. function read_antenna_info replaced the one below it. read_antenna_info reads the caorsee antenna gain used onboard to select the PRN
//-----------------------------------------------------------------------
//
//-----------------------------------------------------------------------
...
int read_antenna_info() {
  ...
  for (j=0; j<nThetaAntenna; j++) { // row theta
    ...
    //    sscanf(line_file, "%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f", &AntennaInfo.gain[iant][0][j], &AntennaInfo.gain[iant][1][j], &AntennaInfo.gain[iant][2][j], &AntennaInfo.gain[iant][3][j], &AntennaInfo.gain[iant][4][j], &AntennaInfo.gain[iant][5][j], &AntennaInfo.gain[iant][6][j], &AntennaInfo.gain[iant][7][j], &AntennaInfo.gain[iant][8][j], &AntennaInfo.gain[iant][9][j], &AntennaInfo.gain[iant][10][j], &AntennaInfo.gain[iant][11][j], &AntennaInfo.gain[iant][12][j], &AntennaInfo.gain[iant][13][j], &AntennaInfo.gain[iant][14][j], &AntennaInfo.gain[iant][15][j], &AntennaInfo.gain[iant][16][j], &AntennaInfo.gain[iant][17][j], &AntennaInfo.gain[iant][18][j], &AntennaInfo.gain[iant][19][j], &AntennaInfo.gain[iant][20][j], &AntennaInfo.gain[iant][21][j], &AntennaInfo.gain[iant][22][j], &AntennaInfo.gain[iant][23][j]);
       ...
       sscanf(line_file, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", &AntennaInfo.gain[iant][0 + j * nPhiAntenna], &AntennaInfo.gain[iant][1 + j * nPhiAntenna], &AntennaInfo.gain[iant][2 + j * nPhiAntenna], &AntennaInfo.gain[iant][3 + j * nPhiAntenna], &AntennaInfo.gain[iant][4 + j * nPhiAntenna], &AntennaInfo.gain[iant][5 + j * nPhiAntenna], &AntennaInfo.gain[iant][6 + j * nPhiAntenna], &AntennaInfo.gain[iant][7 + j * nPhiAntenna], &AntennaInfo.gain[iant][8 + j * nPhiAntenna], &AntennaInfo.gain[iant][9 + j * nPhiAntenna], &AntennaInfo.gain[iant][10 + j * nPhiAntenna], &AntennaInfo.gain[iant][11 + j * nPhiAntenna], &AntennaInfo.gain[iant][12 + j * nPhiAntenna], &AntennaInfo.gain[iant][13 + j * nPhiAntenna], &AntennaInfo.gain[iant][14 + j * nPhiAntenna], &AntennaInfo.gain[iant][15 + j * nPhiAntenna], &AntennaInfo.gain[iant][16 + j * nPhiAntenna], &AntennaInfo.gain[iant][17 + j * nPhiAntenna], &AntennaInfo.gain[iant][18 + j * nPhiAntenna], &AntennaInfo.gain[iant][19 + j * nPhiAntenna], &AntennaInfo.gain[iant][20 + j * nPhiAntenna], &AntennaInfo.gain[iant][21 + j * nPhiAntenna], &AntennaInfo.gain[iant][22 + j * nPhiAntenna], &AntennaInfo.gain[iant][23 + j * nPhiAntenna]);
    ...
    for (i=0; i<nPhiAntenna; i++) { // column phi */
      ...
      AntennaInfo.phi[i] = -180 + i * AntennaInfo.dPhi; 
...
double radius_at_point(float lat) { // lat is geocentric latitude, not geodetic latitude, unless using wikipedia source. in degrees

  double r;
  double ecc = EARTH_ECCENTRICITY;
  double rad = EARTH_RADIUS;
  double rdelta, rk;
  double fac;
  double latr = lat * dtor;
  // MODIFIED BY CBV ON 09/01/18 FOR ASPHERICAL EARTH. SOURCE: VALLADO PAGE 148
  fac = rad*sqrt(1-ecc*ecc) / sqrt(1-ecc*ecc*cos(latr)*cos(latr));
  r= fac;
  r = r * 1000. ;
  //rdelta = 
  // end of  MODIFIED BY CBV ON 09/01/18 FOR ASPHERICAL EARTH

  // MODIFIED BY CBV ON 09/01/18 FOR ASPHERICAL EARTH. SOURCE: https://en.wikipedia.org/wiki/Earth_radius. NEED GEODETIC LATITUDE
/*   double num = req*req*cos(latr)*req*req*cos(latr) + rpole*rpole*sin(latr)*rpole*rpole*sin(latr); */
/*   double den = req*cos(latr)*req*cos(latr) + rpole*sin(latr)*rpole*sin(latr); */

/*   r = sqrt( num / den); */
  // end of  MODIFIED BY CBV ON 09/01/18 FOR ASPHERICAL EARTH


  // BEFORE CBV ON 09/01/18, USED TO BE:
  //          r = fabs(lat)/90.0 * (rpole-req) + req;
  // end of BEFORE CBV ON 09/01/18, USED TO BE:
  //    printf("r %f\n",  r);
...
int read_stk(char filename[256], struct SatelliteDetails *sat, int isCYGNSS) {
  ...
  double ecef_v_interpolated[3];
  double earth_flattening = EARTH_FLATENNING;
  double earth_radius = EARTH_RADIUS;
  double r_ecef2cg_ECEF[3];
  double altitude_dummy;
  ...
  while (!IsDone) {
	...
	if (strlen(line) > 20) {
	  ...
	  // !!!!!!!!! added by cbv on 09-01-18 for aspherical
	  r_ecef2cg_ECEF[0] = x/1000;  r_ecef2cg_ECEF[1] = y/1000; r_ecef2cg_ECEF[2] = z/1000;
	  geocentric_to_geodetic( r_ecef2cg_ECEF,
				  &earth_radius,
				  &earth_flattening,
				  &altitude_dummy,
				  &sat[nSats].latGeod[nPts],
				  &sat[nSats].lon[nPts]);

	  sat[nSats].latGeod[nPts] = sat[nSats].latGeod[nPts] / dtor; // geodetic latitude
	  sat[nSats].lon[nPts] = sat[nSats].lon[nPts] / dtor; // 
	  // !!!!!!!!! added by cbv on 09-01-18 for aspherical
	  /// before cbv 09-01-18
	  sat[nSats].lat[nPts] = asin(z/r)/dtor; // geocentric latitude
/* 	  sat[nSats].lon[nPts] = acos(x/xy)/dtor; */
/* 	  if (y < 0) sat[nSats].lon[nPts] = 360.0-sat[nSats].lon[nPts]; */
	  // end of  before cbv 09-01-18

	  sat[nSats].radius[nPts] = r;

	  //printf("lat lon r : %lf %lf %lf\n", sat[nSats].lat[nPts], sat[nSats].lon[nPts], sat[nSats].radius[nPts]);

	  sat[nSats].vx[nPts] = getdouble(line,3)*1000.0; // cbcv added the *1000.0 to convert to m/s because right above the postition is converted from km  to m so need ot be consistent (05-11-18)
	  //printf("here\n");
	  sat[nSats].vy[nPts] = getdouble(line,4)*1000.0;
	  //printf("here %d %d %s\n",nSats, nPts, line);
	  sat[nSats].vz[nPts] = getdouble(line,5)*1000.0;
	  //printf("here\n");


	  /* // !!!!!! REMOVE BLOCK BELOW */
	  /* if (isCYGNSS == 1){ */
	  /*   sat[nSats].vx[nPts] = -1.91 *1000.0; */
	  /*   sat[nSats].vy[nPts] = 6.916 *1000.0; */
	  /*   sat[nSats].vz[nPts] = 0.217 *1000.0; */
	  /* } */
	  /* // !!!!!! REMOVE BLOCK BELOW */
	  
	  if (isCYGNSS == 1){ // compute heading only for CYGNS
	  ecef_v_interpolated[0] = sat[nSats].vx[nPts]; ecef_v_interpolated[1] = sat[nSats].vy[nPts]; ecef_v_interpolated[2] = sat[nSats].vz[nPts];       
	  compute_heading(&sat[nSats].heading[nPts], ecef_v_interpolated, sat[nSats].lon[nPts]*dtor, sat[nSats].lat[nPts]*dtor, earth_flattening); // heading
	  sat[nSats].heading[nPts] = sat[nSats].heading[nPts] / dtor;
	  }
	  else{
	    sat[nSats].heading[nPts]= 0.;
	  }
...
double compute_diff(double theta, double r, double r0, double r1, double d, double delta, double dgeoc_geod, double lat_sp, double lat_cyg, int printvar) { //dgeoc_geod absolute diffference between geocentric and geodetic latitudes. lat_sp and lat_cyg geocentric
  ...
  // !!!!!!!! added by cbv on 09-05-18
  double beta_corrected;
  if (lat_sp < 0){
    if (lat_cyg > lat_sp){
      beta_corrected = beta - dgeoc_geod;
    }
    else{
      beta_corrected = beta + dgeoc_geod;
      if (printvar == 1){
  ...
  // !!!!!!!! end of added by cbv on 09-02-18


  diff  = twopi - sigma - 2*beta_corrected; // !!!!!!! before 09-02-18 used to be diff  = twopi - sigma - 2*beta.
...
int specular_point(float CygLatGeod, float CygLat, float CygLon, float CygR,
		   float GpsLatGeod, float GpsLat, float GpsLon, float GpsR,
		   int iMethod,
		   struct specular_point_info *info, int printvar) {

  if (printvar == 1){
    printf("lat lon %f %f %f| %f %f %f\n", CygLatGeod, CygLat, CygLon, GpsLatGeod, GpsLat, GpsLon);
  }
  double earth_eccentricity = EARTH_ECCENTRICITY;
  double dgeoc_geod;
  ...
  double dlat, dr, dr_threshold, x, dlatGeod;
  double dlatr;
  ...
  double timing;
  double latGeod_sp ;
  maxDiff = 0.00001 ;//!!!!!!!!! before 09-01-18 used to be 0.000001 but an analysis showed that 0.00001 provided the same accuracy (sledetion algo and position)
  dr_threshold = 0.01;//;0.25;//10000000000.0; //!!!!!!!! before 09-01-18 used to be 0.25;
  ...
  dlat = GpsLat-CygLat; // sph for GpsLat and CygLat
  dlatGeod = GpsLatGeod-CygLatGeod; // sph for GpsLat and CygLat

  CygLatr = CygLat*dtor;
  CygLonr = CygLon*dtor;

  dlatr = GpsLat-CygLatr; // sph for GpsLat and CygLat
  ...
  if (printvar == 1){
    printf("cyg %f %f %f\n", CygX*CygR/1000, CygY*CygR/1000, CygZ*CygR/1000);
    printf("gps %f %f %f\n", GpsX*GpsR/1000, GpsY*GpsR/1000, GpsZ*GpsR/1000);
  }
  // gps by spock (modified in OCNSTELLATION to match netcdf):  [21767.272943, -2645.079803, -14975.937355]. Netcdf: [21762.909, -2671.973, -14978.639]
  ...
  //  latGeod_sp = (GpsLatGeod-CygLatGeod)*(0.08+0.2*theta/pi) + CygLatGeod;
  ...
  nOuter = 0;
    ...
    while (fabs(diff) > maxDiff && n < nMax) {
      ...
      // !!!!!!!! added by cbv on 09-02-18 for apsheircal
    lat_sp = dlat * deltaLeft/theta + CygLat;  ///// before 09-02-18 used to be lat_sp = dlat * delta/theta + CygLat; 
    latGeod_sp = atan2( tan(lat_sp * dtor) , (1 - earth_eccentricity * earth_eccentricity)) / dtor; //dlatGeod * delta/theta + CygLatGeod;
    dgeoc_geod = fabs(latGeod_sp - lat_sp) * dtor;
     rguess = radius_at_point(lat_sp); // sph 
      // !!!!!!!! end of added by cbv on 09-02-18 for apsheircal
      diffLeft   = compute_diff(theta, rguess, CygR, GpsR, d, deltaLeft, dgeoc_geod, lat_sp, CygLat,printvar);

      // !!!!!!!! added by cbv on 09-02-18 for apsheircal
    lat_sp = dlat * deltaCenter/theta + CygLat;  ///// before 09-02-18 used to be lat_sp = dlat * delta/theta + CygLat; 
    latGeod_sp = atan2( tan(lat_sp * dtor) , (1 - earth_eccentricity * earth_eccentricity)) / dtor; //dlatGeod * delta/theta + CygLatGeod;
    dgeoc_geod = fabs(latGeod_sp - lat_sp) * dtor;
     rguess = radius_at_point(lat_sp); // sph 
      // !!!!!!!! end of added by cbv on 09-02-18 for apsheircal
      diffCenter = compute_diff(theta, rguess, CygR, GpsR, d, deltaCenter, dgeoc_geod, lat_sp, CygLat, printvar);

      // !!!!!!!! added by cbv on 09-02-18 for apsheircal
    lat_sp = dlat * deltaRight/theta + CygLat;  ///// before 09-02-18 used to be lat_sp = dlat * delta/theta + CygLat; 
    latGeod_sp = atan2( tan(lat_sp * dtor) , (1 - earth_eccentricity * earth_eccentricity)) / dtor; //dlatGeod * delta/theta + CygLatGeod;
    dgeoc_geod = fabs(latGeod_sp - lat_sp) * dtor;
     rguess = radius_at_point(lat_sp); // sph 
      // !!!!!!!! end of added by cbv on 09-02-18 for apsheircal
      diffRight  = compute_diff(theta, rguess, CygR, GpsR, d, deltaRight, dgeoc_geod, lat_sp, CygLat, printvar);
 ...
 /*       } */
  ...
  // !!!!!!!! added by cbv on 09-05-18
  double beta_corrected;
    lat_sp = dlat * delta/theta + CygLat;  ///// before 09-02-18 used to be lat_sp = dlat * delta/theta + CygLat; 
    latGeod_sp = atan2( tan(lat_sp * dtor) , (1 - earth_eccentricity * earth_eccentricity)) / dtor; //dlatGeod * delta/theta + CygLatGeod;
    dgeoc_geod = fabs(latGeod_sp - lat_sp) * dtor;
  ...
  // !!!!!!!! end of added by cbv on 09-02-18
     ...
     diff  = twopi - sigma - 2*beta_corrected; // !!!!!!! before 09-05-18 used to be diff  = twopi - sigma - 2*beta.
     ...
     lat_sp = dlat * delta/theta + CygLat;  ///// before 09-02-18 used to be lat_sp = dlat * delta/theta + CygLat; 
     latGeod_sp = atan2( tan(lat_sp * dtor) , (1 - earth_eccentricity * earth_eccentricity)) / dtor; //dlatGeod * delta/theta + CygLatGeod; !!!! used to be (until 09-06-18)     latGeod_sp = dlatGeod * delta/theta + CygLatGeod;
     rguess = radius_at_point(lat_sp); // sph 
     dr = fabs(rguess-rguessold);
     if (printvar == 1){
       printf("beta beta2 %f %f\n", beta / dtor, beta2/dtor);
       printf("dr %e | lat geocentric %f | delta %f\n", dr, lat_sp, delta / dtor);
     }
     //    printf("diff %e\n", diff);
     //printf("lat_sp : %lf\n",lat_sp);
     //if (dr >= dr_threshold) delta = 0.99*delta;
     /* printf("beta beta2 %f %f %f\n", beta / dtor, beta2/dtor, lat_sp); */
     /* printf("dr %f %f | %f %f | %d\n", dr , dr_threshold, lat_sp, CygLat, fabs(lat_sp) > fabs(CygLat)); */
    //  }

         	} // end of while dr < dr_threshold
	  //printf("\nXXX\n");
...
//printf("timing : %lf\n",timing);
  ...
  CygX_tp = CygR * CygX * clp; // equal to cl * CygX
...
//  lat_sp = asin(sZ/r_sp);
  ...
  lat_sp = asin(z_sp/r_sp); // sph
  latGeod_sp = atan2( tan(lat_sp) , (1 - earth_eccentricity * earth_eccentricity)); 
  xy = sqrt(x_sp*x_sp + y_sp*y_sp); // sph
  if (xy > 0) { // sph
    lon_sp = acos(x_sp/xy); // sph 
    if (y_sp < 0) lon_sp = twopi - lon_sp; // sph
  } else { // sph
    lon_sp = 0.0; // sph
  } // sph
  ...
  info->latitude  = lat_sp/dtor; // geocentric latitude
  info->latitude_geod  = latGeod_sp /dtor; // geodetic latitude
  info->longitude = lon_sp/dtor;
  info->radius    = radius_at_point(lat_sp);


  if (printvar == 1){
    printf( "lat again %f %f | r %f\n", info->latitude, atan2( tan(info->latitude * dtor) , (1 - earth_eccentricity * earth_eccentricity)) / dtor, info->radius / 1000.);
    printf("ecef_to_lvlh(r_cyg_netcdf[itime,:] , v_cyg_netcdf[itime,:], r_spec_netcdf - np.array([%f, %f, %f]))\n", x_sp/1000, y_sp/1000, z_sp/1000);
    //    printf("np.linalg.norm( r_spec_netcdf - np.array([%f, %f, %f]))\n", x_sp/1000, y_sp/1000, z_sp/1000);
...
/*   // !!!!!!!!!! end of remove below */
  ...
  double t, t2, ct, st, xy;// theta_satm phi_satm;
  int theta_sat, phi_sat;
  ...
  double C2G[3];
  C2G[0] = GpsX - CygX;   C2G[1] = GpsY - CygY;   C2G[2] = GpsZ - CygZ; 
  double C2GNorm[3];
  v_norm(C2GNorm, C2G);
...
/* 			 cygnss_v); */
  ...
  compute_T_ecef_2_lvlh_not_like_prop_math( T_ecef_2_ntw,
			 cygnss_r,
			 cygnss_v);


  double cygnss_r_norm[3];
  v_norm(cygnss_r_norm, cygnss_r);
  double cygnss_r_dot_C2G;
  v_dot(&cygnss_r_dot_C2G, cygnss_r_norm, C2GNorm);
  info->elev_gps_from_cyg = 90. -  acos(cygnss_r_dot_C2G)/dtor; // 0 if GPS at horizon from CYGNSS, 90 if GPS right above CYGNSS
...
/*  printf("%f %f %f\n", v_angle[0], v_angle[1], v_angle[2]); */
 ...
 double  cygnss_r_body[3], cygnss_r_orbit[3];
 double  C2G_body[3], C2G_orbit[3];
 double  cygnss_r_body_norm[3], cygnss_r_orbit_norm[3];
 double  C2G_body_norm[3], C2G_orbit_norm[3];

 // ecef to orbit
 double z_orbit_here[3];
 z_orbit_here[0] = 0; z_orbit_here[1] = 0; z_orbit_here[2] = -1;
 m_x_v(cygnss_r_orbit, T_ecef_2_ntw, cygnss_r);
 m_x_v(C2G_orbit, T_ecef_2_ntw, C2G);
 v_norm(cygnss_r_orbit_norm, cygnss_r_orbit);
 v_norm(C2G_orbit_norm, C2G_orbit);
 double cygnss_r_dot_C2G_orbit;
 v_dot(&cygnss_r_dot_C2G_orbit, z_orbit_here, C2G_orbit_norm);
 // orbit to body
 double z_body_here[3];
 z_body_here[0] = 0; z_body_here[1] = 0; z_body_here[2] = -1;
 m_x_v(cygnss_r_body, T_ntw_to_sc, cygnss_r_orbit);
 m_x_v(C2G_body, T_ntw_to_sc, C2G_orbit);
 v_norm(cygnss_r_body_norm, cygnss_r_body);
 v_norm(C2G_body_norm, C2G_body);
 double cygnss_r_dot_C2G_body;
 v_dot(&cygnss_r_dot_C2G_body, z_body_here, C2G_body_norm);

 double elev_gps_from_cyg_orbit, elev_gps_from_cyg_body;
 elev_gps_from_cyg_orbit = 90. -  acos(cygnss_r_dot_C2G_orbit)/dtor; // 0 if GPS at horizon from CYGNSS, 90 if GPS right above CYGNSS
 elev_gps_from_cyg_body = 90. -  acos(cygnss_r_dot_C2G_body)/dtor; // 0 if GPS at horizon from CYGNSS, 90 if GPS right above CYGNSS
 ...
 v_norm(C2S_body_xy_norm, C2S_body_xy);
    ...
    info->azim_not_int = atan2(C2S_body[1], C2S_body[0])/dtor;
    info->azim = (int16_t)(round(atan2(C2S_body[1], C2S_body[0])/dtor));
    
    


/*   if (C2S_body_xy_norm[1] < 0){ */
/*     info->azim = 360. - (int)(round(acos(C2S_body_xy_norm[0])/dtor)); */
/*   } */
/*   else{ */
/*     info->azim = (int)(acos(C2S_body_xy_norm[0])/dtor); */
/*   } */
/*   if (info->azim == 360.){ */
/*     info->azim = 0.; */
/*   } */
/*     printf("%d\n", info->azim); */
    ...
    info->elev_not_int = 90. - asin(C2S_body_norm[2])/dtor;
    info->elev = 90 - (int8_t)(round(asin(C2S_body_norm[2])/dtor)); // 0 if SP right below CYGNSS, 90 if SP at horizon of CYGNSS (this comment is written on Nov 25 2020)
/*     if (printVar == 1){ */
...
/*     printf("c2s: %f %f %f\n",C2Sx, C2Sy, C2Sz); */
  ...
  int8_t gain_antenna0 = -99;
  int8_t gain_antenna1 = -99;
  info->which_ant = 0; 
...
/*     fy = (theta_sat - AntennaInfo.theta[jj])/AntennaInfo.dTheta; */
    double dAzFine, dElFine; // delta azimuth/elevation in the E2ES map
    dAzFine = 360./(AntennaInfo.numAzFine);
    dElFine = 180./(AntennaInfo.numElFine);
    uint8_t azimIndex = (info->azim + 180)/ANTENNA_GAIN_MAP_AZIMUTH_STEP;
    int azimIndex_rcg = (info->azim_not_int + 180)/dAzFine;// for the E2ES map (not used to select PRN);
    //!!!!!! added by cbv on may 12 2019
    ...
    if (azimIndex == AntennaInfo.numAzFine){
      azimIndex = AntennaInfo.numAzFine - 1;
    }
    //!!!!!! end of added by cbv on may 12 2019
    uint8_t elevIndex = info->elev / ANTENNA_GAIN_MAP_ELEVATION_STEP;
    int elevIndex_rcg = (90 + 90 - info->elev_not_int) / dElFine; // info->elev_not_int = 0 if SP below CYGNSS. But the E2ES map assumes that for elev = 90, SP below CYGNSS, which explains the part of the conversion (90 - info->elev_not_int) / dElFine. On top of that, E2ES map starts at elev -90 (and shows unrealistic gains from -90 to 0) -> all gains for elev -90 to 0 in the E2ES should be ignore, thus the conversion (90 + 90 - info->elev_not_int) / dElFine
    int index_rcg_map = azimIndex_rcg*AntennaInfo.numElFine + elevIndex_rcg;
    double gain_fine = rcg_data[index_rcg_map].gain_dB; // gain from the E2ES gain map (not used to select SP)

    info->rcg_fine =  (pow(10,gain_fine/10)/((C2Smag*C2Smag) * (S2Gmag * S2Gmag)))*1e27;
    
    //printf("gain %.2f | elev %.2f %.2f | azim %.2f %.2f\n", rcg_data[index_rcg_map].gain_dB, rcg_data[index_rcg_map].el_deg, 90 - info->elev_not_int , rcg_data[index_rcg_map].az_deg, info->azim_not_int + 180);
    //    printf("elev %.2f azim %.2f -> index  %d (%f - %d, %f - %d)\n", info->elev_not_int, info->azim_not_int + 180, index_rcg_map, dElFine, elevIndex_rcg, dAzFine, azimIndex_rcg);
    // printf("%d %d | %d %d %f | iAnt %d\n", elevIndex, info->elev, azimIndex,info->azim, info->azim_not_int  , iAnt);
    //        gainR = AntennaInfo.gain[iAnt][(elevIndex * ANTENNA_GAIN_MAP_WIDTH) + azimIndex / 2 ] ;
        gainR = AntennaInfo.gain[iAnt][(elevIndex * ANTENNA_GAIN_MAP_WIDTH) + azimIndex ] ;


/* 	if(azimIndex & 1) // if azimindex off */
/*       { */
/* 	//	  printf("azimith %d bbbefore %d\n", azimIndex, gainR); */
/* 	/\* Select the bottom nibble as the gain value to return *\/ */
/* 	gainR = gainR & 0x0F; */
/* /\* 	if (gainR != 0){ *\/ */
/* /\* 	printf("bottom nibble %d\n", gainR); *\/ */
/* /\* 	} *\/ */
/*       } */
/* 	else // if azimindex even */
/*       { */
/* 	/\* Select the top nibble as the gain value to return *\/ */
/* 	//	if (gainR != 0){ */
/* 	//	  printf("azim %d before %d\n", azimIndex, gainR); */
/* 	  //	} */

/* 	  printf("azimith %d aaaefore %d\n", azimIndex, gainR); */
/* 	gainR = (gainR >> 4) & 0x0F; */
/* 	//	if (gainR != 0){ */
/* 	//printf("after nibble %d\n", gainR); */
/* 	//	} */
...
/*            printf("elev %d (ei %d, %f), azim %d (ai %d, ai/1 %d, %f), ei(%d) * wdth(%d) + ai/1(%d) = %d -> %d\n", info->elev, elevIndex, 90 - (asin(C2S_body_norm[2])/dtor), info->azim, azimIndex, azimIndex/1, atan2(C2S_body[1], C2S_body[0])/dtor, elevIndex, ANTENNA_GAIN_MAP_WIDTH, azimIndex / 1,(elevIndex * ANTENNA_GAIN_MAP_WIDTH) + azimIndex / 1, gainR); */
...
/*             (  fx)*(  fy) * AntennaInfo.gain[ii+1][jj+1]; */
    ...
    if (gainR > GainHighest) {

      info->which_ant = iAnt; // !!!! added on 05/09/2019
    }
  ...
  //  exit(0);
  // 2 is starboard, 3 is port (to follow the conventions made onboard). However iAnt is 0 for starboard and 1 for port
  if (info->which_ant == 0){ // antenna is starboard
    info->which_ant = 2;
  }
  else{ // antenna is port
    info->which_ant = 3;
  }
  info->gain_antenna0 = gain_antenna0;
    info->gain_antenna1 = gain_antenna1;
  //
...
/*     if (info->elev_gps_from_cyg < REFL_ELEVATION_CUT_OFF){ */
/*       info->gain = -100; // the onboard algoirthm applies a fileter for lowe CYG to GPS elevation */
/*     } */
...
int fill_specular_point_grid(double rCygnss, double rGps) { // on 09-02-18 cbv commented the call to fill_specular_point_grid from main. cbv also added clatgeod and glatgeod in call to specular_point here bcecause cbv nodified specular_point to have the geodetic latitudes as inputs. so cbv had to be consistent here and add these two inputs, even if they are not computed, which is not a problem since the call to fill_specular_point_grid was commented in main so the function fill_specular_point_grid is not used anymore
  ...
  for (i = 0; i < nPtsGrid; i++) {
    ...
    gR = rGps;
    int printVar = 0;
    err = specular_point(cLatGeod, cLat, cLon, cR, 
			 gLatGeod, gLat, gLon, gR,
			 0,
			 &specularpoint,printVar);
...
int find_specular_points( struct SatelliteDetails GPS[nGps],
  ...
  for (iCygnss=iStart; iCygnss<iEnd; iCygnss++) {
	...
	int iPt_temp = 0;// cbv to test things
	int iCygnss_temp = -1;//3;
	if (iCygnss == iCygnss_temp){ // 3
	   if (iPt == iPt_temp ){
	  int sss;
	  for (sss = 0; sss < 7; sss++){
	...
	if ( fabs((CYGNSS[iCygnss].time[iPt+1] - CYGNSS[iCygnss].time[iPt]) - cDt) < 0.01 ){ // in SpOCK, it's possible that the duratino of the last time step of the output is different from the duration of the other time steps. This happens when the final epoch doesn't fall excalty on initial epoch + multiple of output time step. In that case, if the duration of the time step is not the same as the first one (ie as all the other ones) then don't write in the output file of the specular point location
	...
	for (iPtInner=0; iPtInner < nPtsInner; iPtInner++) {
	  ...
	  cLatGeod = x * CYGNSS[iCygnss].latGeod[iPt+1] + (1-x) * CYGNSS[iCygnss].latGeod[iPt];
	  ...
	  cHeading = x * CYGNSS[iCygnss].heading[iPt+1] + (1-x) * CYGNSS[iCygnss].heading[iPt];


	  for (iGps=0; iGps<nGps_prop; iGps++) {
	    NormPowerAllGps[iGps] = -99;
	  ...
	  for (iGps=0; iGps<nGps_prop; iGps++) {
	    ...
	    if (GPS[iGps].IsGood == 1) {
	      ...
	      gLatGeod = x * GPS[iGps].latGeod[iPt+1] + (1-x) * GPS[iGps].latGeod[iPt];
		...
		if (iPt == iPt_temp ){
		    if (strcmp(GPS[iGps].name, "PRN_999") == 0){
		    printVar = 1;
		    }
		  }
		  		}
		...
		SpecularPointsAllGps[iGps].elev_gps_from_cyg = specularpoint.elev_gps_from_cyg;
		SpecularPointsAllGps[iGps].azim = specularpoint.azim;
		SpecularPointsAllGps[iGps].azim_not_int = specularpoint.azim_not_int;
		SpecularPointsAllGps[iGps].elev_not_int = specularpoint.elev_not_int;
		// end of cbv 04-27-18
...
// END CBV 05/09/16
		...
		SpecularPointsAllGps[iGps].latitude     = specularpoint.latitude; // geocentric
		SpecularPointsAllGps[iGps].latitude_geod     = specularpoint.latitude_geod; // geodetic
		...
		SpecularPointsAllGps[iGps].gain_antenna0         = specularpoint.gain_antenna0;
		SpecularPointsAllGps[iGps].gain_antenna1         = specularpoint.gain_antenna1;
		...
		// CBV in 11/25/20
		SpecularPointsAllGps[iGps].rcg_fine = specularpoint.rcg_fine;
	  ...
	  if (iMaxNormPower > -1) {
	    ...
	    NormPowerAllGps[iMaxNormPower] = -100;
	    iNumber++;
	  }

	  // Determine the order of the other GPS satellites:
	  while (MaxNormPower > -99) {
	    MaxNormPower = -99;
	    iMaxNormPower = -1;
	    for (iGps=0; iGps<nGps_prop; iGps++) {
	      if (NormPowerAllGps[iGps] > MaxNormPower) {
...
/* 	  } */
	  ...
	  for (iGps=0; iGps<nGps_prop; iGps++) {
	    ...
	    if (iOrder[iGps] < nSpecularPointsMax) {
	      iprn_selected = iprn_selected + 1;
	      record_prn[iprn_selected] = iGps;

	      if (reproduce_bug_onboard == 1){
	      if (record_prn_previous_step[0] != -1){ // record_prn_previous_step[0] = 1 only at the first time step. Inside the if, we look at the GPS that were selected at the previous step, which implies that the current step can't be the first step.
	      for (iii_prev = 0; iii_prev < nSpecularPointsMax; iii_prev ++){
		if (iGps == record_prn_previous_step[iii_prev]){ // this means that the GPS was selected in the previous step
		    prn_was_selected_previous_step = 1;
		  }
	      }
	      ...
	      if (iCygnss == iCygnss_temp){
		if (iPtInner == 0){
		  if (iPt == iPt_temp ){//> nPts - 4 ){wdi
		    //		    if ( strcmp(GPS[iGps].name, "PRN_26") == 0){
		    printf("GPS[%d]: <%s> | normPower %d | x %f | elev %f (%d), azim %f (%d) | %d | %f %f %f | %d\n", iGps, GPS[iGps].name, SpecularPointsAllGps[iGps].NormPower, GPS[iGps].x[iPt]/1000., SpecularPointsAllGps[iGps].elev_not_int,  SpecularPointsAllGps[iGps].elev, SpecularPointsAllGps[iGps].azim_not_int,SpecularPointsAllGps[iGps].azim, iOrder[iGps], SpecularPointsAllGps[iGps].SpX, SpecularPointsAllGps[iGps].SpY,SpecularPointsAllGps[iGps].SpZ, SpecularPointsAllGps[iGps].which_ant);
		    //		      printf("<%s> | %f %f %f | %f %f %f | %f\n", GPS[iGps].name,  GPS[iGps].x[iPt]/1000., GPS[iGps].y[iPt]/1000., GPS[iGps].z[iPt]/1000., CYGNSS[iCygnss].x[iPt]/1000., CYGNSS[iCygnss].y[iPt]/1000., CYGNSS[iCygnss].z[iPt]/1000.,CYGNSS[iCygnss].lat[iPt]);
...
/* 		tmp = (float) cLat; fwrite(&tmp,sizeof(tmp),1,fpOutput); */
		...
		// CBV 05/09/16
	      ...
	      tmp = (float) cLon; fwrite(&tmp,sizeof(tmp),1,fpOutput); // used to be in iUseMin
	      tmp = (float) cLat; fwrite(&tmp,sizeof(tmp),1,fpOutput); // used to be in iUseMin. geocentric
	      tmp = (float) cHeading; fwrite(&tmp,sizeof(tmp),1,fpOutput);
    ...
    /* // !!!!!!!!!!!!!!!! END OF COMMENT THESE LINES BELOW! */
		...
		// END CBV 05/09/16
	      ...
	      //	      tmp = (float) SpecularPointsAllGps[iGps].latitude;  fwrite(&tmp,sizeof(tmp),1,fpOutput); // geocentric
	      tmp = (float) SpecularPointsAllGps[iGps].latitude_geod;  fwrite(&tmp,sizeof(tmp),1,fpOutput); // geodetic
		...
		fwrite(&tmp,sizeof(tmp),1,fpOutput);
	      ...
	      tmp = (float) SpecularPointsAllGps[iGps].rcg_fine;  fwrite(&tmp,sizeof(tmp),1,fpOutput); // geodetic
	      ...
	      if ( debug_cbv == 1){
	      tmp =  (float) cygnss_vx_inner/1000; fwrite(&tmp,sizeof(tmp),1,fpOutput);
	      tmp =  (float) cygnss_vy_inner/1000; fwrite(&tmp,sizeof(tmp),1,fpOutput);
	      tmp =  (float) cygnss_vz_inner/1000; fwrite(&tmp,sizeof(tmp),1,fpOutput);
	      }
...
/* 	      tmp =  (float) CYGNSS[iCygnss].vz[iPt]; fwrite(&tmp,sizeof(tmp),1,fpOutput); */
	      ...
	      if ( debug_cbv == 1){
	      int8_t tmp_normpower = SpecularPointsAllGps[iGps].NormPower;
	      fwrite(&tmp_normpower,sizeof(tmp_normpower),1,fpOutput);
	      int8_t tmp_elev = SpecularPointsAllGps[iGps].elev;
	      fwrite(&tmp_elev,sizeof(tmp_elev),1,fpOutput);
	      int16_t tmp_azim = SpecularPointsAllGps[iGps].azim;
	      fwrite(&tmp_azim,sizeof(tmp_azim),1,fpOutput);
	      tmp =  SpecularPointsAllGps[iGps].elev_gps_from_cyg;
	      fwrite(&tmp,sizeof(tmp),1,fpOutput);
	      tmp =  SpecularPointsAllGps[iGps].elev_not_int;
	      fwrite(&tmp,sizeof(tmp),1,fpOutput);
	      tmp =  SpecularPointsAllGps[iGps].azim_not_int;
	      fwrite(&tmp,sizeof(tmp),1,fpOutput);
	      }
	      // END CBV 05/09/16
	      if (iUseMin == 0) {
	    ...
	    SpecularPointsAllGps[iGps].which_ant_previous = SpecularPointsAllGps[iGps].which_ant;
	  }
	...
	} // end of if time step duration is different from first one
...
/*     strcpy(AntennaInfo.Sigma0FileName,"./input/specular_points/sigma0_table.bin"); */
    ...
    err = read_antenna_info(); // reads the coarse antenna gain used onboard to select the PRN
    err = read_antenna_rcg(); // fine antenna used in the CYGNSS E2ES to calculate an accurate RCG (not used to select the PRN)
    //        printf("Reading CYGNSS...\n");
    //    strcpy(CygnssFilename,argv[1]); // CBV 05/08/16


    err = read_stk(CygnssFilename, CYGNSS, 1);
...
return 0;
    ...
    err = read_stk(GpsFilename, GPS, 0);
...
return 0;
    ...
    // err = fill_specular_point_grid(rCygnss, rGps); !!!!!!!!! commented by cbv on 09-01-2018 beause it looks like it's used only if iMethod != 0 in call to specular_point function. But iMethod is set to 0 in find_specular_points function so I don't think this call to fill_specular_point_grid is relevant
