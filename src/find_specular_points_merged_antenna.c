...
// This script is a copy of find_specular_points_aspherical.c on 01/24/2019
// It performs the same operations but uses a single antenna
// pattern file, which is the merged antenna pattern from
// ant_1_starboard_ddmi_v1.agm and ant_1_port_ddmi_v1.agm
// The merged file, merged_ant_1_starboard_ddmi_v1_with_ant_1_port_ddmi_v1.agm,
// was created by the python script cygnss_antenna_merge.py
...
struct AntennaInfoStructure {
  ...
  int8_t gain[1][ANTENNA_GAIN_MAP_HEIGHT * ANTENNA_GAIN_MAP_WIDTH]; // a merged antenna
  float dTheta, dPhi;
...
int read_antenna_info() {
  ...
  while (!IsDone) {
    ...
    if (strstr(line,"#N") != NULL) {
      ...
      AntennaInfo.nAnt = 1; // !!!! added by cbv on 01/24/2019 for merged antenna. the yaws pitches and rolls read in this function are not used anyway
      //printf("%d\n",AntennaInfo.nAnt);
    }
...
/*             (  fx)*(  fy) * AntennaInfo.gain[ii+1][jj+1]; */
	...
	// MERGED ANTENNA COMMENT BLOCK BELOW
    /* if  ((iAnt == 0 ) && ((90 - asin(C2S_body_norm[2])/dtor) > 28) && (atan2(C2S_body[1], C2S_body[0])/dtor > 0 )) {// any elevation greater than 28 deg and on the starboard side cannot be seen by the port antenna so is asigned a gain of -100 (that's what the onboard algoirthm does). Be caareful: this means antenna 0 is port and antenna 1 is starboard */
    /*   gainR = -100; */
    /* } */

    /* if  ((iAnt == 1 ) && ((90 - asin(C2S_body_norm[2])/dtor) > 28) && (atan2(C2S_body[1], C2S_body[0])/dtor < 0 )) {// any elevation greater than 28 deg and on the port side cannot be seen by the starboard antenna so is asigned a gain of -100 (that's what the onboard algoirthm does). Be caareful: this means antenna 0 is port and antenna 1 is starboard */
    /*   gainR = -100; */
    /* } */
	// end of MERGED ANTENNA COMMENT BLOCK BELOW
...
int find_specular_points( struct SatelliteDetails GPS[nGps],
  ...
  for (iCygnss=iStart; iCygnss<iEnd; iCygnss++) {
	...
	int iPt_temp = 4*60+25;// cbv to test things
	...
	for (iPtInner=0; iPtInner < nPtsInner; iPtInner++) {
	  ...
	  for (iGps=0; iGps<nGps_prop; iGps++) {
		...
		if (iPt == iPt_temp ){
		    if (strcmp(GPS[iGps].name, "PRN_06") == 0){
		    printVar = 1;
		    }
		  }
		  		}
	  ...
	  for (iGps=0; iGps<nGps_prop; iGps++) {
		...
		if (iPtInner == 0){
		  if (iPt == iPt_temp ){//> nPts - 4 ){wdi
		    //		    		       if ( strcmp(GPS[iGps].name, "PRN_20") == 0){
/* 		    		    printf("GPS[%d]: <%s> | normPower %d | x %f | elev %d, azim %d | %d | %f %f %f\n", iGps, GPS[iGps].name, SpecularPointsAllGps[iGps].NormPower, GPS[iGps].x[iPt]/1000., SpecularPointsAllGps[iGps].elev, SpecularPointsAllGps[iGps].azim, iOrder[iGps], SpecularPointsAllGps[iGps].SpX, SpecularPointsAllGps[iGps].SpY,SpecularPointsAllGps[iGps].SpZ); */
...
/*     strcpy(AntennaInfo.Sigma0FileName,"./input/specular_points/sigma0_table.bin"); */
    ...
    strcat(AntennaInfo.AntennaFileNamePort,"merged_ant_1_starboard_ddmi_v1_with_ant_1_port_ddmi_v1.agm"); // cbv new antenna onboard. before: nothing
