...
int load_options( OPTIONS_T *OPTIONS,
  ...
  double et_initial;
	    char filename_f107_ap_pred[256], wget_filename_f107_ap_pred[1000];
...
//newstructure                                                                                 
  ...
  if ( strcmp(text, "now") == 0){
    ...
    MPI_Send(&et_initial, 1, MPI_DOUBLE, ccc, 0, MPI_COMM_WORLD);
      ...
      if (nProcs > 1){
      MPI_Recv(&et_initial, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD,
    	       MPI_STATUS_IGNORE);
	...
	et2utc_c(et_initial, "ISOC", 0, 255, OPTIONS->initial_epoch); // don't worry about it
    ...
    char times_end[256];
    et2utc_c(et_end, "ISOC" ,0 ,255 , times_end);    
  ...
  if ( fabs(OPTIONS->dt -  fmod( et_end - et_start, OPTIONS->dt ) ) > 0.01 ){ // 0.01 for numerical reasons
    ...
    et2utc_c(et_end, "ISOC", 0, 255, OPTIONS->final_epoch);
    if (iProc == 0){
...
// A bit before, we read main input file to figure out the path to spice (where leap second file, and other files are). Now we figure out the other SPICE kernels to load
  ...
  if ( strcmp(text_spice, "default") == 0 ){ // if 'default' chosen in section #SPICE. So that means that either the line below the path was empty or that the user put 'default' in this line
    ...
    strcpy(earth_binary_pck_path_temp, "earth_000101_171116_170825.bpc");// !!!!! used to be "earth_000101_160830_160608.bpc");
    strcat(earth_binary_pck_path, "earth_000101_171116_170825.bpc");// !!!!! used to be "earth_000101_160830_160608.bpc");
  }
  else { // if the user wants to use a particular earth_binary_pck file, then this file needs to be put in path_to_spice  (with the other spice files). IMPORTANT: the name of the file must have the format string1_string2_YYMMDD_string3.bpc where YYMMDD is the date of the last available information on the frame (so we don't know anything for the rotating frame any time after this date). Example: earth_000101_160830_160608.bpc (after 160830 we can't use ITRF93)
    strcpy(earth_binary_pck_path,OPTIONS->path_to_spice);
...
/*     strcpy(earth_binary_pck_path_temp, "earth_000101_160830_160608.bpc"); */
/*     strcat(earth_binary_pck_path, "earth_000101_160830_160608.bpc"); */
/*   }  */
...
/* //newstructure */
  ...
  getline(&line,&len,fp);
  sscanf(line,"%lf",&OPTIONS->density_mod);
  }
...
/*   strcat(OPTIONS->tle_constellation_gps_filename, "/"); */
  ...
  getline(&line, &len, fp);
  sscanf(line, "%lf", &OPTIONS->solar_cell_efficiency);
...
/*   strcpy(OPTIONS->filename_surface, ""); */
  ...
  //newstructure
  strcpy(OPTIONS->filename_surface,text_location);
  strcat(OPTIONS->filename_surface, text_surface);
  strtok(OPTIONS->filename_surface, "\n");
...
/* 	  strcat(sat_tle_file_temp,"/"); */
  ...
  else{ //if tle are not used then itnialize et_oldest_tle_epoch and oldest_tle_epoch to the initial epoch of the constellation
...
/* 	strcat(filename_ap, "/"); */
	...
	lin_interpolate(OPTIONS->f107, OPTIONS->f107A, OPTIONS->Ap, OPTIONS->Ap_hist, OPTIONS->et_interpo, &OPTIONS->use_ap_hist, filename_f107_to_calculate_f107_average, filename_ap, "omniweb", OPTIONS->nb_time_steps * 2, OPTIONS->oldest_tle_epoch, OPTIONS->final_epoch,OPTIONS->dt, 999.9,iDebugLevel, iProc);       // "* 2.0" because of the Runge Kunta orfer 4 method
	getline(&line, &len, fp);
/* 	for (ccc = 0; ccc < OPTIONS->nb_time_steps * 2; ccc++){ */
/* 	  etprint(OPTIONS->et_interpo[ccc], ""); */
/* 	  printf("%f %f\n", OPTIONS->f107[ccc], OPTIONS->f107A[ccc]); */
/* 	} */
  ...
  int ifile_f107 = 0, ifile_ap = 0;
	...
	if ( OPTIONS->swpc_need_predictions == 1){
	  ...
	  if ( (  OPTIONS->et_oldest_tle_epoch  >=  et_current_day_plus_45_day + 24*3600 ) || (  et_final_epoch >=  et_current_day_plus_45_day + 24*3600  ) ){ // actually plus 46  because we have the prediction on day + 45
	    printf("***! You chose to compute the drag with predictions of the solar activity from SWPC. However, these predictions are limited to 45 days in the future. Therefore, the spacecraft can not be propagated further in the future than the current date + 45 days (the propagation has to end before %s). The program will stop. !***\n", current_day_plus_45_day); MPI_Finalize(); exit(0);
	  }
	  else{
...
/* 	    strcat(filename_f107_ap_pred, "/"); */
 ...
 MPI_Barrier( MPI_COMM_WORLD ) ; 
	...
	exit(0);
	  ...
	  lin_interpolate_swpc(OPTIONS->f107, OPTIONS->f107A, OPTIONS->Ap, OPTIONS->et_interpo, OPTIONS->sigma_f107, OPTIONS->sigma_ap, OPTIONS->et_sigma_f107_ap, &OPTIONS->use_ap_hist, list_filename_obs_swpc_f107, list_filename_obs_swpc_ap, filename_f107_ap_pred,  OPTIONS->nb_time_steps * 2, OPTIONS->initial_epoch, OPTIONS->final_epoch, OPTIONS->dt, 999.9,iDebugLevel, nb_file_obs_swpc, nb_file_obs_swpc_ap, OPTIONS->swpc_need_observations, OPTIONS->swpc_need_predictions, OPTIONS->nb_ensembles_density, OPTIONS->dir_input_density_msis, &OPTIONS->swpc_et_first_prediction, iProc); 
...
/*     strcat(filename_f107_ap_mod, "/"); */
	...
	etprint(et_current_day, "et_current_day");
	//	str2et_c(OPTIONS->initial_epoch, &et_initial_epoch);
	str2et_c(OPTIONS->final_epoch, &et_final_epoch);
	char initial_epoch_midnight[256], final_epoch_midnight[256];
	strcpy(initial_epoch_midnight, "");  strcpy(final_epoch_midnight, "");
...
/* 	  strcat(final_filename_f107_obs, "/"); */
	  ...
	  if (epoch_year_same_as_current_year == 1){
	    ...
	    else if ( (  OPTIONS->et_oldest_tle_epoch  < et_second_quarter ) && ( et_initial_epoch >= et_first_quarter ) ) {
	      quarter_initial_epoch = 2;
	    }
	    else if ( ( et_initial_epoch < et_third_quarter ) && ( et_initial_epoch >= et_second_quarter ) ) {
	      quarter_initial_epoch = 3;
	    }

	    else if ( ( et_initial_epoch  >= et_third_quarter ) ){
	      quarter_initial_epoch = 4;
	    }
  ...
  int ifile_f107 = 0, ifile_ap = 0;
	...
	if ( OPTIONS->swpc_need_predictions == 1){
	  ...
	  double et_current_day_plus_45_day;
	  et_current_day_plus_45_day = et_current_day + 45. * 24 * 3600;
	etprint(et_current_day, "et_current_days");
	/* exit(0); */

	  char current_day_plus_45_day[256];
	  et2utc_c(et_current_day_plus_45_day, "ISOC", 0, 255, current_day_plus_45_day);
	  if ( ( et_initial_epoch >=  et_current_day_plus_45_day + 24*3600 ) || (  et_final_epoch >=  et_current_day_plus_45_day + 24*3600  ) ){ // actually plus 46  because we have the prediction on day + 45
	    printf("***! You chose to compute the drag with predictions of the solar activity from SWPC. However, these predictions are limited to 45 days in the future. Therefore, the spacecraft can not be propagated further in the future than the current date + 45 days (the propagation has to end before %s). The program will stop. !***\n", current_day_plus_45_day); MPI_Finalize(); exit(0);
	  }
	  else{
...
/* 	    strcat(filename_f107_ap_pred, "/"); */
 ...
 MPI_Barrier( MPI_COMM_WORLD ) ;
	...
	lin_interpolate_swpc_mod(OPTIONS->f107, OPTIONS->f107A, OPTIONS->Ap, OPTIONS->et_interpo, OPTIONS->mod_f107, OPTIONS->mod_ap, OPTIONS->et_mod_f107_ap, &OPTIONS->use_ap_hist, list_filename_obs_swpc_f107, list_filename_obs_swpc_ap, filename_f107_ap_pred,  OPTIONS->nb_time_steps * 2, OPTIONS->initial_epoch, OPTIONS->final_epoch, OPTIONS->dt, 999.9,iDebugLevel, nb_file_obs_swpc, nb_file_obs_swpc_ap, OPTIONS->swpc_need_observations, OPTIONS->swpc_need_predictions, &OPTIONS->swpc_et_first_prediction, iProc, filename_f107_ap_mod);
      // "* 2.0" because of the Runge Kunta orfer 4 method
...
/*     } */
	...
	inc_ensembles_output_final = inc_ensembles_output_final + 1;
      ...
      if (strcmp(filename_output_ensemble_temp[sss], "collision") == 0){
...
/*     strcat(text_location, "/"); */
    ...
    lin_interpolate_attitude(OPTIONS->quaternion,OPTIONS->pitch, OPTIONS->roll, OPTIONS->yaw, OPTIONS->order_pitch, OPTIONS->order_roll, OPTIONS->order_yaw, OPTIONS->et_interpo, OPTIONS->attitude_profile, OPTIONS->nb_time_steps * 2, OPTIONS->initial_epoch, OPTIONS->dt, iProc, &OPTIONS->file_is_quaternion);
...
int lin_interpolate(double *f107_after_interpo,
  ...
  int found_eoh = 0;
  char text[256];
  int line_num, index_to_stop_in_before_interpo=0, index_to_stop_in_before_interpo_for_f107_average =0;
  ...
  for (i = 1; i < nb_time_steps_simu-1; i++){
    ...
    if (  x_after_interpo[i] - x_f107_before_interpo[index_to_stop_in_before_interpo-1]  > 0.01 ){
      f107_after_interpo[i] = f107_before_interpo[index_to_stop_in_before_interpo-1];
    }
    else {
  ...
  if ( strcmp(src_file, "raid3") == 0 ){
	...
	if (  x_after_interpo[i] - x_ap_before_interpo[index_to_stop_in_before_interpo]  > 0.01 ){
	  ap_after_interpo[i] = ap_before_interpo[index_to_stop_in_before_interpo];
	}
	else {
	...
	ap_after_interpo[0] = ap_before_interpo[0];
      if( fabs( x_after_interpo[nb_time_steps_simu-1] - x_ap_before_interpo[index_to_stop_in_before_interpo] ) > 0.01 ) // same comments as the two previous lines
	ap_after_interpo[nb_time_steps_simu-1] = ap_after_interpo[nb_time_steps_simu-2];
      else // if both first times are the same then the first interpolated value of the driver is directly equal to the one on the original driver file
	ap_after_interpo[nb_time_steps_simu-1] = ap_before_interpo[index_to_stop_in_before_interpo];
	...
	printf("----- (load_options) (lin_interpolate) Starting to linear interpolate historical Ap.\n");
      ...
      lin_interpolate_ap_hist(ap_hist_after_interpo,x_after_interpo, ap_filename, src_file, nb_time_steps_simu, initial_epoch, dt, missing_data_value, iDebugLevel, iProc);     
    ...
    f107A_before_interpo = malloc( (index_to_stop_in_before_interpo - save_first_index_in_f107_before_interpo +1 ) * sizeof(double) );
    x_f107A_before_interpo = malloc( (index_to_stop_in_before_interpo - save_first_index_in_f107_before_interpo +1) * sizeof(double) );
 ...
 // // // // stop reading file at epoch end or one day before first prediction
      ...
      for (irow = 0; irow < nrow_pred; irow++){
  	  ...
  	  sscanf(line, "%s %lf %s %lf %s %lf %s %lf %s %lf", date_pred1, &ap_before_interpo_pred1, date_pred2, &ap_before_interpo_pred2, date_pred3, &ap_before_interpo_pred3, date_pred4, &ap_before_interpo_pred4, date_pred5, &ap_before_interpo_pred5);
    	 ...
    	 // // // // convert date into seconds past J2000
      	  ...
      	  str2et_c(date_pred_temp, &x_ap_before_interpo[index_nb_ap]);
  	  ap_before_interpo[index_nb_ap] = ap_before_interpo_pred1;
      	  if (x_ap_before_interpo[index_nb_ap] >= et_final_epoch_midnight + 24 * 3600){
  if ((iProc == 0) & ( iDebugLevel >= 5 ) ){
  	    ...
  	    nb_day_pred = index_nb_ap - nb_day_pred + 1 ; // nb_day_pred is equal to epoch end day - first value of prediction file + 1
      	  ...
      	  str2et_c(date_pred_temp, &x_ap_before_interpo[index_nb_ap+1]);
  	ap_before_interpo[index_nb_ap+1] = ap_before_interpo_pred2;
      	  if (x_ap_before_interpo[index_nb_ap+1] >= et_final_epoch_midnight + 24 * 3600){
  if ((iProc == 0) & ( iDebugLevel >= 5 ) ){
  	    ...
  	    nb_day_pred = index_nb_ap - nb_day_pred + 1  + 1; // nb_day_pred is equal to epoch end day - first value of prediction file + 1
      	  ...
      	  str2et_c(date_pred_temp, &x_ap_before_interpo[index_nb_ap+2]);
  	ap_before_interpo[index_nb_ap+2] = ap_before_interpo_pred3;
      	  if (x_ap_before_interpo[index_nb_ap+2] >= et_final_epoch_midnight + 24 * 3600){
  if ((iProc == 0) & ( iDebugLevel >= 5 ) ){
  	    ...
  	    nb_day_pred = index_nb_ap - nb_day_pred + 1  + 2; // nb_day_pred is equal to epoch end day - first value of prediction file + 1
      	  ...
      	  str2et_c(date_pred_temp, &x_ap_before_interpo[index_nb_ap+3]);
  	ap_before_interpo[index_nb_ap+3] = ap_before_interpo_pred4;
      	  if (x_ap_before_interpo[index_nb_ap+3] >= et_final_epoch_midnight + 24 * 3600){
  if ((iProc == 0) & ( iDebugLevel >= 5 ) ){
  	    ...
  	    nb_day_pred = index_nb_ap - nb_day_pred + 1  + 3; // nb_day_pred is equal to epoch end day - first value of prediction file + 1
      	  ...
      	  str2et_c(date_pred_temp, &x_ap_before_interpo[index_nb_ap+4]);
  	  ap_before_interpo[index_nb_ap+4] = ap_before_interpo_pred5;
      	  if (x_ap_before_interpo[index_nb_ap+4] >= et_final_epoch_midnight + 24 * 3600){
  if ((iProc == 0) & ( iDebugLevel >= 5 ) ){
  ...
  if ((iProc == 0) & ( iDebugLevel >= 5 ) ){
  	...
  	//printf("%s %f %s %f %s %f %s %f %s %f\n", date_pred1, f107_before_interpo[index_nb_f107], date_pred2, f107_before_interpo[index_nb_f107+1], date_pred3, f107_before_interpo[index_nb_f107+2], date_pred4, f107_before_interpo[index_nb_f107+3], date_pred5, f107_before_interpo[index_nb_f107+4]);
  	  ...
  	  if (x_f107_before_interpo[index_nb_f107] >= et_final_epoch_midnight + 24*3600){
	    //	    print_test();
  	    break;
  	  }
  	  ...
  	  if (x_f107_before_interpo[index_nb_f107+1] >= et_final_epoch_midnight + 24*3600){
	    //	    test_print("A");
  	    break;
  	  }
  	  ...
  	  str2et_c(date_pred_temp, &x_f107_before_interpo[index_nb_f107+2]);
  	  f107_before_interpo[index_nb_f107+2] = f107_before_interpo_pred3;
  	  if (x_f107_before_interpo[index_nb_f107+2] >= et_final_epoch_midnight + 24 * 3600){
  	    break;
  	  }
  	  ...
  	  str2et_c(date_pred_temp, &x_f107_before_interpo[index_nb_f107+3]);
  	  f107_before_interpo[index_nb_f107+3] = f107_before_interpo_pred4;
  	  if (x_f107_before_interpo[index_nb_f107+3] >= et_final_epoch_midnight + 24 * 3600){
  	    break;
  	  }
  	  ...
  	  str2et_c(date_pred_temp, &x_f107_before_interpo[index_nb_f107+4]);
  	  f107_before_interpo[index_nb_f107+4] = f107_before_interpo_pred5;
  	  if (x_f107_before_interpo[index_nb_f107+4] >= et_final_epoch_midnight + 24 * 3600){
  	    break;
  	  }
...
/*   /////////////////////////////////////////////////////////////////////////////////// */
  ...
  ///////////////////////////////////////////////////////////////////////////////////
	...
	//	      	printf("%f %f %f\n", x_mod_f107_ap_before_interpo[ipred], mod_f107_before_interpo[ipred], mod_ap_before_interpo[ipred]);
	ipred = ipred + 1;
      	}
      fclose(file_uncertainty);
  ...
  //  exit(0);
    for (i = 1; i < nb_time_steps_in_prediction; i++){
      ...
      //     printf("%d %f\n", x_min_index, x_mod_f107_ap_after_interpo[i]);
    // F10.7
    x_min = x_mod_f107_ap_before_interpo[x_min_index];
      ...
      //              printf("%f %f %f %f %d\n", mod_ap_after_interpo[i], x_mod_f107_ap_after_interpo[i], y_min, y_max, i);
  }
    //        exit(0);
  ...
  ssize_t read;
    ...
    sscanf(line, "%s", text);
     ...
     int count_space=0;
      ...
      for (icount_space = 0; icount_space< strlen(line); icount_space++){
	if (line[icount_space]==' '){
	  count_space = count_space + 1;
	  }
      }


      //    for (count_space=0; text[count_space]; text[count_space]==' ' ? count_space++ : *text++);
