...
int fly_storm( OPTIONS_T *OPTIONS,   char filename_storm_interpolated[300][N_STORM], int iCygnss, int compute_coverage, PARAMS_T *PARAMS)
{
 ...
 double radius_first_quadrant, radius_second_quadrant, radius_third_quadrant, radius_fourth_quadrant;
  ...
  int iGps, iPt;
    int iPtInner;
    float lon_spec, lat_spec, gain_spec, ecef_x_spec, ecef_y_spec, ecef_z_spec, time_ymdhmsm[7], normpower_spec; 
    int sss;
  ...
  while(!feof(file_specular_position_in)){
    ...
    if (!feof(file_specular_position_in)){
            fprintf(file_specular_position_out, "%s %f %f %f %s %e %f %f %f %f %f %f %f %f %f", time_spec, lon_spec, lat_spec, gain_spec, OPTIONS->gps_file_name[iGps], normpower_spec, ecef_x_sat, ecef_y_sat, ecef_z_sat, ecef_x_gps, ecef_y_gps, ecef_z_gps, ecef_x_spec, ecef_y_spec, ecef_z_spec );
