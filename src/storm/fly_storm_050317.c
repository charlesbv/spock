...
int main(int argc, char * argv[]) {
  ...
  load_options( &OPTIONS, filename_input, iProc, nProcs, main_directory_location, iDebugLevel, filename_input_no_path);
 

  // Load Params
  //newstructure
  load_params( &PARAMS, main_directory_location, iDebugLevel, OPTIONS.earth_fixed_frame, OPTIONS.use_ap_hist, iProc , OPTIONS.path_to_spice);
  ...
  for (iCygnss=iStart; iCygnss<iEnd; iCygnss++) {
    ...
    interpolate_position( filenamella, filenameecef,  OPTIONS.dt, 1.0, iCygnss, &OPTIONS, main_directory_location); // one second interpolation
  ...
  /* // Notify Exit */
  if (iProc == 0) {
    printf("Done flying over the storms.\n");
  }
 ...
 double radius_first_quadrant, radius_second_quadrant, radius_third_quadrant, radius_fourth_quadrant;
  ...
  while(!feof(file_specular_position_in)){
    ...
    if (!feof(file_specular_position_in)){
      fprintf(file_specular_position_out, "%s %f %f %f %s", time_spec, lon_spec, lat_spec, gain_spec, OPTIONS->gps_file_name[iGps]);
