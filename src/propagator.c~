...

{
  ...
  *density = *density * SC->INTEGRATOR.density_mod;
  ...
  if (OPTIONS->opengl == 1){ //!!!! For now, Sentman's equations for Cd (with acco coeff) can't be used with the option using openGL. This will be changed in the future.
    ...
    int itheta, iphi;
    itheta = (int)((theta - CONSTELLATION->area_attitude_opengl_theta0) / CONSTELLATION->area_attitude_opengl_dtheta);
    iphi = (int)((phi - CONSTELLATION->area_attitude_opengl_phi0) / CONSTELLATION->area_attitude_opengl_dphi);
    A_ref = CONSTELLATION->area_attitude_opengl[itheta][iphi]; // in km^2 (conversion from cm^2 to km^2 in initialize_constellation.c
    // // compute the drag acceleration. !!!! For now, Sentman's equations for Cd (with acco coeff) can't be used with the option using openGL. This will be changed in the future..
    // // // so here consider the cd of the first surface of the geometry file
    ...
    double norm_v_i2cg_NTW;
    v_norm(&norm_v_i2cg_NTW, v_i2cg_NTW);
    a_i2cg_NTW[0] = -0.5 * ballistic_coefficient * *density * norm_v_i2cg_NTW * v_i2cg_NTW[0];
    a_i2cg_NTW[1] = -0.5 * ballistic_coefficient * *density * norm_v_i2cg_NTW * v_i2cg_NTW[1];
    a_i2cg_NTW[2] = -0.5 * ballistic_coefficient * *density * norm_v_i2cg_NTW * v_i2cg_NTW[2];
