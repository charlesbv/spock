		    ...
		    )
{
  ...
  if (init_flag == 1) {
	...
	fprintf(SC[0].fprho ,"// Epoch of last TLE for %s: %s \n",  SC[0].name_sat, text);
      ...
      fprintf(SC[0].fprho, "// TIME DENSITY(kg/km^3) TEMP(K) CD_TOT_NORM A_REF_TOT(km^2)");
...
/* 				 SC[0].r_ecef2cg_ECEF) ; */
    ...
    if (write_rho == 1){
       	       fprintf(SC[0].fprho,"%s %e %f %f %e\n", text, SC[0].density_here, SC[0].INTEGRATOR.Ta, SC[0].INTEGRATOR.cd_tot_norm, SC[0].INTEGRATOR.A_ref_tot/1000000.); // in output A_ref_tot in km^2 */
    }
  ...
  if (sc_index < n_sc - n_gps){ // if main sc is not a GPS
	...
	if ( nb_ensembles_min > 0 ){
	  ...
	  if ( array_sc[1] > 0 )  { // if this iProc runs ensembles (otherwise array_sc[1] = - 1)
	    for (fff = 0; fff < nb_ensembles_output_files ; fff ++){
	      if ( (strcmp(OPTIONS->filename_output_ensemble[fff], "tca" )!= 0 ) && (strcmp(OPTIONS->filename_output_ensemble[fff], "dca" )!= 0 )){
		if ( ( strcmp( name_file_ensembles[fff], "cd" ) != 0 ) || ( ( strcmp( name_file_ensembles[fff], "cd" ) == 0 ) && ( constellation_et < et_intial_epoch + OPTIONS->dt_output/2. ) ) ) {
		fprintf(SC[0].fpiproc[fff], "\n%s", text );
		fprintf(SC[0].fpiproc[fff], " " ); 
		}
