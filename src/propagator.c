...
//newstructure
  ...
  if (gravity_map_use == 1){
    ...
    gravity_map( &(PARAMS->EARTH.GRAVITY), degree,  iProc);
  }
...
int gravity_map(GRAVITY_T  *Gravity, int degree,  int iProc){
  ...
  char text[200];
  strcpy(text, "");
  strcpy(filename_gravity_map, "grav");
  sprintf(text, "%d", degree);
  strcat(filename_gravity_map, text);
  strcat(filename_gravity_map, "_alt");
  sprintf(text, "%.1f", min_radius-Gravity->radius);
  strcat(filename_gravity_map, text);
  strcat(filename_gravity_map, "-");
  sprintf(text, "%.1f", max_radius-Gravity->radius);
  strcat(filename_gravity_map, text);
    strcat(filename_gravity_map, "-");
  sprintf(text, "%.1f", dradius );
  strcat(filename_gravity_map, text);
  strcat(filename_gravity_map, "_lat");
  ...
  strcat(filename_gravity_map, text);
    strcat(filename_gravity_map, "_lon");
  sprintf(text, "%.1f", dlon);
  strcat(filename_gravity_map, text);
  //strcat(filename_gravity_map, ".txt");  // txt
  strcat(filename_gravity_map, ".bin");
  // Gravity->file_gravity_map = fopen(filename_gravity_map, "w+"); // txt 
  Gravity->file_gravity_map = fopen(filename_gravity_map, "wb");
  /* fprintf(Gravity->file_gravity_map, "// ---------------------------------------------------------------------------------- \n"); */
  /* fprintf(Gravity->file_gravity_map, "// \n"); */
  /* fprintf(Gravity->file_gravity_map, "// Please note this file is auto-generated by the Spacecraft Orbital Characterization Kit (SpOCK)\n"); */

  /* fprintf(Gravity->file_gravity_map, "// 3D gravity map generated by SpOCK\n"); */
  /* fprintf(Gravity->file_gravity_map, "// Parameters of the map:\n"); */
    /* fprintf(Gravity->file_gravity_map, "// Gravity order: %d\n", degree); */
  /* fprintf(Gravity->file_gravity_map, "// radius: %.1f to %.1f (%.1f to %.1f), step %.1f (km)\n", min_radius, max_radius, min_radius-Gravity->radius, max_radius-Gravity->radius, dradius); */
  /* fprintf(Gravity->file_gravity_map, "// latitude: %.1f to %.1f, step %.1f (deg)\n", min_lat, max_lat, dlat); */
  /*   fprintf(Gravity->file_gravity_map, "// longitude: 0.0 to 360.0, step %.1f (deg)\n", dlon); */
  /* fprintf(Gravity->file_gravity_map, "// ---------------------------------------------------------------------------------- \n"); */
...
The program will stop. !***\n"); MPI_Finalize(); exit(0);
	...
	//	fprintf(Gravity->file_gravity_map, "%e %e %e ", Gravity->gravity_map[iradius][ilat][ilon][0], Gravity->gravity_map[iradius][ilat][ilon][1], Gravity->gravity_map[iradius][ilat][ilon][2]); // on one line: all longitudes for a given latitude // txt
	// on one line: all longitudes for a given latitude 
	fwrite(&(Gravity->gravity_map[iradius][ilat][ilon][0]), sizeof(Gravity->gravity_map[iradius][ilat][ilon][0]), 1, Gravity->file_gravity_map);
	fwrite(&(Gravity->gravity_map[iradius][ilat][ilon][1]), sizeof(Gravity->gravity_map[iradius][ilat][ilon][1]), 1, Gravity->file_gravity_map);
	fwrite(&(Gravity->gravity_map[iradius][ilat][ilon][2]), sizeof(Gravity->gravity_map[iradius][ilat][ilon][2]), 1, Gravity->file_gravity_map); 
