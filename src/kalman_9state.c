...
int kalman_computeA( double A[9][9], SPACECRAFT_T *sc, PARAMS_T *PARAMS , double dt, double tau, OPTIONS_T *OPTIONS,  CONSTELLATION_T *CONSTELLATION, int iProc, int iDebugLevel) {
	 ...
	 r[ii] = r[ii] + 0.0005;
	 	 compute_gravity( a2, r, sc->et, &PARAMS->EARTH.GRAVITY, sc->INTEGRATOR.degree, PARAMS->EARTH.earth_fixed_frame, PARAMS->EARTH.flattening, PARAMS->EARTH.radius, sc);
...
/*        double test_density; */
       ...
       //compute_dxdt( v_dummy, a2, &sc->et,  r, sc->v_i2cg_INRTL, PARAMS, &sc->INTEGRATOR, et_initial_epoch, OPTIONS->et_oldest_tle_epoch, &test_density, sc->INTEGRATOR.index_in_attitude_interpolated, sc->INTEGRATOR.index_in_driver_interpolated, CONSTELLATION, OPTIONS, iProc, iDebugLevel, sc); // !!!!!! by cbv: remove

	 v_copy( r, sc->r_i2cg_INRTL);
	 r[ii] = r[ii] - 0.0005;
	 //	 printf("%f\n", sc->INTEGRATOR.degree);
	 compute_gravity( a1, r, sc->et, &PARAMS->EARTH.GRAVITY, sc->INTEGRATOR.degree, PARAMS->EARTH.earth_fixed_frame, PARAMS->EARTH.flattening, PARAMS->EARTH.radius, sc); // !!!!!!!! uncomment
	 //			 	 compute_dxdt( v_dummy, a1, &sc->et,  r, sc->v_i2cg_INRTL, PARAMS, &sc->INTEGRATOR, et_initial_epoch, OPTIONS->et_oldest_tle_epoch, &test_density, sc->INTEGRATOR.index_in_attitude_interpolated, sc->INTEGRATOR.index_in_driver_interpolated, CONSTELLATION, OPTIONS, iProc, iDebugLevel, sc); // !!!!!! by cbv: remove
...
/* // !!!!!!!! comment below (cbv) */
/* 	 v_copy( v, sc->v_i2cg_INRTL); */
/* 	 v[ii] = v[ii] + 0.0000005; */
/* 	 //	 compute_gravity( a2, r, sc->et, &PARAMS->EARTH.GRAVITY, sc->INTEGRATOR.degree, PARAMS->EARTH.earth_fixed_frame, PARAMS->EARTH.flattening, PARAMS->EARTH.radius, sc); */
	

/* 	 compute_dxdt( v_dummy, a2, &sc->et,  sc->r_i2cg_INRTL, v, PARAMS, &sc->INTEGRATOR, et_initial_epoch, OPTIONS->et_oldest_tle_epoch, &test_density, sc->INTEGRATOR.index_in_attitude_interpolated, sc->INTEGRATOR.index_in_driver_interpolated, CONSTELLATION, OPTIONS, iProc, iDebugLevel, sc); // !!!!!! by cbv: remove */

/* 	 v_copy( v, sc->v_i2cg_INRTL); */
/* 	 v[ii] = v[ii] - 0.0000005; */
/* 	 //	 printf("%f\n", sc->INTEGRATOR.degree); */
/* 	 //compute_gravity( a1, r, sc->et, &PARAMS->EARTH.GRAVITY, sc->INTEGRATOR.degree, PARAMS->EARTH.earth_fixed_frame, PARAMS->EARTH.flattening, PARAMS->EARTH.radius, sc); // !!!!!!!! uncomment */
/* 	 compute_dxdt( v_dummy, a1, &sc->et,  sc->r_i2cg_INRTL, v, PARAMS, &sc->INTEGRATOR, et_initial_epoch, OPTIONS->et_oldest_tle_epoch, &test_density, sc->INTEGRATOR.index_in_attitude_interpolated, sc->INTEGRATOR.index_in_driver_interpolated, CONSTELLATION, OPTIONS, iProc, iDebugLevel, sc); // !!!!!! by cbv: remove */
...
int kalman_computeSTM( double Phi[9][9], SPACECRAFT_T *sc, PARAMS_T *PARAMS, double dt, double tau,OPTIONS_T *OPTIONS,  CONSTELLATION_T *CONSTELLATION, int iProc, int iDebugLevel ) {
     ...
     /* printf(" --------- A2 --------- \n"); */
     /* for (jj = 0; jj < 9; jj++ ){ */
     /*   printf("   %e %e %e %e %e %e %e %e %e\n", A2[jj][0], A2[jj][1],A2[jj][2],A2[jj][3],A2[jj][4],A2[jj][5], A2[jj][6],A2[jj][7],A2[jj][8] ); */
     /* } */
...
int kalman_computeQ( double Q[9][9], double Phi[N_STATES][N_STATES], double dt, double tau, double sigma, FILE *fp_kalman_init) {
  ...
  size_t len = 0;
     ...
     m_x_mt9_for_kalman( Qtemp, Qo, Phi);
     m_x_m9_for_kalman( Q, Phi, Qtemp );
