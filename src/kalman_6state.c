...
int kalman_filt( MEAS_T *meas, KALMAN_T *kf, PARAMS_T *params, OPTIONS_T *OPTIONS, GROUND_STATION_T *GROUND_STATION, CONSTELLATION_T  *CONSTELLATION, int iProc, int iDebugLevel, int nProcs ) {
 ...
 int kalman_joseph_update(  KALMAN_T *kf, double H[6], int meas_i) {
     ...
     // Lazy filter update
     m_x_m6_for_kalman( Pnew, I_KH, kf->P);
     m_x_mt6_for_kalman( Pnew2, Pnew, I_KH);
     m_add6_for_kalman( kf->P , Pnew2, KRK);
...
int kalman_init( MEAS_T *meas, KALMAN_T *kf, CONSTELLATION_T  *CONSTELLATION) {
  ...
  size_t len = 0;
     ...
     double T[3][3]      = {{0.0}};
     double T6[6][6]     = {{0.0}};
     double Ptemp6[6][6] = {{0.0}};
     double Plvlh[6][6]  = {{0.0}};
...
/*      } */
	 ...
	 for(jj = 0; jj < 3; jj++ ) {
	     ...
	     T6[ii][jj]     = T[ii][jj];
	     T6[ii+3][ii+3] = T[ii][jj]; // !!!!!!!!! cbv: mistake? shouldn't it be: T6[ii+3][jj+3] = T[ii][jj]; ?
     ...
     m_x_mt6_for_kalman( Ptemp6, Plvlh, T6);
     m_x_m6_for_kalman( kf->P, T6, Ptemp6);
...
kf->read_q = 1; 
 ...
 int kalman_cov_prop( KALMAN_T *kf, PARAMS_T *PARAMS ) {
     ...
     // P = Phi * P * Phi^t + Q
     m_x_m6_for_kalman( PhiP, kf->Phi, kf->P);
     m_x_mt6_for_kalman( Pnew, PhiP, kf->Phi);
     m_add6_for_kalman( kf->P, Pnew, kf->Q ); // OK
 ...
 int kalman_computeSTM( double Phi[6][6], SPACECRAFT_T *sc, PARAMS_T *PARAMS, double dt, double tau ) {
     ...
     /* printf(" --------- STM --------- \n"); */
     /* for (jj = 0; jj < 6; jj++ ){ */
     /* 	 printf("   %e %e %e %e %e %e \n", Phi[jj][0], Phi[jj][1],Phi[jj][2],Phi[jj][3],Phi[jj][4],Phi[jj][5] ); */
     /* } */
...
/*   } */
 ...
 int kalman_write_out( KALMAN_T *kf, FILE *fp) {
    ...
    /*  Area = kf->sc.INTEGRATOR.surface[0].area / ( 1000.0 * 1000 ); // !!!!!! all surfaces */
