...
if [ $compiler = "gcc" ]; then
    ...
    # # Specular points binary files
    $path_compiler -o "$path_spock_abso"/spec ./src/find_specular_points.c -lm # change the path of the executable here  
    # Convert specular points binary files to txt files
    cd ./src/storm
    make clean PATH_EXECUTABLE="$path_spock_abso"
    make all PATH_COMPILER=$path_compiler PATH_SPICE="$path_spice_abso" PATH_GSL="$path_gsl_abso" PATH_EXECUTABLE="$path_spock_abso"
...
if [ $is_sift -eq 1 ];then # if compiling SpOCK to run it within SIFT
    ...
    declare -a py_script=("spock_cygnss_spec_parallel.py"
                "cygnss_tle.py" "gps_tle.py"
                "spock_main_input.py" "read_input_file.py"
		"norad_id_to_cygnss_name.py" "cygnss_read_spock_spec.py"
		"read_output_file.py" "ir_brightness_with_cygnss.py"
		"cygnss_animation.py" "report_coverage_ground_station.py"
